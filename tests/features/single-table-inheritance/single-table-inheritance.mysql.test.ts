...
describe("single table inheritance in mysql", () => {
  ...
  test("check metadata", async () => {
    expect(orm.getMetadata().get("CompanyOwner2").collection).toBe(
      "base_user2",
    );
  ...
  test("loading STI entities respects the entity type (GH #1252)", async () => {
    ...
    expect(managers.map((u) => u.constructor.name)).toEqual([
      "Manager2",
      "CompanyOwner2",
    ]);
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "select `c0`.* from `base_user2` as `c0` where `c0`.`type` = 'owner'",
    );
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      "select `e0`.* from `base_user2` as `e0` where `e0`.`type` = 'employee'",
    );
    expect(employees.length).toBe(2);
    expect(employees.map((u) => u.constructor.name)).toEqual([
      "Employee2",
      "Employee2",
    ]);
    ...
    expect(mock.mock.calls[3][0]).toMatch(
      "select `b0`.* from `base_user2` as `b0`",
    );
    expect(users.length).toBe(4);
    expect(users.map((u) => u.constructor.name).sort()).toEqual([
      "CompanyOwner2",
      "Employee2",
      "Employee2",
      "Manager2",
  ...
  test("persisting and loading STI entities", async () => {
    ...
    expect((users[3] as CompanyOwner2).favouriteEmployee).toBeInstanceOf(
      Employee2,
    );
    expect((users[3] as CompanyOwner2).favouriteManager).toBeInstanceOf(
      Manager2,
    );
    ...
    expect(Object.keys(users[1])).toEqual([
      "firstName",
      "lastName",
      "type",
      "employeeProp",
    ...
    expect(Object.keys(users[2])).toEqual([
      "firstName",
      "lastName",
      "type",
      "managerProp",
    ...
    expect(Object.keys(users[3])).toEqual([
      "firstName",
      "lastName",
      "type",
      "managerProp",
  ...
  test("STI in m:1 and 1:1 relations", async () => {
    await createEntities();
    const owner = await orm.em.findOneOrFail(CompanyOwner2, {
      firstName: "Bruce",
    });
  ...
  test("generated discriminator map", async () => {
    const storage = new MetadataStorage({
      A: {
        ...
        properties: {
          id: { name: "id", type: "string", kind: ReferenceKind.SCALAR },
        },
      },
      B: {
        name: "B",
    ...
    expect(discovered.get("A").discriminatorMap).toEqual({
      a: "A",
      b: "B",
      c: "C",
    });
