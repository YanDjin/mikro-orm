...
describe("changing column in mysql (GH 2407)", () => {
  ...
  test("schema generator respect indexes on FKs on column update", async () => {
    ...
    expect(diff3).toBe(
      "alter table `book` modify `my_column` tinyint(1) null default 123;\n\n",
    );
    ...
    expect(diff4).toBe(
      "alter table `book` modify `my_column` tinyint(1) null default 123 comment 'lalala';\n\n",
    );
    ...
    expect(diff5).toBe(
      "alter table `book` modify `my_column` tinyint(1) null default 123 comment 'lololo';\n\n",
    );
