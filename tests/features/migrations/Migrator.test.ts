...
import type { DatabaseTable, MySqlDriver } from "@yandjin-mikro-orm/mysql";
import { DatabaseSchema, SchemaGenerator } from "@yandjin-mikro-orm/mysql";
...
describe("Migrator", () => {
  ...
  test("initial migration cannot be created if tables already exist", async () => {
    ...
    schemaMock.mockReturnValueOnce([
      { name: "author2" } as DatabaseTable,
      { name: "book2" } as DatabaseTable,
    ]);
  ...
  test("migration is skipped when no diff", async () => {
    ...
    const migrator = new Migrator(orm.em);
    const schemaGeneratorMock = jest.spyOn<any, any>(
  ...
  test("unlogging migrations work even if they have extension", async () => {
    ...
    await orm.em.execute(
      `update ${orm.config.get("migrations").tableName!} set name = 'test.migration.ts'`,
    );
    await expect(storage.executed()).resolves.toEqual(["test.migration"]);
