...
describe("polymorphic embeddables in sqlite", () => {
  ...
  test(`diffing`, async () => {
    const hydrator = orm.config.getHydrator(
      orm.getMetadata(),
    ) as ObjectHydrator;
    expect(
      hydrator
        .getEntityHydrator(orm.getMetadata().get("Owner"), "full")
        .toString(),
    ).toMatchSnapshot();
    expect(
      orm.em.getComparator().getSnapshotGenerator("Owner").toString(),
    ).toMatchSnapshot();
  });
  ...
  test(`working with polymorphic embeddables`, async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `owner` (`name`, `pet_type`, `pet_name`, `pet_can_meow`, `pet2`, `pets`, `pet_can_bark`) values (?, ?, ?, ?, ?, ?, ?), (?, ?, ?, ?, ?, ?, ?), (?, ?, ?, ?, ?, ?, ?)",
    );
  ...
  test("assign and serialization", async () => {
    ...
    expect(owner.pets[0]).toMatchObject({
      name: "dog in array",
      type: AnimalType.DOG,
    });
    expect(owner.pets[0]).toBeInstanceOf(Dog);
    expect(owner.pets[1]).toMatchObject({
      name: "cat in array",
      type: AnimalType.CAT,
    });
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `owner` (`name`, `pet_type`, `pet_name`, `pet_can_meow`, `pet2`, `pets`) values (?, ?, ?, ?, ?, ?)",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
    ...
    expect(mock.mock.calls[4][0]).toMatch(
      "update `owner` set `pet_name` = ?, `pet2` = ?, `pets` = ? where `id` = ?",
    );
