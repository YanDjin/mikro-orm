...
describe("GH issue 2095", () => {
  ...
  test("ordered query with limit/offset works", async () => {
    ...
    const firstMaxDate = new Date(
      Math.max(...firstResults.map((e) => e.end.getTime())),
    );
    const secondMinDate = new Date(
      Math.min(...secondResults.map((e) => e.end.getTime())),
    );
    ...
    await orm.em
      .fork()
      .find(T, {}, { orderBy: { tc: { id: "desc" } }, limit: 5 });

    // this requires casting of the bool column to use it in `min()` (GH #2910)
    await orm.em
      .fork()
      .find(T, {}, { orderBy: { tc: { bool: "desc" } }, limit: 5 });
  });
