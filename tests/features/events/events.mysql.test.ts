...
import { EventType, LoadStrategy, wrap } from "@yandjin-mikro-orm/core";
...
describe("events (mysql)", () => {
  ...
  test("hooks", async () => {
    ...
    expect(author.hookParams[0].changeSet).toMatchObject({
      entity: author,
      type: "create",
      payload: { name: "Jon Snow" },
    });
    ...
    expect(
      Author2Subscriber.log.map((l) => [l[0], l[1].entity.constructor.name]),
    ).toEqual([
  ...
  test("subscribers", async () => {
    ...
    expect(
      EverythingSubscriber.log.map((l) => [l[0], l[1].entity.constructor.name]),
    ).toEqual([
  ...
  test("onLoad event", async () => {
    ...
    const authors2 = await orm.em
      .fork()
      .find(Author2, {}, { populate: ["books"] });
    ...
    const authors4 = await orm.em
      .fork()
      .find(
        Author2,
        {},
        { populate: ["books.tags"], strategy: LoadStrategy.JOINED },
      );
  ...
  test("subscribers in forked entity managers", async () => {
    ...
    forkedEm
      .getEventManager()
      .registerSubscriber(new ManualAuthor2Subscriber());
    ...
    expect(
      ManualAuthor2Subscriber.log.map((l) => [
        l[0],
        l[1].entity.constructor.name,
      ]),
    ).toEqual([
      ["onInit", "Author2"],
      ["beforeUpdate", "Author2"],
