...
import type { EntityMetadata, MikroORM } from "@yandjin-mikro-orm/core";
import { compareObjects, Utils } from "@yandjin-mikro-orm/core";
...
describe("Utils", () => {
  ...
  test("equals", () => {
    ...
    expect(Utils.equals(Buffer.from([1, 2, 3]), Buffer.from([1, 2, 3]))).toBe(
      true,
    );
    expect(
      Utils.equals(Buffer.from([1, 2, 3, 4]), Buffer.from([1, 2, 3])),
    ).toBe(false);
    expect(
      Utils.equals(Buffer.from([1, 2, 3, 4]), Buffer.from([1, 2, 3, 5])),
    ).toBe(false);
    expect(Utils.equals({ a: "a", b: "c" }, { a: "b", b: "c" })).toBe(false);
    expect(Utils.equals({ a: "a", b: "b", c: "c" }, { a: "b", b: "b" })).toBe(
      false,
    );
  ...
  test("merge", () => {
    ...
    ).toEqual({ a: "b", b: "c", c: { a: "u", d: "e", f: ["g", "h"] } });
    ...
    expect(Utils.mergeConfig({ a: "a", b: ["c"] }, { a: undefined })).toEqual({
      a: "a",
      b: ["c"],
    });
    ...
    expect(
      Utils.merge(
        { nestedData: null, moreDeep: { moreDeepData: null } },
        source,
      ),
    ).toEqual(source);
    expect(
      Utils.merge(
        { nestedData: "test", moreDeep: { moreDeepData: "test" } },
        source,
      ),
    ).toEqual(source);
    expect(
      Utils.merge({ nestedData: {}, moreDeep: { moreDeepData: {} } }, source),
    ).toEqual(source);
  });
  ...
  test("diff", () => {
    ...
    expect(
      Utils.diff({ a: new Date() }, { a: new Date("2018-01-01") }),
    ).toEqual({ a: new Date("2018-01-01") });
    expect(
      Utils.diff(
        { a: new ObjectId("00000001885f0a3cc37dc9f0") },
        { a: new ObjectId("00000001885f0a3cc37dc9f0") },
      ),
    ).toEqual({});
  });
  ...
  test("relativePath", () => {
    ...
    expect(Utils.relativePath("test", process.cwd())).toBe("./test");
    expect(Utils.relativePath(process.cwd() + "/tests/", process.cwd())).toBe(
      "./tests",
    );
    expect(
      Utils.relativePath(process.cwd() + "/tests/cli/", process.cwd()),
    ).toBe("./tests/cli");
    expect(
      Utils.relativePath(
        pathToFileURL(process.cwd() + "/tests/cli/").href,
        process.cwd(),
      ),
    ).toBe("./tests/cli");
    expect(
      Utils.relativePath(
        process.cwd() + "/tests/cli/",
  ...
  test("absolutePath", () => {
    expect(Utils.absolutePath("./test")).toBe(
      Utils.normalizePath(process.cwd() + "/test"),
    );
    expect(Utils.absolutePath("test")).toBe(
      Utils.normalizePath(process.cwd() + "/test"),
    );
    expect(Utils.absolutePath(process.cwd() + "/tests/")).toBe(
      Utils.normalizePath(process.cwd() + "/tests"),
    );
    expect(Utils.absolutePath("./tests/cli")).toBe(
      Utils.normalizePath(process.cwd() + "/tests/cli"),
    );
    expect(Utils.absolutePath("")).toBe(Utils.normalizePath(process.cwd()));
    expect(
      Utils.absolutePath(pathToFileURL(process.cwd() + "/tests/").href),
    ).toBe(Utils.normalizePath(process.cwd() + "/tests"));
  });
  ...
  test("pathExists wrapper", async () => {
    ...
    await expect(
      Utils.pathExists("**/tests", { onlyDirectories: true }),
  ...
  test("extractEnumKeys", async () => {
    ...
    expect(Utils.extractEnumValues(PublisherType2)).toEqual([
      "LOCAL",
      "GLOBAL",
    ]);
    expect(Utils.extractEnumValues(PublisherType3)).toEqual([
      "local",
      "GLOBAL",
    ]);
  ...
  test("lookup path from decorator", () => {
    ...
    expect(Utils.lookupPathFromDecorator("Customer", stack1)).toBe(
      "/usr/local/var/www/my-project/dist/entities/Customer.js",
    );
    ...
    expect(Utils.lookupPathFromDecorator("Customer", stack2)).toBe(
      "/usr/local/var/www/my-project/src/entities/Customer.ts",
    );
    ...
    expect(Utils.lookupPathFromDecorator("Customer", stack3)).toBe(
      "/usr/local/var/www/my-project/dist/entities/Customer.js",
    );
    ...
    expect(
      Utils.lookupPathFromDecorator("Customer", [
        "    at Object.__decorate (/usr/local/var/www/my-project/node_modules/tslib/tslib.js:92:96)",
        "    at Object.<anonymous> ( ... )",
      ]),
    ).toBe("Customer");
    ...
    expect(
      ...
      "/opt/app/packages/entity/dist/node/entity/AuthorizationTokenEntity.js",
    );
    ...
    expect(
      ...
      "/opt/app/packages/entity/dist/node/entity/AuthorizationTokenEntityFromDecorate.js",
    );
    ...
    expect(
      ...
      "/opt/app/packages/entity/dist/node/entity/AuthorizationTokenEntityFromReflectDecorate.js",
    );
  ...
  test("lookup path from decorator on windows", () => {
    ...
    expect(Utils.lookupPathFromDecorator("Customer", stack1)).toBe(
      "C:/www/my-project/src/entities/Customer.ts",
    );
  });
  ...
  test("removeDuplicates", () => {
    expect(Utils.removeDuplicates(["foo", "bar", "foo", "bar2"])).toEqual([
      "foo",
      "bar",
      "bar2",
    ]);
