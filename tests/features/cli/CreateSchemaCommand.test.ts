...
const getCreateSchemaSQL = jest.spyOn(
...
describe("CreateSchemaCommand", () => {
  ...
  test("handler", async () => {
    ...
    expect(getCreateSchemaSQL).toHaveBeenCalledWith({
      wrap: undefined,
      dump: true,
    });
    expect(seed.mock.calls.length).toBe(0);
    ...
    expect(seed.mock.calls.length).toBe(0);
    await expect(
      cmd.handler({ run: true, fkChecks: true, seed: "" } as any),
    ...
    await expect(
      cmd.handler({ run: true, seed: "UsersSeeder" } as any),
    ).resolves.toBeUndefined();
