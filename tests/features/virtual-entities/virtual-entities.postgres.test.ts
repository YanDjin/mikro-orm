...
import { EntityManager, MikroORM } from "@yandjin-mikro-orm/postgresql";
...
const authorProfilesSQL =
  "select min(name) as name, favourite_book_uuid_pk, min(age) as age, identity, " +
...
const BookWithAuthor = new EntitySchema<IBookWithAuthor>({
  ...
  expression: (em: EntityManager) => {
    return em
      ...
      .select([
        sql`min(b.title)`.as("title"),
        "author",
        sql`min(a.name)`.as("author_name"),
        raw("array_agg(t.name) as tags"),
      ])
      .join("b.author", "a")
      .join("b.tags", "t")
      .groupBy("b.uuid_pk");
...
describe("virtual entities (sqlite)", () => {
  ...
  async function createEntities(index: number): Promise<Author2> {
    ...
    const book1 = orm.em.create(Book2, {
      title: "My Life on the Wall, part 1/" + index,
      author,
    });
    const book2 = orm.em.create(Book2, {
      title: "My Life on the Wall, part 2/" + index,
      author,
    });
    const book3 = orm.em.create(Book2, {
  ...
  test("with SQL expression", async () => {
    ...
    const res2 = await orm.em.findAndCount(
      AuthorProfile,
      {},
      { cache: 50, orderBy: { name: 1, usedTags: 1 } },
    );
    ...
    const someProfiles4 = await orm.em.find(AuthorProfile, {
  ...
  test("with callback", async () => {
    ...
    const someBooks4 = await orm.em.find(
      BookWithAuthor,
      {
        title: [
          "My Life on the Wall, part 1/2",
    ...
    const sql =
      'select min(b.title) as title, "b"."author_id", min(a.name) as author_name, array_agg(t.name) as tags ' +
      'from "book2" as "b" ' +
    ...
    expect(queries[0]).toMatch(
      `select "a0".*, "f1"."uuid_pk" as "favourite_book_uuid_pk", "a2"."author_id" as "address_author_id" from "author2" as "a0" left join "book2" as "f1" on "a0"."favourite_book_uuid_pk" = "f1"."uuid_pk" and "f1"."author_id" is not null left join "address2" as "a2" on "a0"."id" = "a2"."author_id" where "a0"."id" in (1, 2, 3)`,
    );
    ...
    expect(
      orm.em
        ...
        .keys()
        .map((k) => k.split("-")[0]),
    ).toEqual(["Author2", "Author2", "Author2", "Book2", "Book2", "Book2"]);
