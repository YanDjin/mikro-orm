...
import { CLIConfigurator, CLIHelper } from "@yandjin-mikro-orm/cli";
...
import type * as mikroOrmCoreModule from "@yandjin-mikro-orm/core";
...
jest.mock(
  (global.normalizedCwd = jest
    .requireActual<typeof mikroOrmCoreModule>("@mikro-orm/core")
    .Utils.normalizePath(process.cwd())) + "/mikro-orm.config.js",
  () => ({ driver: MongoDriver, dbName: "foo_bar", entities: ["tests/foo"] }),
  { virtual: true },
);
jest.mock(
  global.normalizedCwd + "/mikro-orm.config.ts",
  () => ({ driver: MongoDriver, dbName: "foo_bar", entities: ["tests/foo"] }),
  { virtual: true },
);
jest.mock(
  global.normalizedCwd + "/mikro-orm-async.config.js",
  () =>
    Promise.resolve({
      driver: MongoDriver,
      dbName: "foo_bar",
...
jest.mock(
  global.normalizedCwd + "/mikro-orm-async-catch.config.js",
  () => Promise.reject("FooError"),
  { virtual: true },
);
...
jest.mock(global.resolvedCwd + "tsconfig.base.json", () => tscBase, {
  virtual: true,
});
...
jest.mock(
  global.resolvedCwd + "tsconfig.extended-abs.json",
  () => tscExtendedAbs,
  { virtual: true },
);

const tscExtended = {
  extends: "./tsconfig.extended-abs.json",
  compilerOptions: { module: "commonjs" },
} as any;
jest.mock(global.resolvedCwd + "tsconfig.extended.json", () => tscExtended, {
  virtual: true,
});

const tscWithoutBaseUrl = {
  compilerOptions: { paths: { "@some-path/some": "./libs/paths" } },
};
jest.mock(
  global.resolvedCwd + "tsconfig.without-baseurl.json",
  () => tscWithoutBaseUrl,
  { virtual: true },
);
...
describe("CLIHelper", () => {
  ...
  beforeEach(() => {
    pathExistsMock = jest.spyOn(require("fs-extra"), "pathExists");
  ...
  test("configures yargs instance [ts-node]", async () => {
    ...
    const cli = (await CLIConfigurator.configure()) as any;
    expect(cli.$0).toBe("mikro-orm");
    expect(requireFromMock).toHaveBeenCalledWith(
      "ts-node",
  ...
  test("configures yargs instance [ts-node and ts-paths and tsconfig.extends]", async () => {
    ...
    expect(requireFromMock).toHaveBeenCalledWith(
      "ts-node",
      resolve(process.cwd(), "./tsconfig.extended-abs.json"),
    );
    expect(requireFromMock).toHaveBeenCalledWith(
      "tsconfig-paths",
      resolve(process.cwd(), "./tsconfig.extended-abs.json"),
  ...
  test("configures yargs instance [ts-node and ts-paths] without baseUrl", async () => {
    ...
    expect(requireFromMock).toHaveBeenCalledWith(
      "ts-node",
      resolve(process.cwd(), "./tsconfig.without-baseurl.json"),
    );
    expect(requireFromMock).toHaveBeenCalledWith(
      "tsconfig-paths",
      resolve(process.cwd(), "./tsconfig.without-baseurl.json"),
  ...
  test("gets ORM configuration [no mikro-orm.config]", async () => {
    ...
    await expect(CLIHelper.getConfiguration()).rejects.toThrow(
      `MikroORM config file not found in ['./src/mikro-orm.config.js', './mikro-orm.config.js']`,
    );

    process.env.MIKRO_ORM_ENV = __dirname + "/../../mikro-orm.env";
    await expect(CLIHelper.getConfiguration()).resolves.toBeInstanceOf(
      Configuration,
    );
  });
  ...
  test("disallows version mismatch of ORM packages", async () => {
    ...
    await expect(ConfigurationLoader.checkPackageVersion()).resolves.toMatch(
      /^\d+\.\d+\.\d+/,
    );
    ...
    await expect(ConfigurationLoader.checkPackageVersion()).rejects
      .toThrow(`Bad @mikro-orm/weird-package version 1.2.3.
...
Maybe you want to check, or regenerate your yarn.lock or package-lock.json file?`);
  ...
  test("registerTsNode works with tsconfig.json with comments", async () => {
    ...
    await expect(
      ConfigurationLoader.registerTsNode(__dirname + "/../tsconfig.json"),
    ).resolves.toBe(true);
    ...
    await expect(
      ConfigurationLoader.registerTsNode("./tests/tsconfig.json"),
    ).resolves.toBe(true);
  ...
  test("gets ORM configuration [no package.json]", async () => {
    pathExistsMock.mockImplementation(async (path) => {
      ...
      return (
        str.includes("/mikro-orm.config.js") &&
        !str.includes("/src/mikro-orm.config.js")
      );
    });
  ...
  test("gets ORM configuration [from package.json] with promise", async () => {
    pathExistsMock.mockResolvedValue(true);
    pkg["mikro-orm"].configPaths = [
      `${Utils.normalizePath(process.cwd())}/mikro-orm-async.config.js`,
    ];
    const conf = await CLIHelper.getConfiguration();
    expect(conf).toBeInstanceOf(Configuration);
    expect(conf.get("dbName")).toBe("foo_bar");
  ...
  test("gets ORM configuration [from package.json] with rejected promise", async () => {
    ...
    pathExistsMock.mockResolvedValue(true);
    pkg["mikro-orm"].configPaths = [
      `${Utils.normalizePath(process.cwd())}/mikro-orm-async-catch.config.js`,
    ];
  ...
  test("gets ORM instance [ts-node]", async () => {
    pathExistsMock.mockImplementation(async (path) => {
      ...
      return (path as string).endsWith(
        Utils.normalizePath(process.cwd() + "/mikro-orm.config.ts"),
      );
    });
    pkg["mikro-orm"].useTsNode = true;
    await expect(CLIHelper.getORM()).rejects.toThrow(
  ...
  test("dump", async () => {
    ...
    CLIHelper.dump(
      "select 1 + 1",
      new Configuration(
        { driver: SqliteDriver, highlighter: new SqlHighlighter() },
        false,
      ),
    );
    process.env.FORCE_COLOR = "0";
    ...
    expect(logSpy.mock.calls[1][0]).toMatch(
      "[37m[1mselect[22m[39m [32m1[39m [0m+[0m [32m1[39m",
    );
  ...
  test("getSettings", async () => {
    ...
    ).resolves.toBeInstanceOf(Configuration);
  ...
  test("getPackageConfig checks parent folders for package.json", async () => {
    ...
    const ret2 = await ConfigurationLoader.getPackageConfig(
      process.cwd() + "/../..",
    );
    expect(ret2).toEqual({});
  ...
  test("getConfigPaths", async () => {
    (global as any).process.argv = [
      "node",
      "start.js",
      "--config",
      "./override1/orm-config.ts",
    ];
    ...
    (global as any).process.argv = [
      "node",
      "start.js",
      "--config=./override2/orm-config.ts",
    ];
    await expect(CLIHelper.getConfigPaths()).resolves.toEqual([
      "./override2/orm-config.ts",
    ]);
    (global as any).process.argv = [
      "npx",
      "mikro-orm",
      "debug",
      "--config",
      "./override3/orm-config.ts",
    ];
    await expect(CLIHelper.getConfigPaths()).resolves.toEqual([
      "./override3/orm-config.ts",
    ]);
    (global as any).process.argv = ["node", "start.js"];
    (global as any).process.env.MIKRO_ORM_CLI_CONFIG =
