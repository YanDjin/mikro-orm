...
import { MetadataStorage, MikroORM } from "@yandjin-mikro-orm/core";
import {
  Migration,
  MigrationStorage,
  Migrator,
  TSMigrationGenerator,
} from "@yandjin-mikro-orm/migrations";
import type { DatabaseTable } from "@yandjin-mikro-orm/postgresql";
import {
  DatabaseSchema,
  PostgreSqlDriver,
...
describe("Migrator (postgres)", () => {
  ...
  beforeAll(async () => {
    orm = await MikroORM.init<PostgreSqlDriver>({
      ...
      logger: () => void 0,
      migrations: {
        path: BASE_DIR + "/../temp/migrations-456",
        snapshot: false,
      },
      extensions: [Migrator],
    });
  ...
  test("generate migration with custom name", async () => {
    ...
    await migrator.up();
    await migrator.down(
      migration.fileName.replace("migration-", "").replace(".ts", ""),
    );
    orm.config.set("migrations", migrationsSettings); // Revert migration config changes
    await remove(process.cwd() + "/temp/migrations-456/" + migration.fileName);
    upMock.mockRestore();
  ...
  test("generate migration with custom name with name option", async () => {
    ...
    orm.config.set("migrations", {
      ...migrationsSettings,
      fileName: (time, name) => `migration${time}_${name}`,
    });
    const migrator = orm.migrator;
    const migration = await migrator.createMigration(
      undefined,
    ...
    expect(migration).toMatchSnapshot("migration-dump");
    expect(migration.fileName).toEqual(
      "migration20191013214813_custom_name.ts",
    );
    const upMock = jest.spyOn(Umzug.prototype, "up");
    upMock.mockImplementation(() => void 0 as any);
    const downMock = jest.spyOn(Umzug.prototype, "down");
  ...
  test("generate initial migration", async () => {
    await orm.em
      .getKnex()
      .schema.dropTableIfExists(orm.config.get("migrations").tableName!)
      .withSchema("custom");
    const getExecutedMigrationsMock = jest.spyOn<any, any>(
      Migrator.prototype,
      "getExecutedMigrations",
    );
    const getPendingMigrationsMock = jest.spyOn<any, any>(
      Migrator.prototype,
    ...
    const migrator = orm.migrator;
    const err =
      "Initial migration cannot be created, as some migrations already exist";
    await expect(
      migrator.createMigration(undefined, false, true),
    ).rejects.toThrow(err);

    getExecutedMigrationsMock.mockResolvedValueOnce([]);
    const logMigrationMock = jest.spyOn<any, any>(
      MigrationStorage.prototype,
      "logMigration",
    );
    ...
    await expect(migrator.createInitialMigration(undefined)).rejects.toThrow(
      err2,
    );
    ...
    expect(logMigrationMock).not.toHaveBeenCalledWith(
      "Migration20191013214813.ts",
    );
    ...
    expect(logMigrationMock).toHaveBeenCalledWith({
  ...
  test("migration is skipped when no diff", async () => {
    const migrator = orm.migrator;
    const getSchemaDiffMock = jest.spyOn<any, any>(
      Migrator.prototype,
      "getSchemaDiff",
    );
  ...
  test("ensureTable and list executed migrations", async () => {
    ...
    await expect(storage.getExecutedMigrations()).resolves.toMatchObject([
      { name: "test" },
    ]);
  ...
  test("runner", async () => {
    ...
    await expect(runner.run(migration1, "down")).rejects.toThrow(
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      'insert into "custom"."foo_bar2" ("name") values ($1) returning "id", "version"',
    );
...
test("ensureTable when the schema does not exist", async () => {
  ...
  expect(mock.mock.calls[3][0]).toMatch(
    `create table "custom2"."mikro_orm_migrations" ("id" serial primary key, "name" varchar(255), "executed_at" timestamptz default current_timestamp)`,
  );
  await orm.close();
});
