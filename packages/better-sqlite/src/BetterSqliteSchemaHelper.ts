...
export class BetterSqliteSchemaHelper extends SchemaHelper {
  ...
  override async getColumns(
    ...
    const tableDefinition = await connection.execute<{ sql: string }>(
      sql,
      ["table", tableName],
      "get",
    );
    const composite =
      columns.reduce((count, col) => count + (col.pk ? 1 : 0), 0) > 1;
    ...
    const columnDefinitions = this.parseTableDefinition(
      tableDefinition.sql,
      columns,
    );
  ...
  override async getIndexes(
    ...
    for (const index of indexes.filter(
      (index) => !this.isImplicitIndex(index.name),
    )) {
  ...
  override mapForeignKeys(fks: any[], tableName: string): Dictionary {
    return fks.reduce((ret, fk: any) => {
      ret[fk.from] = {
        constraintName: this.platform.getIndexName(
          tableName,
          [fk.from],
          "foreign",
        ),
