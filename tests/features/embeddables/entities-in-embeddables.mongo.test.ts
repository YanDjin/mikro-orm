import type { ObjectHydrator } from "@yandjin-mikro-orm/core";
import {
  Embeddable,
  Embedded,
  Entity,
...
import { MikroORM, ObjectId } from "@yandjin-mikro-orm/mongodb";
...
describe("embedded entities in mongo", () => {
  ...
  beforeAll(async () => {
    orm = await MikroORM.init({
      entities: [User],
      clientUrl:
        "mongodb://localhost:27017/mikro-orm-test-entities-in-embeddables",
    });
  });
  ...
  test("diffing", async () => {
    ...
    expect(
      hydrator.getEntityHydrator(metadata.get("User"), "full").toString(),
    ).toMatchSnapshot();
  });
  ...
  async function createUsers() {
    ...
    user2.profile1.identity.source._id = new ObjectId(
      "600000000000000000000013",
    );
    user2.profile1.identity.links[0].source = new Source("ils31");
    user2.profile1.identity.links[0].source._id = new ObjectId(
      "600000000000000000000014",
    );
    user2.profile1.identity.links[1].source = new Source("ils32");
    user2.profile1.identity.links[1].source._id = new ObjectId(
      "600000000000000000000015",
    );
    user2.profile1.identity.links[0].metas[0].source = new Source("ilms311");
    user2.profile1.identity.links[0].metas[0].source._id = new ObjectId(
      "600000000000000000000016",
    );
    user2.profile1.identity.links[0].metas[1].source = new Source("ilms312");
    user2.profile1.identity.links[0].metas[1].source._id = new ObjectId(
      "600000000000000000000017",
    );
    user2.profile1.identity.links[0].metas[2].source = new Source("ilms313");
    user2.profile1.identity.links[0].metas[2].source._id = new ObjectId(
      "600000000000000000000018",
    );
    user2.profile1.identity.links[1].metas[0].source = new Source("ilms321");
    user2.profile1.identity.links[1].metas[0].source._id = new ObjectId(
      "600000000000000000000019",
    );
    user2.profile1.identity.links[1].metas[1].source = new Source("ilms322");
    user2.profile1.identity.links[1].metas[1].source._id = new ObjectId(
      "60000000000000000000001a",
    );
    user2.profile1.identity.links[1].metas[2].source = new Source("ilms323");
    user2.profile1.identity.links[1].metas[2].source._id = new ObjectId(
      "60000000000000000000001b",
    );
    ...
    user2.profile2.identity.source._id = new ObjectId(
      "60000000000000000000001d",
    );
    user2.profile2.identity.links[0].source = new Source("ils41");
    user2.profile2.identity.links[0].source._id = new ObjectId(
      "60000000000000000000001e",
    );
    user2.profile2.identity.links[1].source = new Source("ils42");
    user2.profile2.identity.links[1].source._id = new ObjectId(
      "60000000000000000000001f",
    );
  ...
  test("persist and load", async () => {
    ...
    expect(u1.profile1).toMatchObject({
      ...
      identity: {
        ...
        meta: {
          bar: "b1",
          foo: "f1",
          source: { _id: u1.profile1.identity.meta!.source!._id },
        },
      },
      source: { _id: u1.profile1.source!._id },
    });
    ...
    expect(u1.profile2).toMatchObject({
      ...
      identity: {
        ...
        meta: {
          bar: "b2",
          foo: "f2",
          source: { _id: u1.profile2.identity.meta!.source!._id },
        },
        source: { _id: u1.profile2.identity.source!._id },
      },
      source: { _id: u1.profile2.source!._id },
    });
    ...
    expect(u2.profile1.identity.links[1].metas[0].source).toBeInstanceOf(
      Source,
    );
    expect(u2.profile1).toMatchObject({
            ...
            metas: [
              {
                ...
                foo: "f2",
                source: {
                  _id: u2.profile1.identity.links[0].metas[0].source!._id,
                },
              },
              {
                bar: "b3",
                foo: "f3",
                source: {
                  _id: u2.profile1.identity.links[0].metas[1].source!._id,
                },
              },
              {
                bar: "b4",
                foo: "f4",
                source: {
                  _id: u2.profile1.identity.links[0].metas[2].source!._id,
                },
              },
            ],
          },
            ...
            metas: [
              {
                ...
                foo: "f2",
                source: {
                  _id: u2.profile1.identity.links[1].metas[0].source!._id,
                },
              },
              {
                bar: "b3",
                foo: "f3",
                source: {
                  _id: u2.profile1.identity.links[1].metas[1].source!._id,
                },
              },
              {
                bar: "b4",
                foo: "f4",
                source: {
                  _id: u2.profile1.identity.links[1].metas[2].source!._id,
                },
              },
            ],
          },
      ...
      source: { _id: u2.profile1.source!._id },
    });
    ...
    expect(u2.profile2).toMatchObject({
      ...
      identity: {
        ...
        source: { _id: u2.profile2.identity.source!._id },
      },
      source: { _id: u2.profile2.source!._id },
    });
  ...
  test("populating entities in embeddables", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `db.getCollection('user').find({}, {}).sort([ [ 'name', -1 ] ]).toArray();`,
    );
    expect(mock.mock.calls[1][0]).toMatch(
    ...
    expect(
      wrap(users[1].profile1.identity.links[1].source!).isInitialized(),
    ).toBe(true);
    expect(users[1].profile1.identity.links[1].source!.name).toBe("ils32");
    expect(
      wrap(
        users[1].profile1.identity.links[1].metas[2].source!,
      ).isInitialized(),
    ).toBe(true);
    expect(users[1].profile1.identity.links[1].metas[2].source!.name).toBe(
      "ilms323",
    );
