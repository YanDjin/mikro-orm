import { Configuration, QueryOrder } from "@yandjin-mikro-orm/core";
import type { EntityManager } from "@yandjin-mikro-orm/knex";
import { EntityRepository } from "@yandjin-mikro-orm/knex";
import { Publisher, Author } from "./entities";
import type { MongoEntityManager } from "@yandjin-mikro-orm/mongodb";
import { MongoDriver, MongoEntityRepository } from "@yandjin-mikro-orm/mongodb";
...
describe("EntityRepository", () => {
  test("should forward calls to EntityManager", async () => {
    ...
    expect(methods.getReference.mock.calls[0]).toEqual([
      Publisher,
      "bar",
      undefined,
    ]);
    ...
    expect(methods.find.mock.calls[0]).toEqual([
      Publisher,
      { name: "bar" },
      undefined,
    ]);
    await repo.findAndCount({ name: "bar" });
    expect(methods.findAndCount.mock.calls[0]).toEqual([
      Publisher,
      { name: "bar" },
      undefined,
    ]);
    ...
    expect(methods.findOne.mock.calls[0]).toEqual([
      Publisher,
      "bar",
      undefined,
    ]);
    await repo.findOneOrFail("bar");
    expect(methods.findOneOrFail.mock.calls[0]).toEqual([
      Publisher,
      "bar",
      undefined,
    ]);
    await repo.upsert({ name: "bar", id: "1" });
    expect(methods.upsert.mock.calls[0]).toEqual([
      Publisher,
      { name: "bar", id: "1" },
      undefined,
    ]);
    await repo.upsertMany([{ name: "bar", id: "1" }]);
    expect(methods.upsertMany.mock.calls[0]).toEqual([
      Publisher,
      [{ name: "bar", id: "1" }],
      undefined,
    ]);
    await repo.createQueryBuilder();
    expect(methods.createQueryBuilder.mock.calls[0]).toEqual([
      Publisher,
      undefined,
    ]);
    await repo.qb();
    ...
    expect(methods.insert.mock.calls[0]).toEqual([
      Publisher,
      { name: "bar" },
      undefined,
    ]);
    await repo.nativeUpdate({ name: "bar" }, { name: "baz" });
    expect(methods.nativeUpdate.mock.calls[0]).toEqual([
      Publisher,
      { name: "bar" },
      { name: "baz" },
      undefined,
    ...
    expect(methods.nativeDelete.mock.calls[0]).toEqual([
      Publisher,
      { name: "bar" },
      undefined,
    ]);
    await repoMongo.aggregate([{ name: "bar" }]);
    expect(methods.aggregate.mock.calls[0]).toEqual([
      Publisher,
      [{ name: "bar" }],
    ]);
  });
