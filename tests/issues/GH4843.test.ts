...
describe("populate with citext", () => {
  beforeAll(async () => {
    ...
    await knex
      ...
      .insert([
        ...
        { a_id1: "ASdF", a_id2: "TEst", b_id: "TEST" },
      ])
      .into("c");
  });
  ...
  test("composite FK (select-in)", async () => {
    const a1 = await orm.em
      .fork()
      .find(A, ["test1", "test2"], { populate: ["c"], strategy: "select-in" });
    expect(a1[0].c.$.toArray()).toEqual([
      { id: 1, a: { id1: "test1", id2: "test2" }, b: "test3" },
    ]);

    const a2 = await orm.em
      .fork()
      .find(A, ["asdf", "test"], { populate: ["c"], strategy: "select-in" });
    expect(a2[0].c.$.toArray()).toEqual([
      { id: 2, a: { id1: "asdf", id2: "test" }, b: "test" },
    ]);
  });

  test("simple FK (select-in)", async () => {
    const b1 = await orm.em
      .fork()
      .find(B, "test3", { populate: ["c"], strategy: "select-in" });
    expect(b1[0].c.$.toArray()).toEqual([
      { id: 1, a: { id1: "test1", id2: "test2" }, b: "test3" },
    ]);

    const b2 = await orm.em
      .fork()
      .find(B, "test", { populate: ["c"], strategy: "select-in" });
    expect(b2[0].c.$.toArray()).toEqual([
      { id: 2, a: { id1: "asdf", id2: "test" }, b: "test" },
    ]);
  });
