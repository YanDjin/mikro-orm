...
export class MongoConnection extends Connection {
  ...
  async connect(): Promise<void> {
    ...
    if (driverOptions instanceof MongoClient) {
      this.logger.log(
        "info",
        "Reusing MongoClient provided via `driverOptions`",
      );
      this.client = driverOptions;
    } else {
      this.client = new MongoClient(
        this.config.getClientUrl(),
  ...
  async find<T extends object>(
    ...
    const resultSet = this.getCollection<T>(collection).find(
      where as Filter<T>,
      options,
    );
    ...
    if (Array.isArray(orderBy) && orderBy.length > 0) {
      ...
      orderBy.forEach((o) => {
        Utils.keys(o).forEach((k) => {
          ...
          orderByTuples.push([
            k.toString(),
            Utils.isString(direction)
              ? direction.toUpperCase() === QueryOrder.ASC
                ? 1
                : -1
              : (direction as number),
          ]);
  ...
  private async runQuery<
    T extends object,
    U extends QueryResult<T> | number = QueryResult<T>,
  >(
    method:
      | "insertOne"
      | "insertMany"
    ...
    switch (method) {
      case "insertOne":
        ...
        res = await this.rethrow(
          this.getCollection<T>(collection).insertOne(
      ...
      case "insertMany":
        ...
        res = await this.rethrow(
          this.getCollection<T>(collection).insertMany(
            data as OptionalUnlessRequiredId<T>[],
            options,
          ),
          query,
      ...
      case "updateMany": {
        ...
        res = (await this.rethrow(
          this.getCollection<T>(collection).updateMany(
            where as Filter<T>,
            payload as UpdateFilter<T>,
            options,
          ),
          query,
        )) as UpdateResult;
      ...
      case "bulkUpdateMany": {
        ...
        (data as T[]).forEach((row, idx) => {
          ...
          if (upsert) {
            query += log(
              () =>
                `bulk.find(${this.logObject(cond)}).upsert().update(${this.logObject(doc)});\n`,
            );
          ...
          query += log(
            () =>
              `bulk.find(${this.logObject(cond)}).update(${this.logObject(doc)});\n`,
          );
      ...
      case "countDocuments":
        ...
        res = await this.rethrow(
          this.getCollection<T>(collection)[method](
            where as Filter<T>,
            options,
          ) as Promise<number>,
          query,
  ...
  private createUpdatePayload<T extends object>(
    row: T,
    upsertOptions?: UpsertOptions<T>,
  ): { $set?: unknown[]; $unset?: unknown[]; $setOnInsert?: unknown[] } {
  ...
  private transformResult<T>(res: any): QueryResult<T> {
    return {
      affectedRows:
        res.modifiedCount || res.deletedCount || res.insertedCount || 0,
