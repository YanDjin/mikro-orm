...
import { MariaDbDriver } from "@yandjin-mikro-orm/mariadb";
import {
  Author2,
  Book2,
  BookTag2,
...
describe("EntityManagerMariaDb", () => {
  ...
  beforeAll(
    async () => (orm = await initORMMySql<MariaDbDriver>("mariadb", {}, true)),
  );
  ...
  test("should return mariadb driver", async () => {
    ...
    expect(driver.getPlatform().denormalizePrimaryKey("1")).toBe("1");
    await expect(
      driver.find(BookTag2.name, { books: { $in: [1] } }),
  ...
  test("driver appends errored query", async () => {
    const driver = orm.em.getDriver();
    const err1 =
      /insert into `not_existing` \(`foo`\) values \('bar'\) - \(conn=\d+, no: \d+, SQLState: \w+\) Table 'mikro_orm_test_\w+\.not_existing' doesn't exist/;
    await expect(
      driver.nativeInsert("not_existing", { foo: "bar" }),
    ).rejects.toThrow(err1);
    const err2 =
      /delete from `not_existing` - \(conn=\d+, no: \d+, SQLState: \w+\) Table 'mikro_orm_test_\w+\.not_existing' doesn't exist/;
