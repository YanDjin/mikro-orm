...
import type { Platform } from "@yandjin-mikro-orm/core";
import {
  Collection,
  Entity,
  ManyToOne,
...
class TransformType extends Type<0 | number | Date> {
  ...
  override convertToJSValue(value: 0 | number | Date): 0 | number | Date {
    return (value as unknown) == "0" || value == null || value == 0
      ? 0
      : new Date(value);
  }
...
test("custom date like mapped type in composite PK (GH 3209)", async () => {
  ...
  expect(mock.mock.calls[2][0]).toMatch(
    "insert into `book` (`title`, `author_id`, `author_deleted_date`) values ('b1', 'a1', 0), ('b2', 'a1', 0), ('b3', 'a1', 0) returning `book_id`",
  );
  expect(mock.mock.calls[3][0]).toMatch(`commit`);
  expect(mock.mock.calls[4][0]).toMatch(`begin`);
  expect(mock.mock.calls[5][0]).toMatch(
    /update `author` set `deleted_date` = \d+, `version` = `version` \+ 1 where `id` = 'a1' and `deleted_date` = 0 and `version` = 1 returning `version`/,
  );
  ...
  expect(mock.mock.calls[8][0]).toMatch(
    /select `a0`.* from `author` as `a0` where \(`a0`.`id`, `a0`.`deleted_date`\) in \(\('a1', \d+\)\) limit 1/,
  );
  expect(mock.mock.calls[9][0]).toMatch(`begin`);
  expect(mock.mock.calls[10][0]).toMatch(
    /update `author` set `name` = 'new', `version` = `version` \+ 1 where `id` = 'a1' and `deleted_date` = \d+ and `version` = 2 returning `version`/,
  );
  ...
  expect(mock.mock.calls[14][0]).toMatch(
    /update `author` set `deleted_date` = 0, `version` = `version` \+ 1 where `id` = 'a1' and `deleted_date` = \d+ and `version` = 3 returning `version`/,
  );
