...
import { LoadStrategy } from "@yandjin-mikro-orm/core";
...
describe("partial loading via `exclude` (mysql)", () => {
  ...
  beforeAll(
    async () =>
      (orm = await initORMMySql("mysql", { dbName: "partial_loading2" }, true)),
  );
  ...
  test("exclude option", async () => {
    ...
    const a = (await orm.em.findOne(Author2, author, {
      exclude: ["email", "born", "termsAccepted"],
    }))!;
    ...
    const a4 = orm.em
      .repo(Author2)
      .assign(a2, { born: "1990-03-24", asd: true });
  ...
  test("partial nested loading (1:m)", async () => {
    ...
    const rr = await orm.em.findOneOrFail(Author2, god, {
      ...
      exclude: [
        ...
        "favouriteBook.publisher",
        "books.title",
        "books.publisher.type",
      ],
    ...
    // @ts-expect-error
    rr.favouriteBook?.publisher?.$.name;
    ...
    expect(
      `This is User #${rr.id.toFixed()} with name '${rr.name.substring(0, 3)}'`,
    ).toBe(`This is User #1 with name 'God'`);

    const mock = mockLogger(orm, ["query"]);

    const r1 = await orm.em.find(Author2, god, {
      populate: ["books"],
      exclude: ["name", "books.price"],
      strategy: "select-in",
    });
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `a0`.`id`, `a0`.`created_at`, `a0`.`updated_at`, `a0`.`email`, `a0`.`age`, `a0`.`terms_accepted`, `a0`.`optional`, `a0`.`identities`, `a0`.`born`, `a0`.`born_time`, `a0`.`favourite_book_uuid_pk`, `a0`.`favourite_author_id`, `a0`.`identity`, `a1`.`author_id` as `address_author_id` from `author2` as `a0` left join `address2` as `a1` on `a0`.`id` = `a1`.`author_id` where `a0`.`id` = ?",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      "select `b0`.`uuid_pk`, `b0`.`created_at`, `b0`.`title`, `b0`.`double`, `b0`.`meta`, `b0`.`author_id`, `b0`.`publisher_id`, `b0`.price * 1.19 as `price_taxed`, `t1`.`id` as `test_id` from `book2` as `b0` left join `test2` as `t1` on `b0`.`uuid_pk` = `t1`.`book_uuid_pk` where `b0`.`author_id` is not null and `b0`.`author_id` in (?) order by `b0`.`title` asc",
    );
    orm.em.clear();
    mock.mock.calls.length = 0;

    const r0 = await orm.em.find(Author2, god, {
      populate: ["books"],
      exclude: ["name", "books.price"],
      strategy: "joined",
    });
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `a0`.`id`, `a0`.`created_at`, `a0`.`updated_at`, `a0`.`email`, `a0`.`age`, `a0`.`terms_accepted`, `a0`.`optional`, `a0`.`identities`, `a0`.`born`, `a0`.`born_time`, `a0`.`favourite_book_uuid_pk`, `a0`.`favourite_author_id`, `a0`.`identity`, `b1`.`uuid_pk` as `b1__uuid_pk`, `b1`.`created_at` as `b1__created_at`, `b1`.`title` as `b1__title`, `b1`.price * 1.19 as `b1__price_taxed`, `b1`.`double` as `b1__double`, `b1`.`meta` as `b1__meta`, `b1`.`author_id` as `b1__author_id`, `b1`.`publisher_id` as `b1__publisher_id`, `a2`.`author_id` as `address_author_id` from `author2` as `a0` left join `book2` as `b1` on `a0`.`id` = `b1`.`author_id` and `b1`.`author_id` is not null left join `address2` as `a2` on `a0`.`id` = `a2`.`author_id` where `a0`.`id` = ? order by `b1`.`title` asc",
    );
    // @ts-expect-error
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `a`.`id`, `b`.`uuid_pk` as `b__uuid_pk`, `b`.`title` as `b__title`, `b`.`author_id` as `b__author_id` from `author2` as `a` inner join `book2` as `b` on `a`.`id` = `b`.`author_id` where `a`.`id` = ? order by `b`.`title` asc",
    );
    orm.em.clear();
    mock.mock.calls.length = 0;
  });
  ...
  test("partial nested loading (m:1)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.`uuid_pk`, `b0`.`created_at`, `b0`.`title`, `b0`.`double`, `b0`.`meta`, `b0`.`author_id`, `b0`.`publisher_id`, `b0`.price * 1.19 as `price_taxed`, `t1`.`id` as `test_id` from `book2` as `b0` left join `test2` as `t1` on `b0`.`uuid_pk` = `t1`.`book_uuid_pk` where `b0`.`uuid_pk` = ?",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      "select `a0`.`id`, `a0`.`created_at`, `a0`.`updated_at`, `a0`.`email`, `a0`.`age`, `a0`.`terms_accepted`, `a0`.`optional`, `a0`.`identities`, `a0`.`born`, `a0`.`born_time`, `a0`.`favourite_book_uuid_pk`, `a0`.`favourite_author_id`, `a0`.`identity`, `a1`.`author_id` as `address_author_id` from `author2` as `a0` left join `address2` as `a1` on `a0`.`id` = `a1`.`author_id` where `a0`.`id` in (?)",
    );
    orm.em.clear();
    mock.mock.calls.length = 0;
  });
  ...
  test("partial nested loading (m:n)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.`id`, `b0`.`name`, `b1`.`uuid_pk` as `b1__uuid_pk`, `b1`.`created_at` as `b1__created_at`, `b1`.`title` as `b1__title`, `b1`.price * 1.19 as `b1__price_taxed`, `b1`.`double` as `b1__double`, `b1`.`meta` as `b1__meta`, `b1`.`publisher_id` as `b1__publisher_id` from `book_tag2` as `b0` left join `book2_tags` as `b2` on `b0`.`id` = `b2`.`book_tag2_id` left join `book2` as `b1` on `b2`.`book2_uuid_pk` = `b1`.`uuid_pk` order by `b2`.`order` asc",
    );
    orm.em.clear();
    mock.mock.calls.length = 0;

    const r2 = await orm.em.find(
      BookTag2,
      { name: "t1" },
      {
        exclude: ["books.author", "books.price"],
  ...
  test("partial nested loading (m:n -> m:1)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.`id`, `b0`.`name` from `book_tag2` as `b0`",
    );
    expect(mock.mock.calls[1][0]).toMatch(
    ...
    expect(mock.mock.calls).toHaveLength(1);
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.`id`, `b0`.`name`, `b1`.`uuid_pk` as `b1__uuid_pk`, `b1`.`created_at` as `b1__created_at`, `b1`.`title` as `b1__title`, `b1`.price * 1.19 as `b1__price_taxed`, `b1`.`double` as `b1__double`, `b1`.`meta` as `b1__meta`, `b1`.`author_id` as `b1__author_id`, `b1`.`publisher_id` as `b1__publisher_id`, `a3`.`id` as `a3__id`, `a3`.`created_at` as `a3__created_at`, `a3`.`updated_at` as `a3__updated_at`, `a3`.`email` as `a3__email`, `a3`.`age` as `a3__age`, `a3`.`terms_accepted` as `a3__terms_accepted`, `a3`.`optional` as `a3__optional`, `a3`.`identities` as `a3__identities`, `a3`.`born` as `a3__born`, `a3`.`born_time` as `a3__born_time`, `a3`.`favourite_book_uuid_pk` as `a3__favourite_book_uuid_pk`, `a3`.`favourite_author_id` as `a3__favourite_author_id`, `a3`.`identity` as `a3__identity` " +
  ...
  test("populate partial (joined)", async () => {
    ...
    expect(r2.books[0].price).toBe(123.0);
    ...
    await expect(
      orm.em.findAll(BookTag2, { fields: ["books.title"], exclude: ["name"] }),
    ).rejects.toThrow(`Cannot combine 'fields' and 'exclude' option.`);
  });
});
