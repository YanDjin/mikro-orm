...
import { Migration, Migrator } from "@yandjin-mikro-orm/migrations-mongodb";
...
class MigrationTest1 extends Migration {
  async up(): Promise<void> {
    await this.getCollection<any>("Book").updateMany(
      {},
...
class MigrationTest2 extends Migration {
  async up(): Promise<void> {
    await this.getCollection("Book").updateMany(
      {},
      { $unset: { title: 1 } },
      { session: this.ctx },
    );
...
describe("Migrator (mongo)", () => {
  ...
  test("generate js schema migration", async () => {
    ...
    await remove(
      process.cwd() + "/temp/migrations-mongo/" + migration.fileName,
    );
  });
  ...
  test("runner", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `db.getCollection('books-table').deleteMany({ foo: false }, {})`,
    );
  });
