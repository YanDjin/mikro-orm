import { RequestContext, MikroORM, wrap } from "@yandjin-mikro-orm/core";
...
describe("RequestContext", () => {
  ...
  test("create new context", async () => {
    ...
    RequestContext.create(orm.em, () => {
      ...
      expect(em.unitOfWork.getIdentityMap()).not.toBe(
        orm.em.unitOfWork.getIdentityMap(),
      );
...
describe("MultiRequestContext", () => {
  ...
  test("create new context", async () => {
    ...
    RequestContext.create([orm1.em, orm2.em], () => {
      ...
      // @ts-ignore
      expect(em1.unitOfWork.getIdentityMap()).not.toBe(
        orm1.em.unitOfWork.getIdentityMap(),
      );
      ...
      // access UoW via property so we do not get the one from request context automatically
      // @ts-ignore
      expect(em2.unitOfWork.getIdentityMap()).not.toBe(
        orm2.em.unitOfWork.getIdentityMap(),
      );
