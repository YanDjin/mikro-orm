...
test("raw fragments with orderBy", async () => {
  ...
  expect(mock.mock.calls[0][0]).toMatch(
    "select `j0`.* from `job` as `j0` order by j0.DateCompleted desc",
  );
  expect(RawQueryFragment.checkCacheSize()).toBe(0);
});
...
test("raw fragments with multiple items in filter", async () => {
  ...
  expect(mock.mock.calls[0][0]).toMatch(
    "select `t0`.* from `tag` as `t0` where id >= 10 and id <= 50",
  );
  expect(RawQueryFragment.checkCacheSize()).toBe(0);
});
...
test("qb.joinAndSelect", async () => {
  const query = orm.em
    ...
    .limit(100)
    .offset(0);
  expect(query.getKnexQuery().toSQL().sql).toMatch(
