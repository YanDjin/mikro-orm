...
test("it should add discriminator to the query", async () => {
  ...
  expect(qb2.getFormattedQuery()).toMatch(
    "select `p`.* from `base_entity` as `p` where `p`.`type` = 'post'",
  );
  ...
  expect(qb3.getFormattedQuery()).toMatch(
    "select `u`.* from `user` as `u` inner join `user_posts2` as `u1` on `u`.`id` = `u1`.`user_id` inner join `base_entity` as `p` on `u1`.`post_id` = `p`.`id` and `p`.`type` = 'post'",
  );
