...
export class ObjectCriteriaNode<T extends object> extends CriteriaNode<T> {
  override process(
    ...
    if (this.shouldAutoJoin(qb, nestedAlias)) {
      if (keys.some((k) => ["$some", "$none", "$every"].includes(k))) {
        ...
        const primaryKeys = this.metadata
          ...
          .primaryKeys.map((pk) => {
            return [QueryType.SELECT, QueryType.COUNT].includes(qb.type!)
              ? `${alias}.${pk}`
              : pk;
          });
        ...
        for (const key of keys) {
          ...
          $and.push({
            [Utils.getPrimaryKeyHash(primaryKeys)]: {
              [op]: (sub as Dictionary).getKnexQuery(),
            },
          });
        }
    ...
    return keys.reduce((o, field) => {
      ...
      const virtual =
        childNode.prop?.persist === false && !childNode.prop?.formula;
      ...
      if (childNode.shouldInline(payload)) {
        ...
        o[
          raw(
            rawField.sql.replaceAll(ALIAS_REPLACEMENT, alias!),
            rawField.params,
          )
        ] = payload;
      } else if (
        ...
        operator ||
        field.includes(".") ||
        ![QueryType.SELECT, QueryType.COUNT].includes(
  ...
  override shouldInline(payload: any): boolean {
    ...
    const operator =
      ...
      Object.keys(payload).every((k) => Utils.isOperator(k, false));

    return (
      !!this.prop &&
      this.prop.kind !== ReferenceKind.SCALAR &&
      !scalar &&
      !operator
    );
  }
        ...
        if (key in o) {
          ...
          o[key] = payload[k].map((child: Dictionary) =>
            Object.keys(child).reduce((o, childKey) => {
              const key =
                this.isPrefixed(childKey) || Utils.isOperator(childKey)
                  ? childKey
  ...
  private shouldAutoJoin(
    ...
    const operatorKeys =
      knownKey && keys.every((key) => Utils.isOperator(key, false));
    const primaryKeys =
      knownKey &&
      keys.every((key) => {
  ...
  private autoJoin<T>(qb: IQueryBuilder<T>, alias: string): string {
    const nestedAlias = qb.getNextAlias(
      this.prop?.pivotTable ?? this.entityName,
    );
    const customExpression = RawQueryFragment.isKnownFragment(this.key!);
    const scalar =
    ...
    const method = qb.hasFlag(QueryFlag.INFER_POPULATE)
      ? "joinAndSelect"
      : "join";

    if (
      this.prop!.kind === ReferenceKind.MANY_TO_MANY &&
      (scalar || operator)
    ) {
      qb.join(
        field,
