...
describe("QueryBuilder", () => {
  ...
  test("select query", async () => {
    ...
    qb.select("*")
      ...
      .orderBy({ name: QueryOrder.DESC, type: QueryOrder.ASC })
      .limit(2, 1);
    ...
    qb1
      ...
      .orderBy({
        [raw(
          `(point(location_latitude, location_longitude) <@> point(?, ?))`,
          [53, 9],
        )]: "ASC",
      });
    ...
    expect(() => qb2.where("foo = 123")).toThrow(
      "This QueryBuilder instance is already finalized, clone it first if you want to modify it.",
    );
  });
  ...
  test("select where is null", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`type` is null limit ? offset ?",
    );
  ...
  test("select query with order by variants", async () => {
    ...
    qb1
      .select("*")
      .where({ name: "test 123" })
      .orderBy({ name: QueryOrder.DESC, type: "ASC" })
      .limit(2, 1);
    expect(qb1.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` = ? order by `e0`.`name` desc, `e0`.`type` asc limit ? offset ?",
    );
  ...
  test("select constant expression", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select 1 from `publisher2` as `e0` where `e0`.`id` = ?",
    );
  ...
  test("select in query with composite keys", async () => {
    ...
    qb.select("*")
      ...
      .orderBy({ "name~~~year": QueryOrder.DESC });
    expect(qb.getQuery()).toEqual(
  ...
  test("select query with auto-joined composite key entity", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.* from `car_owner2` as `e0` where (`e0`.`car_name`, `e0`.`car_year`) = (?, ?)",
    );
    ...
    expect(qb3.getQuery()).toEqual(
      "select `e0`.* from `car_owner2` as `e0` where (`e0`.`car_name`, `e0`.`car_year`) in ((?, ?))",
    );
  ...
  test("select query with auto-joined query with operator and scalar", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where `e0`.`author_id` is not null",
    );
    ...
    expect(qb2.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` where `e0`.`author_id` is not null and `e1`.`name` = ?",
    );
  ...
  test("select andWhere/orWhere", async () => {
    ...
    qb.select("*")
      ...
      .limit(2, 1);
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where ((`e0`.`name` = ? and `e0`.`type` = ?) or `e0`.`name` = ?) limit ? offset ?",
    );
    expect(qb.getParams()).toEqual([
      "test 123",
      PublisherType.GLOBAL,
  ...
  test("select andWhere/orWhere as first where condition", async () => {
    const qb1 = orm.em
      .createQueryBuilder(Publisher2)
      .select("*")
      .andWhere({ type: PublisherType.GLOBAL })
      .orWhere({ name: "lol 321" })
      .limit(2, 1);
    expect(qb1.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where (`e0`.`type` = ? or `e0`.`name` = ?) limit ? offset ?",
    );
    ...
    const qb2 = orm.em
      ...
      .limit(2, 1);
    expect(qb2.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` = ? and `e0`.`type` = ? limit ? offset ?",
    );
  ...
  test("select complex where", async () => {
    ...
    qb.where({
      name: "test 123",
      $or: [{ name: "test 321" }, { type: PublisherType.GLOBAL }],
    }).limit(2, 1);
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` = ? and (`e0`.`name` = ? or `e0`.`type` = ?) limit ? offset ?",
    );
    expect(qb.getParams()).toEqual([
      "test 123",
      "test 321",
  ...
  test("select with 1:1 owner auto-join", async () => {
    ...
    const sql =
      "select `fz`.*, `e1`.`id` as `bar_id` from `foo_baz2` as `fz` left join `foo_bar2` as `e1` on `fz`.`id` = `e1`.`baz_id` limit ? offset ?";
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual([2, 1]);
  });
  ...
  test("validation of unknown alias", async () => {
    ...
    expect(() => qb.join("fb0.baz", "b")).toThrow(
      `Trying to join 'baz' with alias 'fb0', but 'fb0' is not a known alias. Available aliases are: 'fb1', 'fz'.`,
    );
  });
  ...
  test("select with boolean", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `author2` as `e0` where `e0`.`terms_accepted` = ?",
    );
  ...
  test("select with boolean in relation (GH issue #940)", async () => {
    ...
    expect(qb.getFormattedQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` where `e1`.`terms_accepted` = true",
    );
  });
  ...
  test("select with custom expression", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where json_contains(`e0`.`meta`, ?)",
    );
  ...
  test("select with prototype-less object", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where json_extract(`e0`.`meta`, '$.foo') = ?",
    );
  ...
  test("select by regexp", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` like ?",
    );
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` regexp ?",
    );
  ...
  test("$exists operator", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` is not null",
    );
    expect(qb.getParams()).toEqual([]);
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` where `e0`.`name` is null",
    );
    expect(qb.getParams()).toEqual([]);
  ...
  test("select by m:1", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `author2` as `e0` where `e0`.`favourite_book_uuid_pk` = ?",
    );
    expect(qb.getParams()).toEqual(["123"]);
  });
  ...
  test("GH #1668", async () => {
    ...
    qb1
      .select(raw("floor(`a`.`age`) as `books_total`"))
      .groupBy("booksTotal")
      .orderBy({ booksTotal: QueryOrder.ASC });

    expect(qb1.getQuery()).toEqual(
      "select floor(`a`.`age`) as `books_total` from `author2` as `a` group by `books_total` order by `books_total` asc",
    );
    expect(qb1.getParams()).toEqual([]);

    const qb2 = orm.em.createQueryBuilder(Author2, "a");
    qb2
      .select(raw("floor(`a`.`age`) as `code`"))
      .groupBy("code")
      .orderBy({ code: QueryOrder.ASC });

    expect(qb2.getQuery()).toEqual(
      "select floor(`a`.`age`) as `code` from `author2` as `a` group by `code` order by `code` asc",
    );
  ...
  test("select by 1:1", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.*, (select 123) as `random` from `foo_bar2` as `e0` where `e0`.`baz_id` = ?",
    );
    expect(qb.getParams()).toEqual([123]);
  });
  ...
  test("select by 1:1 inversed", async () => {
    ...
    qb.select("*")
      ...
      .populate([{ field: "bar" }]);
    expect(qb.getQuery()).toEqual(
      "select `e0`.*, `e1`.`id` as `bar_id` from `foo_baz2` as `e0` left join `foo_bar2` as `e1` on `e0`.`id` = `e1`.`baz_id` where `e0`.`id` = ?",
    );
    expect(qb.getParams()).toEqual([123]);
  });
  ...
  test("select by 1:1 inversed (search by association)", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.*, `e1`.`id` as `bar_id` from `foo_baz2` as `e0` left join `foo_bar2` as `e1` on `e0`.`id` = `e1`.`baz_id` where `e1`.`id` = ?",
    );
    expect(qb.getParams()).toEqual([123]);
  });
  ...
  test("select by 1:1 inversed (uuid pk)", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.*, `e1`.`id` as `test_id`, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `test2` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` where `e1`.`id` = ?",
    );
    expect(qb.getParams()).toEqual([123]);
  });
  ...
  test("select by m:n inversed", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `book_tag2` as `e0` " +
  ...
  test("select with operator (simple)", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `test2` as `e0` where `e0`.`id` not in (?, ?)",
    );
  ...
  test("select with operator (wrapped)", async () => {
    ...
    qb1
      .select("*")
      .where({ $and: [{ id: { $nin: [3, 4] } }, { id: { $gt: 2 } }] });
    expect(qb1.getQuery()).toEqual(
      "select `e0`.* from `test2` as `e0` where `e0`.`id` not in (?, ?) and `e0`.`id` > ?",
    );
  ...
  test("select with $and", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.* from `test2` as `e0` where `e0`.`id` = ? and `e0`.`id` = ?",
    );
  ...
  test("select with operator (not)", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `test2` as `e0` where not (`e0`.`id` in (?, ?))",
    );
    expect(qb.getParams()).toEqual([3, 4]);
  });
  ...
  test("select with unsupported operator", async () => {
    const qb = orm.em.createQueryBuilder(Test2);
    expect(() => qb.select("*").where({ $test: { foo: "bar" } })).toThrow(
      "Trying to query by not existing property Test2.$test",
    );
  });
  ...
  test("select where (not) null via $eq/$ne operators", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where `e0`.`publisher_id` is not null",
    );
    expect(qb1.getParams()).toEqual([]);
    ...
    expect(qb2.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where `e0`.`publisher_id` is null",
    );
    expect(qb2.getParams()).toEqual([]);
  });
  ...
  test("select count query", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select count(*) as `count` from `publisher2` as `e0` where `e0`.`name` = ? and `e0`.`type` = ?",
    );
  ...
  test("select count distinct query", async () => {
    ...
    qb.count("id", true)
      .where({ name: "test 123", type: PublisherType.GLOBAL })
      .setFlag(QueryFlag.DISTINCT);
    expect(qb.getQuery()).toEqual(
      "select count(distinct `e0`.`id`) as `count` from `publisher2` as `e0` where `e0`.`name` = ? and `e0`.`type` = ?",
    );
    expect(qb.getParams()).toEqual(["test 123", PublisherType.GLOBAL]);
  });
  ...
  test("select count with non-standard PK field name (uuid_pk)", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select count(*) as `count` from `book2` as `e0` where `e0`.`title` = ?",
    );
    expect(qb.getParams()).toEqual(["test 123"]);
  });
  ...
  test("select with locking", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.* from `test2` as `e0` where `e0`.`name` = ?",
    );
    ...
    await orm.em.transactional(async (em) => {
      ...
      expect(qb2.getQuery()).toEqual(
        "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where `e0`.`title` = ?",
      );
      ...
      expect(qb3.getQuery()).toEqual(
        "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where `e0`.`title` = ? lock in share mode",
      );
      ...
      expect(qb4.getQuery()).toEqual(
        "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` where `e0`.`title` = ? for update",
      );
    });
  });
  ...
  test("select with deep where condition", async () => {
    ...
    qb1
      .select("*")
      .where({ author: { name: "Jon Snow", termsAccepted: true } });
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` where `e1`.`name` = ? and `e1`.`terms_accepted` = ?",
    );
    ...
    expect(qb2.getQuery()).toEqual(
      "select `e0`.* from `author2` as `e0` left join `book2` as `e1` on `e0`.`id` = `e1`.`author_id` where `e1`.`title` = ?",
    );
  ...
  test("select formula property explicitly", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select (select 123) as `random` from `foo_bar2` as `e0` where `e0`.`id` = ?",
    );
    ...
    expect(qb2.getQuery()).toEqual(
      "select `e0`.*, (select 123) as `random` from `foo_bar2` as `e0` where `e0`.`id` = ?",
    );
    ...
    expect(qb3.getQuery()).toEqual(
      "select `e0`.*, (select 123) as `random` from `foo_bar2` as `e0` where (select 123) > ?",
    );
    ...
    expect(qb4.getQuery()).toEqual(
      "select `e0`.*, (select 123) as `random` from `foo_bar2` as `e0` having (select 123) > ?",
    );
  ...
  test("select with deep where condition with self-reference", async () => {
    ...
    qb1
      .select("*")
      .where({
        author: { favouriteAuthor: { name: "Jon Snow", termsAccepted: true } },
      });
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` " +
        "left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` " +
  ...
  test("select with deep order by", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` order by `e1`.`name` desc",
    );

    const qb2 = orm.em.createQueryBuilder(Author2);
    qb2.select("*").orderBy({ books: { title: QueryOrder.ASC } });
    expect(qb2.getQuery()).toEqual(
      "select `e0`.* from `author2` as `e0` left join `book2` as `e1` on `e0`.`id` = `e1`.`author_id` order by `e1`.`title` asc",
    );
  ...
  test("select with deep where and deep order by", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` where `e1`.`name` = ? order by `e1`.`name` desc",
    );
  ...
  test("select with deep where condition with operators", async () => {
    ...
    qb1
      ...
      .where({
        $or: [
          { author: { name: "Jon Snow 1", termsAccepted: true } },
    ...
    qb4
      ...
      .where({
        $or: [
          {
            author: {
              $or: [{ $not: { name: "Jon Snow 1" } }, { email: /^snow@/ }],
            },
          },
          { publisher: { name: "My Publisher" } },
        ],
    ...
    qb5
      ...
      .where({
        books: {
          $or: [{ title: "Book 1" }, { publisher: { name: "Publisher 1" } }],
        },
      });
    expect(qb5.getQuery()).toEqual(
    ...
    qb6
      ...
      .where({
        books: {
          publisher: {
            $or: [{ name: "My Publisher 1" }, { name: "My Publisher 2" }],
          },
        },
      });
    expect(qb6.getQuery()).toEqual(
    ...
    qb8
      .select("*")
      .where({ books: { test: { name: { $in: ["Test 1", "Test 2"] } } } });
    expect(qb8.getQuery()).toEqual(
      "select `e0`.* " +
        "from `book_tag2` as `e0` " +
        "left join `book2_tags` as `e2` on `e0`.`id` = `e2`.`book_tag2_id` " +
  ...
  test("select with deep where with invalid property throws error", async () => {
    const qb0 = orm.em.createQueryBuilder(Book2);
    const err =
      "Trying to query by not existing property Author2.undefinedName";
    expect(() =>
      qb0
        .select("*")
  ...
  test("pessimistic locking requires active transaction", async () => {
    ...
    expect(() => qb.setLockMode(LockMode.NONE)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.PESSIMISTIC_READ)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.PESSIMISTIC_WRITE)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.PESSIMISTIC_WRITE_OR_FAIL)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.PESSIMISTIC_PARTIAL_WRITE)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.PESSIMISTIC_READ_OR_FAIL)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.PESSIMISTIC_PARTIAL_READ)).toThrow(
      "An open transaction is required for this operation",
    );
    expect(() => qb.setLockMode(LockMode.OPTIMISTIC).getQuery()).toThrow(
      "The optimistic lock on entity Author2 failed",
    );
  });
  ...
  test("insert query", async () => {
    ...
    qb0.insert([{}, {}] as any);
    expect(qb0.getQuery()).toEqual(
      "insert into `publisher2` (`id`) values (default), (default)",
    );
    ...
    expect(qb1.getQuery()).toEqual(
      "insert into `publisher2` (`name`, `type`) values (?, ?)",
    );
    ...
    expect(qb2.getQuery()).toEqual(
      "insert into `author2` (`email`, `favourite_book_uuid_pk`, `name`, `terms_accepted`) values (?, ?, ?, ?)",
    );
    ...
    expect(qb3.getQuery()).toEqual(
      "insert into `test123`.`book_tag2` (`books`) values (?)",
    );
  ...
  test("insert on conflict ignore/merge (GH #1774)", async () => {
    ...
    qb0
      ...
      .ignore();
    expect(qb0.getQuery()).toEqual(
      "insert ignore into `author2` (`email`, `name`) values (?, ?)",
    );
    ...
    expect(() => qb5.ignore()).toThrow(
      "You need to call `qb.onConflict()` first to use `qb.ignore()`",
    );
    expect(() => qb5.merge()).toThrow(
      "You need to call `qb.onConflict()` first to use `qb.merge()`",
    );
  });
  ...
  test("update query with column reference", async () => {
    ...
    expect(qb.getFormattedQuery()).toEqual(
      "update `book2` set `price` = price + 1 where `uuid_pk` = '123'",
    );
  });
  ...
  test("update query with composite keys and auto-joining", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "update `foo_param2` set `value` = ?, `version` = current_timestamp(3) " +
  ...
  test("trying to call qb.update/delete() after qb.where() will throw", async () => {
    const err1 =
      "You are trying to call `qb.where().update()`. Calling `qb.update()` before `qb.where()` is required.";
    expect(() =>
      orm.em
        .qb(Publisher2)
    ...
    const err2 =
      "You are trying to call `qb.where().delete()`. Calling `qb.delete()` before `qb.where()` is required.";
  ...
  test("update query with entity in data", async () => {
    ...
    test.id = 321;
    qb.update({ name: "test 123", test }).where({
      id: 123,
      type: PublisherType.LOCAL,
    });
    expect(qb.getQuery()).toEqual(
  ...
  test("delete query", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "delete from `publisher2` where `name` = ? and `type` = ?",
    );
    expect(qb.getParams()).toEqual(["test 123", PublisherType.GLOBAL]);
  });
  ...
  test("clone QB", async () => {
    ...
    clone
      .orWhere({ "p.name": "or this name" })
      .orderBy({ "p.name": QueryOrder.ASC });
  ...
  test("automatic pagination when to-many join detected together with offset/limit", async () => {
    ...
    const sql =
      "select `p`.*, `b`.`uuid_pk` as `b__uuid_pk`, `b`.`created_at` as `b__created_at`, `b`.`title` as `b__title`, `b`.`price` as `b__price`, `b`.price * 1.19 as `b__price_taxed`, `b`.`double` as `b__double`, `b`.`meta` as `b__meta`, `b`.`author_id` as `b__author_id`, `b`.`publisher_id` as `b__publisher_id`, " +
  ...
  test("$or and $and combined", async () => {
    ...
    expect(qb5.getQuery()).toEqual(
      "select `a`.* from `author2` as `a` where (`a`.`name` = ? or `a`.`email` = ?) and (`a`.`name` = ? or `a`.`email` = ?)",
    );
    ...
    expect(qb10.getQuery()).toEqual(
      "select `a`.* from `author2` as `a` where (`a`.`email` = ? or (`a`.`name` in (?) and `a`.`name` != ?))",
    );
  ...
  test("select fk by operator should not trigger auto-joining", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `a`.* from `author2` as `a` where `a`.`favourite_book_uuid_pk` in (?, ?, ?)",
    );
  });
  ...
  test("order by virtual property", async () => {
    ...
    qb1
      ...
      .orderBy({ code: "asc" });
    expect(qb1.getQuery()).toEqual(
      'select `a`.*, "1" as code from `author2` as `a` where `a`.`id` in (?, ?) order by `code` asc',
    );
  });
  ...
  test("select where sub-query", async () => {
    ...
    qb4
      .select("*")
      .withSubQuery(qb3, "a.booksTotal")
      .where({ "a.booksTotal": 1 });
    expect(qb4.getQuery()).toEqual(
    ...
    const qb6 = orm.em
      ...
      .select("*")
      .where(`id in (${qb5.getFormattedQuery()})`);
    expect(qb6.getQuery()).toEqual(
  ...
  test("join sub-query", async () => {
    ...
    expect(qb5.getFormattedQuery()).toEqual(
      "select `a`.*, `sub`.*, `sub`.`uuid_pk` as `sub__uuid_pk`, `sub`.`created_at` as `sub__created_at`, `sub`.`title` as `sub__title`, `sub`.`price` as `sub__price`, `sub`.price * 1.19 as `sub__price_taxed`, `sub`.`double` as `sub__double`, `sub`.`meta` as `sub__meta`, `sub`.`author_id` as `sub__author_id`, `sub`.`publisher_id` as `sub__publisher_id` from `author2` as `a` left join `book2` as `sub` on `a`.`id` = `sub`.`author_id` and `sub`.`author_id` = `a`.`id` where `sub`.`title` like 'foo%'",
    );
  ...
  test("CriteriaNode", async () => {
    ...
    expect(inspect(node)).toBe(
      `CriteriaNode { entityName: 'Author2', payload: { foo: 123 } }`,
    );
  });
  ...
  test("order by asc nulls first", async () => {
    ...
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` order by `e0`.`name` is not null, `e0`.`name` asc",
    );
  });
  ...
  test("order by nulls last", async () => {
    ...
    qb.select("*").orderBy({
      ...
      type: QueryOrder.ASC_NULLS_LAST,
    });
    expect(qb.getQuery()).toEqual(
  ...
  test("order by custom expression", async () => {
    const qb = orm.em.createQueryBuilder(Publisher2);
    qb.select("*").orderBy({
      [sql`length(name)`]: QueryOrder.DESC,
      type: QueryOrder.ASC,
    });
    expect(qb.getQuery()).toEqual(
      "select `e0`.* from `publisher2` as `e0` order by length(name) desc, `e0`.`type` asc",
    );
  });
  ...
  test("postgres", async () => {
...

    {
      ...
      expect(qb.getQuery()).toEqual(
        'insert into "author2" ("created_at", "email", "name", "updated_at") values ($1, $2, $3, $4) on conflict do nothing returning "id", "created_at", "updated_at", "age", "terms_accepted"',
      );
      expect(qb.getParams()).toEqual([
        timestamp,
        "ignore@example.com",
...

    {
      ...
      expect(qb.getQuery()).toEqual(
        'insert into "author2" ("created_at", "email", "name", "updated_at") values ($1, $2, $3, $4) on conflict do nothing returning *',
      );
      expect(qb.getParams()).toEqual([
        timestamp,
        "ignore@example.com",
...

    {
      ...
      expect(qb.getQuery()).toEqual(
        'insert into "author2" ("created_at", "email", "name", "updated_at") values ($1, $2, $3, $4) on conflict do nothing returning "id", "email"',
      );
      expect(qb.getParams()).toEqual([
        timestamp,
        "ignore@example.com",
    ...
    expect(qb1.getQuery()).toEqual(
      'select "p0".* from "publisher2" as "p0" where "p0"."name" @> $1',
    );
    expect(qb1.getFormattedQuery()).toEqual(
      `select "p0".* from "publisher2" as "p0" where "p0"."name" @> 'test'`,
    );
    ...
    expect(qb4.getQuery()).toEqual(
      'select "p0".* from "publisher2" as "p0" where "p0"."name" ilike $1',
    );
    ...
    const qb8 = pg.em
      ...
      .where({ id: { $in: qb7.getKnexQuery() } });
    expect(qb8.getQuery()).toEqual(
      'select "a".* from "author2" as "a" where "a"."id" in (select "b"."author_id" from "book2" as "b" where "b"."price" > $1)',
    );
    expect(qb8.getParams()).toEqual([100]);
    ...
    qb9
      ...
      .ignore();
    expect(qb9.getQuery()).toEqual(
      'insert into "author2" ("email", "name") values ($1, $2) on conflict ("email") do nothing returning "id", "created_at", "updated_at", "age", "terms_accepted"',
    );
    ...
    expect(qb10.getQuery()).toEqual(
      'insert into "author2" ("created_at", "email", "name", "updated_at") values ($1, $2, $3, $4) on conflict ("email") do update set "name" = $5,"updated_at" = $6 where "updated_at" < $7 returning "id", "created_at", "updated_at", "age", "terms_accepted"',
    );
    expect(qb10.getParams()).toEqual([
      timestamp,
      "ignore@example.com",
      "John Doe",
      timestamp,
    ...
    const qb12 = pg.em
      .createQueryBuilder(Book2)
      .where({ meta: { foo: { $eq: 123 } } });
    expect(qb12.getFormattedQuery()).toBe(
      `select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where ("b0"."meta"->>'foo')::float8 = 123`,
    );
    const qb13 = pg.em
      .createQueryBuilder(Book2)
      .where({ meta: { foo: { $lte: 123 } } });
    ...
    await pg.em.transactional(async (em) => {
      ...
      qb1
        .select("*")
        .where({ title: "test 123" })
        .setLockMode(LockMode.PESSIMISTIC_PARTIAL_READ);
      expect(qb1.getQuery()).toEqual(
        'select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for share skip locked',
      );
      ...
      expect(qb2.getQuery()).toEqual(
        'select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for update skip locked',
      );
      ...
      qb3
        .select("*")
        .where({ title: "test 123" })
        .setLockMode(LockMode.PESSIMISTIC_READ_OR_FAIL);
      expect(qb3.getQuery()).toEqual(
        'select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for share nowait',
      );
      ...
      qb4
        .select("*")
        .where({ title: "test 123" })
        .setLockMode(LockMode.PESSIMISTIC_WRITE_OR_FAIL);
      expect(qb4.getQuery()).toEqual(
        'select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for update nowait',
      );
      ...
      expect(qb5.getQuery()).toEqual(
        'select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" left join "author2" as "a" on "b0"."author_id" = "a"."id" where "b0"."title" = $1 for update of "book2"',
      );
    });
    ...
    // query comments
    {
      ...
      expect(sql3).toBe(
        `update "my_schema"."author2" set "name" = '...' where "favourite_book_uuid_pk" in ('1', '2', '3')`,
      );
    }
    ...
    // lateral join
    {
      ...
      qb3
        .select(["*", "sub.*"])
        .innerJoinLateral(qb1.getKnexQuery(), "sub", {
          author_id: sql.ref("a.id"),
        })
        .where({ "sub.title": /^foo/ });
      expect(qb2.getFormattedQuery()).toEqual(
        'select "a".*, "sub".* from "author2" as "a" left join lateral (select "b".*, "b".price * 1.19 as "price_taxed" from "book2" as "b" order by "b"."title" asc limit 1) as "sub" on "sub"."author_id" = "a"."id" where "sub"."title" like \'foo%\'',
      ...
      expect(() =>
        qb5.leftJoinLateralAndSelect("a.books", "sub", {
          author: sql.ref("a.id"),
        }),
      ).toThrow("Lateral join can be used only with a sub-query.");
      pg.em.clear();
    }
  ...
  test("joining 1:1 inverse inside $and condition (GH issue 849)", async () => {
    ...
    const sql7 = orm.em
      ...
      .getQuery();
    expect(sql7).toBe(
      "select `e0`.* from `test2` as `e0` where `e0`.`book_uuid_pk` in (?)",
    );
  });
  ...
  test("count query with auto-joining (GH issue 858)", async () => {
    ...
    const sql4 = orm.em
      ...
      .getQuery();
    expect(sql4).toBe(
      "select `e0`.*, `e1`.`id` as `test_id`, `e0`.price * 1.19 as `price_taxed` " +
        "from `book2` as `e0` " +
        "left join `test2` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` " +
        "where `e1`.`id` = ?",
    );
  ...
  test("query comments", async () => {
    ...
    const sql3 = orm.em
      ...
      .getFormattedQuery();
    expect(sql3).toBe(
      "/* test 123 */ /* test 456 */ update `my_schema`.`author2` set `name` = '...' where `favourite_book_uuid_pk` in ('1', '2', '3')",
    );
  });
  ...
  test("$or operator inside auto-joined relation", async () => {
    ...
    const expected =
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` where (`e1`.`id` = 123 or `e1`.`name` like '%jon%')";
    const sql1 = orm.em
      .createQueryBuilder(Book2)
      .select("*")
      .where(query)
  ...
  test(`order by forumla field should not include 'as'`, async () => {
    const sql = orm.em
      .createQueryBuilder(Book2)
      .select("*")
      .orderBy({ priceTaxed: QueryOrder.DESC })
      .getFormattedQuery();
    expect(sql).toBe(
      "select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` order by `e0`.price * 1.19 desc",
    );
  });
  ...
  test(`sub-query order-by fields should not include 'as'`, async () => {
    const sql = orm.em
      ...
      .joinAndSelect("books", "books")
      .orderBy([
        { books: { priceTaxed: QueryOrder.DESC } },
        { id: QueryOrder.DESC },
      ])
      .limit(10)
      .getFormattedQuery();
  ...
  test("select via fulltext search", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `a`.* from `author2` as `a` where match(`a`.`name`) against (? in boolean mode)",
    );
  });
  ...
  test("select via multiple where clauses with fulltext search", async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      "select `a`.* from `author2` as `a` where `a`.`terms_accepted` = ? and match(`a`.`name`) against (? in boolean mode) and match(`a`.`email`) against (? in boolean mode)",
    );
  });
  ...
  test("from an query builder on creation", async () => {
    ...
    expect(qb2.getQuery()).toEqual(
      "select `p`.* from (select `e0`.* from `publisher2` as `e0`) as `p`",
    );
  });
  ...
  test("from a query builder", async () => {
    ...
    qb2.from(qb1);
    expect(qb2.getQuery()).toEqual(
      "select `e1`.* from (select `e0`.* from `author2` as `e0`) as `e1`",
    );
  });
});
