...
describe("EntityManagerMySql", () => {
  ...
  test("should return mysql driver", async () => {
    ...
    await expect(
      driver.findOne<Book2>(Book2.name, { title: "bar" }),
    ).resolves.toBeNull();
    ...
    await expect(
      driver
        .getConnection()
        .execute("insert into test2 (name) values (?)", ["test"], "run"),
    ).resolves.toEqual({
      affectedRows: 1,
      insertId: 1,
      rows: [],
    ...
    expect(driver.getPlatform().denormalizePrimaryKey("1")).toBe("1");
    await expect(
      driver.find<BookTag2>(BookTag2.name, { books: { $in: ["1"] } }),
    ...
    await conn.execute(
      orm.em.getRepository(Author2).getKnex().raw("select 1"),
      [],
      "all",
      tx,
    );
    ...
    // multi updates
    const res3 = await driver.nativeUpdateMany<Publisher2>(
      Publisher2.name,
      [1, 2, 3],
      [
  ...
  test("connection returns correct URL", async () => {
    const conn1 = new MySqlConnection(
      new Configuration(
        {
          driver: MySqlDriver,
    ...
    await expect(conn1.getClientUrl()).toBe(
      "mysql://usr:*****@example.host.com:1234",
    );
    const conn2 = new MySqlConnection(
      new Configuration({ driver: MySqlDriver, port: 3307 } as any, false),
    );
  ...
  test(`populating inverse side of 1:1 also back-links inverse side's owner`, async () => {
    ...
    const a = await repo.findOne(bar.id, {
      populate: ["baz"],
  ...
  test(`1:1 relationships with an inverse side primary key of 0 should link`, async () => {
    ...
    const response = await orm.em.execute(
      "set sql_mode = 'NO_AUTO_VALUE_ON_ZERO'; insert into foo_baz2 (id, name) values (?, ?); set sql_mode = ''",
      [0, "testBaz"],
      "run",
    );
  ...
  test("should load entities", async () => {
    ...
    expect(
      await authorRepository.findOne({ favouriteBook: bible.uuid }),
    ).not.toBe(null);
    orm.em.clear();
    ...
    expect(
      await authorRepository.findOne({ email: "not existing" }),
    ).toBeNull();
    await expect(
      orm.em.populate([] as Author2[], ["books", "favouriteBook"]),
    ).resolves.toEqual([]);
  ...
  test("json properties", async () => {
    ...
    const b2 = await orm.em.findOneOrFail(Book2, {
      meta: { category: "god like", items: 3 },
    });
  ...
  test("findOne supports optimistic locking [versioned entity]", async () => {
    ...
    const test2 = await orm.em.findOne(Test2, test.id, {
      ...
      lockVersion: test.version,
    });
    await orm.em.lock(test2!, LockMode.OPTIMISTIC, test.version);
    const test3 = await orm.em.findOne(Test2, test.id, {
      lockMode: LockMode.OPTIMISTIC,
      lockVersion: test.version,
    });
  ...
  test("findOne supports optimistic locking [unversioned entity]", async () => {
    ...
    await expect(
      orm.em.findOne(Author2, author.id, { lockMode: LockMode.OPTIMISTIC }),
    ).rejects.toThrow(
      "Cannot obtain optimistic lock on unversioned entity Author2",
    );
  });
  ...
  test("lock supports pessimistic locking [pessimistic write]", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "select 1 from `author2` as `a0` where `a0`.`id` = ? for update",
    );
  ...
  test("lock supports pessimistic locking [pessimistic read]", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      "select 1 from `author2` as `a0` where `a0`.`id` = ? lock in share mode",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
    ...
    await orm.em.transactional(async (em) => {
      await em.findOne(
        Author2,
        { email: "foo" },
        { lockMode: LockMode.PESSIMISTIC_READ, strategy: "select-in" },
      );
    });
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      " from `author2` as `a0` left join `address2` as `a1` on `a0`.`id` = `a1`.`author_id` where `a0`.`email` = ? limit ? lock in share mode",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
  });
  ...
  test("custom query expressions via query builder", async () => {
    ...
    const res1 = await qb1
      .select("*")
      .where({
        [raw("JSON_CONTAINS(`b0`.`meta`, ?)", [{ foo: "bar" }])]: false,
      })
      .execute("get");
    ...
    const res2 = await qb2
      .select("*")
      .where({ [raw("JSON_CONTAINS(meta, ?)", [{ category: "foo" }])]: true })
      .execute("get", false);
    ...
    const res4 = await orm.em.fork().findOneOrFail(Book2, {
      [raw("JSON_CONTAINS(meta, ?)", [{ items: 1 }])]: true,
    });
  ...
  test("tuple comparison", async () => {
    ...
    const mock = mockLogger(orm, ["query"]);
    const res4 = await orm.em.findOneOrFail(Book2, {
      [raw<Book2>(["price", "createdAt"])]: { $lte: [100, new Date()] },
    });
    expect(res4).toBeInstanceOf(Book2);
    expect(res4.createdAt).toBeDefined();
    expect(res4.price).toBe(100.0);
    expect(res4.meta).toEqual({ category: "foo", items: 1 });
    expect(mock.mock.calls[0][0]).toMatch(
      "where `b0`.`author_id` is not null and (`b0`.`price`, `b0`.`created_at`) <= (?, ?)",
    );
  });
  ...
  test("populate OneToOne relation on inverse side (select-in)", async () => {
    ...
    const b0 = (await orm.em.findOne(
      FooBaz2,
      { id: baz.id },
      { strategy: "select-in" },
    ))!;
    expect(mock.mock.calls[0][0]).toMatch(
      "select `f0`.*, `f1`.`id` as `bar_id` from `foo_baz2` as `f0` left join `foo_bar2` as `f1` on `f0`.`id` = `f1`.`baz_id` where `f0`.`id` = 3 limit 1",
    );
    ...
    const b1 = (await orm.em.findOne(
      FooBaz2,
      { id: baz.id },
      { populate: ["bar"], strategy: "select-in" },
    ))!;
    expect(mock.mock.calls[1][0]).toMatch(
      "select `f0`.*, `f1`.`id` as `bar_id` from `foo_baz2` as `f0` left join `foo_bar2` as `f1` on `f0`.`id` = `f1`.`baz_id` where `f0`.`id` = 3 limit 1",
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "select `f0`.*, (select 123) as `random` from `foo_bar2` as `f0` where `f0`.`baz_id` in (3)",
    );
    ...
    const b2 = (await orm.em.findOne(
      FooBaz2,
      { bar: bar.id },
      { populate: ["bar"], strategy: "select-in" },
    ))!;
    expect(mock.mock.calls[3][0]).toMatch(
      "select `f0`.*, `f1`.`id` as `bar_id` from `foo_baz2` as `f0` left join `foo_bar2` as `f1` on `f0`.`id` = `f1`.`baz_id` where `f1`.`id` = 2 limit 1",
    );
    expect(mock.mock.calls[4][0]).toMatch(
      "select `f0`.*, (select 123) as `random` from `foo_bar2` as `f0` where `f0`.`baz_id` in (3)",
    );
  ...
  test("populate passes nested where and orderBy", async () => {
    ...
    expect(a0[0].books.map((b) => b.title)).toEqual([
      "b1",
      "b2",
      "b3",
      "b4",
    ...
    expect(a1[0].books.map((b) => b.title)).toEqual([
      "b5",
      "b4",
      "b3",
      "b2",
    ...
    expect(a2.books.map((b) => b.title)).toEqual([
      "b5",
      "b4",
      "b3",
      "b2",
    ...
    const a3 = await orm.em.findOneOrFail(
      Author2,
      { books: { tags: { name: { $in: ["silly", "strange"] } } } },
      {
        populate: ["books.tags"],
        populateWhere: PopulateHint.INFER,
        orderBy: {
          books: { tags: { name: QueryOrder.DESC }, title: QueryOrder.ASC },
        },
      },
    );
    ...
    const a4 = await orm.em.findOneOrFail(
      Author2,
      { books: { tags: { name: { $in: ["silly", "strange"] } } } },
      {
        populate: ["books.tags"],
    ...
    const a5 = await orm.em.findOneOrFail(
      Author2,
      { books: { tags: { name: { $in: ["silly", "strange"] } } } },
      {
        populate: ["books.tags"],
    ...
    expect(a5.books.map((b) => b.title)).toEqual([
      "b4",
      "b1",
      "b2",
      "b3",
    ...
    const a6 = await orm.em.findOneOrFail(
      Author2,
      { books: { tags: { name: { $in: ["silly", "strange"] } } } },
      {
        populate: ["books.tags"],
    ...
    expect(a6.books.map((b) => b.title)).toEqual([
      "b4",
      "b1",
      "b2",
      "b3",
  ...
  test("many to many working with inverse side", async () => {
    ...
    await orm.em.flush();
    orm.em.clear();
    tag = await orm.em.findOneOrFail(BookTag2, tag1.id, {
      populate: ["books"],
    });
    ...
    book4 = await orm.em.findOneOrFail(Book2, book4.uuid, {
      populate: ["tags.books"],
    });
  ...
  test("populating many to many relation", async () => {
    ...
    orm.em.clear();
    const publishers2 = await repo.findAll({
      populate: ["tests:ref"],
      orderBy: { id: 1 },
    });
    ...
    orm.em.clear();
    const publishers3 = await repo.findAll({
      populate: ["tests:ref"],
      strategy: "joined",
      orderBy: { id: 1 },
    });
  ...
  test("nested populating", async () => {
    ...
    expect(
      tags0[0].books[0].publisher!.unwrap().tests.isInitialized(true),
    ).toBe(true);
    ...
    expect(
      tags1[0].books[0].publisher!.unwrap().tests.isInitialized(true),
    ).toBe(true);
    ...
    expect(
      tags2[0].books[0].publisher!.unwrap().tests.isInitialized(true),
    ).toBe(true);
    ...
    expect(
      tags3[0].books[0].publisher!.unwrap().tests.isInitialized(true),
    ).toBe(true);
  ...
  test("collection allows populate, custom where and orderBy", async () => {
    ...
    expect(ent1.tags.map((t) => t.name)).toEqual([
      tag1.name,
      tag2.name,
      tag3.name,
      tag4.name,
    ...
    expect(ent2.tags.map((t) => t.name)).toEqual([
      tag4.name,
      tag1.name,
      tag3.name,
      tag5.name,
    ...
    await ent3.tags.init({
      where: { name: { $ne: "funny" } },
      orderBy: { name: QueryOrder.DESC },
    });
    expect(ent3.tags.map((t) => t.name)).toEqual([
      tag4.name,
      tag1.name,
      tag3.name,
      tag5.name,
  ...
  test("many to many collection allows custom orderBy", async () => {
    ...
    expect(books[0].tags.map((t) => t.name)).toEqual([
      "awkward",
      "sexy",
      "strange",
    ]);
    ...
    expect(books[2].tags.map((t) => t.name)).toEqual([
      "awkward",
      "sick",
      "silly",
      "zupa",
  ...
  test("populateWhere: all/infer with different loading strategies", async () => {
    ...
    expect(books1[0].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "funny",
      "sexy",
      "strange",
    ...
    expect(books1[1].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "funny",
      "sexy",
      "silly",
      "zupa",
    ...
    expect(books1[2].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sick",
      "silly",
      "zupa",
    ...
    expect(books2[0].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sexy",
      "strange",
    ]);
    expect(books2[1].title).toBe("My Life on The Wall, part 2");
    expect(books2[1].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "sexy",
      "silly",
      "zupa",
    ]);
    expect(books2[2].title).toBe("My Life on The Wall, part 1");
    expect(books2[2].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sick",
      "silly",
      "zupa",
    ...
    orm.em.clear();
    const books3 = await orm.em.find(
      Book2,
      { tagsUnordered: { name: { $ne: "funny" } } },
      {
        populate: ["tagsUnordered", "perex"],
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.*, `b0`.price * 1.19 as `price_taxed`, `t1`.`id` as `t1__id`, `t1`.`name` as `t1__name`, `t5`.`id` as `test_id` " +
        "from `book2` as `b0` " +
    ...
    expect(books3[0].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "funny",
      "sexy",
      "strange",
    ...
    expect(books3[1].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "funny",
      "sexy",
      "silly",
      "zupa",
    ...
    expect(books3[2].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sick",
      "silly",
      "zupa",
    ...
    orm.em.clear();
    mock.mockReset();
    const books4 = await orm.em.find(
      Book2,
      { tagsUnordered: { name: { $ne: "funny" } } },
      {
        populate: ["tagsUnordered", "perex"],
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.*, `b0`.price * 1.19 as `price_taxed`, `t1`.`id` as `t1__id`, `t1`.`name` as `t1__name`, `t3`.`id` as `test_id` " +
        "from `book2` as `b0` " +
        "left join `book_to_tag_unordered` as `b2` on `b0`.`uuid_pk` = `b2`.`book2_uuid_pk` " +
    ...
    expect(books4[0].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sexy",
      "strange",
    ]);
    expect(books4[1].title).toBe("My Life on The Wall, part 2");
    expect(books4[1].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "sexy",
      "silly",
      "zupa",
    ]);
    expect(books4[2].title).toBe("My Life on The Wall, part 1");
    expect(books4[2].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sick",
      "silly",
      "zupa",
    ...
    orm.em.clear();
    mock.mockReset();
    const books5 = await orm.em.find(
      Book2,
      { tagsUnordered: { name: { $ne: "funny" } } },
      {
        populate: ["tagsUnordered", "perex"],
    ...
    expect(books5[0].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sexy",
      "strange",
    ]);
    expect(books5[1].title).toBe("My Life on The Wall, part 2");
    expect(books5[1].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "sexy",
      "silly",
      "zupa",
    ]);
    expect(books5[2].title).toBe("My Life on The Wall, part 1");
    expect(books5[2].tagsUnordered.getItems().map((t) => t.name)).toEqual([
      "awkward",
      "sick",
      "silly",
      "zupa",
    ...
    const tags = await orm.em.find(
      BookTag2,
      { booksUnordered: { title: { $ne: "My Life on The Wall, part 3" } } },
      {
    ...
    expect(tags.map((tag) => tag.booksUnordered.count())).toEqual([
      1, 1, 1, 1, 2, 2,
    ]);
  });
  ...
  test("self referencing M:N (unidirectional)", async () => {
    ...
    expect(jon.friends.getIdentifiers()).toEqual([
      a1.id,
      a2.id,
      a3.id,
      author.id,
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `a0`.*, `a3`.`author_id` as `address_author_id` " +
        "from `author2` as `a0` " +
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "select `a1`.*, `a0`.`author2_2_id` as `fk__author2_2_id`, `a0`.`author2_1_id` as `fk__author2_1_id`, `a2`.`author_id` as `address_author_id` " +
    ...
    const jon2 = await orm.em.findOneOrFail(
      Author2,
      { friends: a2.id },
      {
        populate: ["friends"],
    ...
    expect(jon2.friends.getIdentifiers()).toEqual([
      a1.id,
      a2.id,
      a3.id,
      author.id,
  ...
  test("self referencing M:N (owner)", async () => {
    ...
    expect(jon.following.getIdentifiers()).toEqual([
      a1.id,
      a2.id,
      a3.id,
      author.id,
    ...
    expect(jon2.following.getIdentifiers()).toEqual([
      a1.id,
      a2.id,
      a3.id,
      author.id,
  ...
  test("EM supports native insert/update/delete", async () => {
    ...
    const res6 = await orm.em.nativeUpdate<Author2>(
      "author2",
      { name: "new native name" },
      { name: "native name 3" },
    );
    ...
    const id = await orm.em.insert(Author2, {
      name: "native name 1",
  ...
  test("self referencing (1 step)", async () => {
    ...
    expect(mock.mock.calls[5][0]).toMatch(
      "select `a0`.*, `a1`.`author_id` as `address_author_id` from `author2` as `a0` left join `address2` as `a1` on `a0`.`id` = `a1`.`author_id` where `a0`.`id` = ?",
    );
  });
  ...
  test("persisting entities in parallel inside forked EM with copied IM", async () => {
    ...
    orm.em.clear();
    const a1 = (await orm.em.findOne(Author2, author.id, {
      populate: ["books"],
    }))!;
  ...
  test("lookup by array", async () => {
    ...
    await orm.em.persistAndFlush(author);
    await expect(
      orm.em.count(Book2, [book1.uuid, book2.uuid, book3.uuid]),
    ).resolves.toBe(3);
    ...
    await expect(
      orm.em.getRepository(Book2).count([book1, book2, book3]),
    ).resolves.toBe(3);
  });
  ...
  test("insert with raw sql fragment", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      /insert into `author2` \(`id`, `created_at`, `updated_at`, `name`, `email`, `age`, `terms_accepted`\) values \(1, '.*', '.*', 'name', 'email', 100 \+ 20 \+ 3, false\)/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "select `a0`.`id`, `a0`.`age` from `author2` as `a0` where `a0`.`id` in (1)",
    );
  ...
  test("update with raw sql fragment", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      /update `author2` set `age` = age \* 2, `updated_at` = '.*' where `id` = 1/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "select `a0`.`id`, `a0`.`age` from `author2` as `a0` where `a0`.`id` in (1)",
    );
  ...
  test("update with raw sql fragment (batch)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      /update `author2` set `age` = case when \(`id` = 1\) then age \* 2 when \(`id` = 2\) then age \/ 2 else `age` end, `updated_at` = case when \(`id` = 1\) then '.*' when \(`id` = 2\) then '.*' else `updated_at` end where `id` in \(1, 2\)/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "select `a0`.`id`, `a0`.`age` from `author2` as `a0` where `a0`.`id` in (1, 2)",
    );
    expect(mock.mock.calls[3][0]).toMatch("commit");
  ...
  test("find by joined property", async () => {
    ...
    const res1 = await orm.em.find(
      ...
      { populate: ["perex"], strategy: "select-in" },
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.*, `b0`.price * 1.19 as `price_taxed`, `t2`.`id` as `test_id` " +
        "from `book2` as `b0` " +
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.*, `b0`.price * 1.19 as `price_taxed`, `t4`.`id` as `test_id` " +
        "from `book2` as `b0` " +
        "left join `author2` as `a1` on `b0`.`author_id` = `a1`.`id` " +
  ...
  test("find and count", async () => {
    ...
    const [authors1, count1] = await orm.em.findAndCount(
      Author2,
      {},
      { limit: 10, offset: 10, orderBy: { id: QueryOrder.ASC } },
    );
    ...
    const [authors2, count2] = await orm.em.findAndCount(
      Author2,
      {},
      {
        limit: 10,
  ...
  test("query highlighting", async () => {
    const mock = mockLogger(orm, ["query"]);
    Object.assign(orm.config.getLogger(), {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      "[37m[1minsert[22m[39m [37m[1minto[22m[39m [33m`author2`[39m ([33m`created_at`[39m[0m,[0m [33m`updated_at`[39m[0m,[0m [33m`name`[39m[0m,[0m [33m`email`[39m[0m,[0m [33m`terms_accepted`[39m) [37m[1mvalues[22m[39m (?[0m,[0m ?[0m,[0m ?[0m,[0m ?[0m,[0m ?)",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
  ...
  test("colors: false", async () => {
    ...
    expect(mock.mock.calls.length).toBe(3);
    expect(mock.mock.calls[0][0]).toMatch(
      "\x1B[90m[query] \x1B[39mbegin\x1B[36m (via write connection '127.0.0.1')\x1B[39m",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /\x1B\[90m\[query] \x1B\[39minsert into `author2` \(`created_at`, `updated_at`, `name`, `email`, `terms_accepted`\) values \(\?, \?, \?, \?, \?\)\x1B\[90m \[took \d+ ms, 1 row affected]\x1B\[39m\x1B\[36m \(via write connection '127\.0\.0\.1'\)\x1B\[39m/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "\x1B[90m[query] \x1B[39mcommit\x1B[36m (via write connection '127.0.0.1')\x1B[39m",
    );
    ...
    expect(mock.mock.calls.length).toBe(3);
    expect(mock.mock.calls[0][0]).toMatch(
      "[query] begin (via write connection '127.0.0.1')",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /\[query] delete from `author2` where `id` in \(\?\) \[took \d+ ms, 1 row affected] \(via write connection '127\.0\.0\.1'\)/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "[query] commit (via write connection '127.0.0.1')",
    );
  });
  ...
  test("datetime is stored in correct timezone", async () => {
    ...
    const res = await orm.em
      ...
      .execute<
        { created_at: string }[]
      >(`select date_format(created_at, '%Y-%m-%d %T.%f') as created_at from author2 where id = ${author.id}`);
  ...
  test("explicit removing of entity that is loaded as a relation already", async () => {
    ...
    const a = await orm.em.findOneOrFail(FooBar2, bar.id, {
      populate: ["baz"],
    });
    orm.em.remove(a.baz!);
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      "delete from `foo_baz2` where `id` in (?)",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
  });

  test("em.remove() with null or undefined in where parameter throws", async () => {
    expect(() => orm.em.remove(undefined as any)).toThrow(
      `You need to pass entity instance or reference to 'em.remove()'. To remove entities by condition, use 'em.nativeDelete()'.`,
    );
    expect(() => orm.em.remove(null as any)).toThrow(
      `You need to pass entity instance or reference to 'em.remove()'. To remove entities by condition, use 'em.nativeDelete()'.`,
    );
    expect(() => orm.em.remove({} as any)).toThrow(
      `You need to pass entity instance or reference to 'em.remove()'. To remove entities by condition, use 'em.nativeDelete()'.`,
    );
    expect(() => orm.em.remove({ foo: 1 } as any)).toThrow(
      `You need to pass entity instance or reference to 'em.remove()'. To remove entities by condition, use 'em.nativeDelete()'.`,
    );
  });
  ...
  test("adding items to not initialized collection", async () => {
    ...
    const a2 = await orm.em.findOneOrFail(Author2, god.id, {
      populate: ["books"],
    });
    ...
    const a3 = await orm.em.findOneOrFail(Author2, god.id, {
      populate: ["books"],
    });
    ...
    const a4 = await orm.em.findOneOrFail(Author2, god.id, {
      populate: ["books.tags"],
    });
    expect(a4.books[0].tags.getIdentifiers()).toEqual([tag1.id, tag3.id]);
    expect(a4.books[1].tags.getIdentifiers()).toEqual([
      tag1.id,
      tag2.id,
      tag5.id,
    ]);
  ...
  test("pagination", async () => {
    ...
    const res1 = await orm.em.find(
      ...
      { books: { title: /^Bible/ } },
      {
        ...
        having: {
          $or: [{ age: { $gt: 0 } }, { age: { $lte: 0 } }, { age: null }],
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `a0`.*, `a2`.`author_id` as `address_author_id` " +
        "from `author2` as `a0` " +
    ...
    expect(res3.map((a) => a.name)).toEqual([
      "God 01",
      "God 02",
      "God 03",
      "God 04",
  ...
  test("formulas", async () => {
    ...
    expect(b.price).toBe(1000.0);
    expect(b.priceTaxed).toBe("1190.0000");
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.`uuid_pk`, `b0`.`created_at`, `b0`.`title`, `b0`.`price`, `b0`.`double`, `b0`.`meta`, `b0`.`author_id`, `b0`.`publisher_id`, `b0`.price * 1.19 as `price_taxed`, `t2`.`id` as `test_id` " +
        "from `book2` as `b0` " +
        "left join `author2` as `a1` on `b0`.`author_id` = `a1`.`id` " +
        "left join `test2` as `t2` on `b0`.`uuid_pk` = `t2`.`book_uuid_pk` " +
        "where `b0`.`author_id` is not null and `a1`.`name` = ? " +
    ...
    await orm.em.findOneOrFail(
      FooBar2,
      { random: { $gt: 0.5 } },
      { having: { random: { $gt: 0.5 } } },
    );
    expect(mock.mock.calls[1][0]).toMatch("begin");
    expect(mock.mock.calls[2][0]).toMatch(
      "insert into `foo_bar2` (`name`) values (?)",
    );
    expect(mock.mock.calls[3][0]).toMatch(
      "select `f0`.`id`, `f0`.`version` from `foo_bar2` as `f0` where `f0`.`id` in (?)",
    );
    expect(mock.mock.calls[4][0]).toMatch("commit");
    expect(mock.mock.calls[5][0]).toMatch(
      "select `f0`.*, (select 123) as `random` from `foo_bar2` as `f0` where (select 123) > ? having (select 123) > ? limit ?",
    );
  });
  ...
  test("lazy formulas (gh #1229)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `f0`.*, (select 123) as `random` from `foo_bar2` as `f0` where `f0`.`id` = ? limit ?",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      "select `f0`.`id`, (select 456) as `lazy_random` from `foo_bar2` as `f0` where `f0`.`id` in (?)",
    );
    orm.em.clear();

    const b2 = await orm.em.findOneOrFail(FooBar2, b.id, {
      populate: ["lazyRandom"],
    });
    expect(b2.random).toBe(123);
    expect(b2.lazyRandom).toBe(456);
    expect(mock.mock.calls[2][0]).toMatch(
      "select `f0`.*, (select 123) as `random`, (select 456) as `lazy_random` from `foo_bar2` as `f0` where `f0`.`id` = ? limit ?",
    );
  });
  ...
  test("search by formulas (gh #3048)", async () => {
    ...
    const b = await orm.em
      .fork()
      .findOneOrFail(
        Book2,
        { priceTaxed: "1190.0000" },
        { strategy: "select-in" },
      );
    expect(b.price).toBe(1000.0);
    expect(b.priceTaxed).toBe("1190.0000");
    expect(mock.mock.calls[0][0]).toMatch(
      "select `b0`.`uuid_pk`, `b0`.`created_at`, `b0`.`title`, `b0`.`price`, `b0`.`double`, `b0`.`meta`, `b0`.`author_id`, `b0`.`publisher_id`, `b0`.price * 1.19 as `price_taxed`, `t1`.`id` as `test_id` " +
        "from `book2` as `b0` " +
    ...
    const a1 = await orm.em
      ...
      .find(
        Author2,
        { $or: [{ favouriteBook: { priceTaxed: "1190.0000" } }] },
        { populate: ["books"], strategy: "select-in" },
      );
    expect(a1[0].books[0].price).toBe(1000.0);
    ...
    const a2 = await orm.em
      .fork()
      .find(
        Author2,
        { favouriteBook: { $or: [{ priceTaxed: "1190.0000" }] } },
        { populate: ["books"], strategy: "select-in" },
      );
    expect(a2[0].books[0].price).toBe(1000.0);
  ...
  test("batch update with changing OneToOne relation (GH issue #1025)", async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      "update `foo_bar2` set `foo_bar_id` = case when (`id` = ?) then ? when (`id` = ?) then ? else `foo_bar_id` end, `version` = current_timestamp where `id` in (?, ?)",
    );
    expect(mock.mock.calls[3][0]).toMatch(
      "select `f0`.`id`, `f0`.`version` from `foo_bar2` as `f0` where `f0`.`id` in (?, ?)",
    );
  ...
  test("exceptions", async () => {
    ...
    await expect(
      orm.em.insert(Author2, { name: "author", email: "email" }),
    ).rejects.toThrow(UniqueConstraintViolationException);
    await expect(
      orm.em.insert<any>("not_existing", { foo: "bar" }),
    ).rejects.toThrow(TableNotFoundException);
    await expect(
      orm.em.execute("create table author2 (foo text not null)"),
    ).rejects.toThrow(TableExistsException);
    await expect(orm.em.execute("foo bar 123")).rejects.toThrow(
      SyntaxErrorException,
    );
    await expect(
      orm.em.execute("select id from author2, foo_bar2"),
    ).rejects.toThrow(NonUniqueFieldNameException);
    await expect(orm.em.execute("select uuid from author2")).rejects.toThrow(
      InvalidFieldNameException,
    );
  ...
  test("GH 625", async () => {
    ...
    const res = await orm.em.findOne(Author2, {
      id: a.id,
      termsAccepted: true,
    });
    expect(res).toBeNull();
  ...
  test("em.execute()", async () => {
    const res1 = await orm.em.execute(
      "insert into author2 (name, email) values (?, ?)",
      ["name", "email"],
    );
    ...
    const config = new Configuration(
      {
        ...
        replicas: [{ clientUrl: "mysql://usr2:pswd2@129.0.0.1:5434/bar" }],
