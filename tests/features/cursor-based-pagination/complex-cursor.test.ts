...
describe.each([
  ...
  "postgresql",
  "mongo",
] as const)("simple cursor based pagination (%s)", (type) => {
  let orm: MikroORM;
  ...
  test("complex cursor based pagination using `first` and `after` (id asc)", async () => {
    ...
    const where = {
      $or: [{ termsAccepted: true }, { name: "User 1" }, { age: { $lte: 30 } }],
      name: { $ne: "User 2" },
  ...
  test("complex joined cursor based pagination using `last` and `before` (id asc)", async () => {
    if (type === "mongo") {
      // skip in mongo, there are no joins, so we can't order by a relation property
      expect(1).toBe(1);
      return;
    }
    ...
    const mock = mockLogger(orm, ["query", "query-params"]);
    const where = {
      bestFriend: { name: "User 3" },
    } satisfies FilterQuery<User>;
    const orderBy = {
      bestFriend: { email: "asc", name: "asc" },
      name: "desc",
      age: "asc",
      email: "asc",
    } as const;
    ...
    expect(cursor1).toBeInstanceOf(Cursor);
    expect(
      serialize(cursor1.items, { populate: ["bestFriend"] }),
    ).toMatchObject([
      {
        bestFriend: { email: "email-96", name: "User 3" },
        name: "User 3",
        age: 38,
        email: "email-76",
      },
    ...
    expect(cursor1.startCursor).toBe(
      "W3siYmVzdEZyaWVuZCI6eyJlbWFpbCI6ImVtYWlsLTk2IiwibmFtZSI6IlVzZXIgMyJ9fSwiVXNlciAzIiwzOCwiZW1haWwtNzYiXQ",
    );
    expect(cursor1.endCursor).toBe(
      "W3siYmVzdEZyaWVuZCI6eyJlbWFpbCI6ImVtYWlsLTk2IiwibmFtZSI6IlVzZXIgMyJ9fSwiVXNlciAzIiw0OCwiZW1haWwtOTYiXQ",
    );
    ...
    expect(cursor2).toBeInstanceOf(Cursor);
    expect(
      serialize(cursor2.items, { populate: ["bestFriend"] }),
    ).toMatchObject([
      {
        bestFriend: { email: "email-96", name: "User 3" },
        name: "User 3",
    ...
    expect(cursor2.startCursor).toBe(
      "W3siYmVzdEZyaWVuZCI6eyJlbWFpbCI6ImVtYWlsLTk2IiwibmFtZSI6IlVzZXIgMyJ9fSwiVXNlciAzIiwyNiwiZW1haWwtNTEiXQ",
    );
    expect(cursor2.endCursor).toBe(
      "W3siYmVzdEZyaWVuZCI6eyJlbWFpbCI6ImVtYWlsLTk2IiwibmFtZSI6IlVzZXIgMyJ9fSwiVXNlciAzIiwzNiwiZW1haWwtNzEiXQ",
    );
    expect(cursor2.hasNextPage).toBe(false);
    expect(cursor2.hasPrevPage).toBe(true);
    queries = mock.mock.calls.map((call) => call[0]).sort();
