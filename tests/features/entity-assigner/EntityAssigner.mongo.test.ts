import type { EntityData, MikroORM } from "@yandjin-mikro-orm/core";
import { assign, wrap } from "@yandjin-mikro-orm/core";
...
describe("EntityAssignerMongo", () => {
  ...
  test("#assign() should update entity collection", async () => {
    ...
    expect(() =>
      assign(book, { publisher: [{ foo: "bar" }] } as EntityData<Book>),
    ).toThrow(
  ...
  test("#assign() should ignore undefined properties in nullability validation (#4566)", async () => {
    ...
    assign<any>(
      jon,
      { name: "test", emptyUnknown1: null, emptyUnknown2: undefined },
      { onlyProperties: true },
    );
  ...
  test("#assign() should merge references", async () => {
    ...
    orm.em.assign(
      jon,
      { favouriteBook: { _id: ObjectId.createFromTime(1), title: "b1" } },
      { merge: false },
    );
