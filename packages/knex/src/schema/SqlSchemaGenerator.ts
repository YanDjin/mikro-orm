...
export class SqlSchemaGenerator
  extends AbstractSchemaGenerator<AbstractSqlDriver>
  implements ISchemaGenerator
{
  ...
  getTargetSchema(schema?: string): DatabaseSchema {
    ...
    const metadataOrdered = metadataRootOrdered.reduce(
      (prev: EntityMetadata[], curr) => [
        ...prev,
        ...metadata.filter((m) => m.root.className === curr.className),
      ],
      [],
    );
    return DatabaseSchema.fromMetadata(
      metadataOrdered,
      this.platform,
      this.config,
      schemaName,
  ...
  override async getCreateSchemaSQL(
    ...
    for (const namespace of toSchema.getNamespaces()) {
      ...
      ret += await this.dump(
        this.knex.schema.createSchemaIfNotExists(namespace),
      );
    }
    ...
    for (const tableDef of toSchema.getTables()) {
      ret += await this.dump(
        this.createSchemaBuilder(tableDef.schema).alterTable(
          tableDef.name,
          (table) => this.createForeignKeys(table, tableDef, options.schema),
        ),
      );
    }
  ...
  override async getDropSchemaSQL(
    ...
    for (const meta of metadata) {
      ...
      if (!this.platform.usesCascadeStatement() && table && !wrap) {
        for (const fk of Object.values(table.getForeignKeys())) {
          const builder = this.createSchemaBuilder(table.schema).alterTable(
            table.name,
            (tbl) => {
    ...
    for (const meta of metadata) {
      ret += await this.dump(
        this.dropTable(meta.collection, this.getSchemaName(meta, options)),
        "\n",
      );
    }
    ...
    if (this.platform.supportsNativeEnums()) {
      for (const columnName of Object.keys(schema.getNativeEnums())) {
        const sql = this.helper.getDropNativeEnumSQL(
          columnName,
          options.schema ?? this.config.get("schema"),
        );
    ...
    if (options.dropMigrationsTable) {
      ret += await this.dump(
        this.dropTable(
          this.config.get("migrations").tableName!,
          this.config.get("schema"),
        ),
        "\n",
      );
  ...
  override async getUpdateSchemaSQL(
    options: UpdateSchemaOptions<DatabaseSchema> = {},
  ): Promise<string> {
    await this.ensureDatabase();
    const { fromSchema, toSchema } =
      await this.prepareSchemaForComparison(options);
  ...
  private async prepareSchemaForComparison(
    options: UpdateSchemaOptions<DatabaseSchema>,
  ) {
  ...
  async diffToSQL(
    ...
    if (this.platform.supportsSchemas()) {
      for (const newNamespace of schemaDiff.newNamespaces) {
        ...
        ret += await this.dump(
          this.knex.schema.createSchemaIfNotExists(newNamespace),
        );
      }
    }
    ...
    if (!options.safe) {
      for (const orphanedForeignKey of schemaDiff.orphanedForeignKeys) {
        const [schemaName, tableName] = this.splitTableName(
          orphanedForeignKey.localTableName,
        );
        ret += await this.dump(
          this.createSchemaBuilder(schemaName).alterTable(
            tableName,
            (table) => {
              return table.dropForeign(
                orphanedForeignKey.columnNames,
                orphanedForeignKey.constraintName,
              );
            },
          ),
        );
    ...
    for (const newTable of Object.values(schemaDiff.newTables)) {
      ret += await this.dump(
        this.createSchemaBuilder(newTable.schema).alterTable(
          newTable.name,
          (table) => {
  ...
  private createForeignKey(
    ...
    const builder = table
      ...
      .inTable(
        this.getReferencedTableName(foreignKey.referencedTableName, schema),
      )
  ...
  private preAlterTable(
    ...
    const push = (sql: string) =>
      sql ? ret.push(this.knex.schema.raw(sql)) : undefined;
    push(this.helper.getPreAlterTable(diff, safe));
    const [schemaName, tableName] = this.splitTableName(diff.name);

    ret.push(
      this.createSchemaBuilder(schemaName).alterTable(tableName, (table) => {
  ...
  private alterTable(
    ...
    if (this.platform.supportsNativeEnums()) {
      ...
      for (const { column, changedProperties } of Object.values(
        diff.changedColumns,
      )) {
        if (
      ...
      Utils.removeDuplicates(changedNativeEnums).forEach(
        ([enumName, itemsNew, itemsOld]) => {
          // postgres allows only adding new items, the values are case insensitive
          itemsOld = itemsOld.map((v) => v.toLowerCase());
          const newItems = itemsNew.filter(
            (val) => !itemsOld.includes(val.toLowerCase()),
          );
          ret.push(
            ...newItems.map((val) =>
              this.knex.schema.raw(
                this.helper.getAlterNativeEnumSQL(enumName, schemaName, val),
              ),
            ),
          );
        },
      ...
      this.createSchemaBuilder(schemaName).alterTable(tableName, (table) => {
        ...
        if (!safe) {
          for (const column of Object.values(diff.removedColumns)) {
            this.helper.pushTableQuery(
              table,
              `alter table ${this.platform.quoteIdentifier(tableName)} drop column ${this.platform.quoteIdentifier(column.name)}`,
            );
          }
        }
      }),
    ...
    ret.push(
      this.createSchemaBuilder(schemaName).alterTable(tableName, (table) => {
        for (const column of Object.values(diff.addedColumns)) {
          const col = this.helper.createTableColumn(
            table,
            column,
            diff.fromTable,
            undefined,
          ...
          const foreignKey = Object.values(diff.addedForeignKeys).find(
            (fk) =>
              fk.columnNames.length === 1 && fk.columnNames[0] === column.name,
          );
          ...
          if (foreignKey && this.options.createForeignKeyConstraints) {
            ...
            col
              ...
              .inTable(
                this.getReferencedTableName(foreignKey.referencedTableName),
              )
              .withKeyName(foreignKey.constraintName)
        ...
        for (const { column } of Object.values(diff.changedColumns).filter(
          (diff) => diff.changedProperties.has("autoincrement"),
        )) {
          this.helper.pushTableQuery(
            table,
            this.helper.getAlterColumnAutoincrement(
              tableName,
              column,
              schemaName,
            ),
        ...
        for (const { column, changedProperties } of Object.values(
          diff.changedColumns,
        ).filter((diff) => diff.changedProperties.has("comment"))) {
          if (
            [
              "type",
              "nullable",
          ...
          this.helper.pushTableQuery(
            table,
            this.helper.getChangeColumnCommentSQL(
              tableName,
              column,
              schemaName,
            ),
        ...
        for (const [oldColumnName, column] of Object.entries(
          ...
          this.helper.pushTableQuery(
            table,
            this.helper.getRenameColumnSQL(
              tableName,
              oldColumnName,
              column,
              schemaName,
        ...
        for (const [oldIndexName, index] of Object.entries(
          diff.renamedIndexes,
        )) {
          if (index.unique) {
            ...
            this.helper.pushTableQuery(
              table,
              this.helper.getRenameIndexSQL(diff.name, index, oldIndexName),
            );
          }
        }
  ...
  override async dropDatabase(name?: string): Promise<void> {
    ...
    await this.driver.execute(
      this.helper.getDropDatabaseSQL("" + this.knex.ref(name)),
    );
  }
  ...
  private createTable(
    ...
    return this.createSchemaBuilder(tableDef.schema).createTable(
      tableDef.name,
      (table) => {
        ...
        for (const index of tableDef.getIndexes()) {
          const createPrimary =
            !tableDef.getColumns().some((c) => c.autoincrement && c.primary) ||
        ...
        this.helper.finalizeTable(
          table,
          this.config.get("charset"),
          this.config.get("collate"),
        );
      },
    );
  }
  ...
  private createIndex(
    ...
    if (index.primary) {
      ...
      if (index.columnNames.some((column) => column.includes("."))) {
        ...
        table.index(
          columns.map((column) => this.knex.raw(`(${column})`)),
          index.keyName,
          { indexType: "unique" },
        );
      } else {
      ...
      if (this.platform.supportsCreatingFullTextIndex()) {
        this.helper.pushTableQuery(
          table,
          this.platform.getFullTextIndexExpression(
            index.keyName,
            tableDef.schema,
            tableDef.name,
            columns,
