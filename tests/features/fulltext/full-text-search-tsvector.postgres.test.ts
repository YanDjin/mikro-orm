...
const createWeightedValue = (book: Book): WeightedFullTextValue => ({
  A: book.title!,
  B: book.description!,
});
...
export class Book {
  ...
  @Property({
    type: FullTextType,
    nullable: true,
    onUpdate: createWeightedValue,
    onCreate: createWeightedValue,
  })
...
describe("full text search tsvector in postgres", () => {
  ...
  test("load entities", async () => {
    ...
    const fullTextBooks2 = (await repo.find({
      searchableTitleNoType: { $fulltext: "life wall" },
    }))!;
  ...
  test("should throw error when invalid weight is used", async () => {
    ...
    expect(orm.em.flush()).rejects.toThrow(
      "Weight should be one of A, B, C, D.",
    );
  });
  ...
  test("should find entities with custom regconfig", async () => {
    ...
    const fullTextBooks2 = (await repo.find({
      searchableTitleEnglish: { $fulltext: "life wall" },
    }))!;
    expect(mock.mock.calls[0][0]).toMatch(
    ...
    const fullTextBooks3 = (await repo.find({
      searchableTitleWeighted: { $fulltext: "life wall" },
    }))!;
    expect(mock.mock.calls[0][0]).toMatch(
