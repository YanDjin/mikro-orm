...
it("populate with select-in strategy", async () => {
  ...
  expect(brands1[0].client.account).toMatchObject({
    id: "1",
    name: "Account 1",
  });
  expect(brands1[1].client.account).toMatchObject({
    id: "1",
    name: "Account 1",
  });
});
...
it("populate with query builder", async () => {
  ...
  expect(brands2[0].client.account).toMatchObject({
    id: "1",
    name: "Account 1",
  });
  expect(brands2[1].client.account).toMatchObject({
    id: "1",
    name: "Account 1",
  });
});
