...
import { MikroORM, Utils } from "@yandjin-mikro-orm/core";
import { SeedManager } from "@yandjin-mikro-orm/seeder";
import { SchemaGenerator } from "@yandjin-mikro-orm/sqlite";
...
describe("Seeder", () => {
  ...
  test("seedString", async () => {
    ...
    const re =
      "Seeder class Unknown not found in ./tests/database/seeder/!(*.d).{js,ts}";
  ...
  test("createSeeder (TS)", async () => {
    ...
    expect(seederFile).toBe(
      Utils.normalizePath(process.cwd()) + `/temp/seeders/Publisher3Seeder.ts`,
    );
    const fileContents = await readFile(seederFile, "utf8");
    expect(fileContents).toContain(
      "export class Publisher3Seeder extends Seeder {",
    );
  ...
  test("createSeeder (JS)", async () => {
    ...
    const seederFile = await orm.seeder.createSeeder("Publisher3Seeder");
    expect(seederFile).toBe(
      Utils.normalizePath(process.cwd()) + `/temp/seeders/Publisher3Seeder.js`,
    );
    const fileContents = await readFile(seederFile, "utf8");
    expect(fileContents).toContain(
      "exports.Publisher3Seeder = Publisher3Seeder;",
    );
    await remove(seederFile);
  });
});
