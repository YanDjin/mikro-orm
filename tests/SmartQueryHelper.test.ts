...
import { Reference, QueryHelper } from "@yandjin-mikro-orm/core";
...
describe("QueryHelper", () => {
  ...
  test("test entity conversion to PK", async () => {
    ...
    expect(QueryHelper.processParams({ book: bookRef })).toEqual({
      book: bookRef.uuid,
    });
  ...
  test("test entity conversion to composite PK", async () => {
    ...
    baz.id = 7;
    expect(
      QueryHelper.processParams({ field: new FooParam2(bar, baz, "val") }),
    ).toEqual({ field: [3, 7] });
  });
  ...
  test("test array conversion to $in query", async () => {
    ...
    expect(
      QueryHelper.processWhere<Author2>({
        where: [1, 2, 3],
        entityName: "uuid",
        metadata: orm.getMetadata(),
        platform: orm.em.getDriver().getPlatform(),
    ...
    expect(
      QueryHelper.processWhere<Book2>({
