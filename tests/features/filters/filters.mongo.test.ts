...
describe("filters [mongo]", () => {
  ...
  test("global filters", async () => {
    ...
    em.addFilter(
      "writtenBy",
      async (args) => ({ author: args.author }),
      Book,
      false,
    );
    em.addFilter("tenant", async (args) => ({ tenant: args.tenant }), [
      Author,
      Book,
      FooBar,
    ]);
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `db.getCollection('author').find({ tenant: 123 }, {}).toArray()`,
    );
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ tenant: 123 }, {}\)/,
    );
    ...
    expect(mock.mock.calls[4][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ author: '123' }, {}\)/,
    );
    await em.find(Book, {}, { filters: false, populate: ["perex"] });
    expect(mock.mock.calls[5][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({}, {}\)/,
    );

    await em.find(
      FooBar,
      {},
      { filters: { allowedFooBars: { allowed: [1, 2, 3] } } },
    );
  ...
  test("that filters in the config are enabled by default", async () => {
    ...
    expect(Object.keys(orm.config.get("filters"))[0]).toEqual(
      "needsTermsAccepted",
    );
