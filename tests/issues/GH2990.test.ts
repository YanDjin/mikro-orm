...
describe("GH issue 2990", () => {
  ...
  it("should lookup all (non-circular) eager relationships for population", async () => {
    ...
    const activeParticipant = populate.find(
      (p) => p.field === "activeParticipant",
    );

    expect(
      activeParticipant?.children?.some((p) => p.field === "provider"),
    ).toBe(true);
    expect(activeParticipant?.children?.some((p) => p.field === "user")).toBe(
      true,
    );
  });
});
