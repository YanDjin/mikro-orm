...
export class SchemaCommandFactory {
  ...
  static create<U extends Options = Options>(
    ...
    return {
      ...
      builder: (args: Argv) =>
        SchemaCommandFactory.configureSchemaCommand(args, command) as Argv<U>,
      handler: (args: ArgumentsCamelCase<U>) =>
        SchemaCommandFactory.handleSchemaCommand(args, command, successMessage),
    };
  }
  ...
  static async handleSchemaCommand(
    ...
    if (args.dump) {
      const m =
        `get${method.substr(0, 1).toUpperCase()}${method.substr(1)}SchemaSQL` as
          | "getCreateSchemaSQL"
          | "getUpdateSchemaSQL"
          | "getDropSchemaSQL";
    ...
    if (args.seed !== undefined) {
      ...
      await seeder.seedString(
        args.seed || orm.config.get("seeder").defaultSeeder!,
      );
    }
