...
describe("GH issue 3051", () => {
  ...
  test("find a result and throw if it is not a single entity", async () => {
    const jonSnow = await orm.em.findOneOrFail(
      Author,
      { name: "Jon Snow" },
      { strict: true, populate: ["books"] },
    );

    expect(jonSnow).not.toBeNull();

    await expect(
      orm.em.findOneOrFail(Book, { author: jonSnow }, { strict: true }),
    ).rejects.toThrow(
      `Wrong number of Book entities found for query { author: ${jonSnow.id}n }, expected exactly one`,
    );
    await expect(orm.em.findOneOrFail(Book, { title: "b4" })).rejects.toThrow(
      "Book not found ({ title: 'b4' })",
    );
    await expect(
      orm.em.findOneOrFail(
        Book,
        { author: jonSnow },
        { strict: true, failHandler: () => new Error("Test") },
      ),
      ...
      orm.em.findOneOrFail(
        ...
        { author: jonSnow },
        {
          ...
          failHandler: (entityName: string) =>
            new Error(`Failed: ${entityName}`),
        },
      ),
    ).rejects.toThrow("Failed: Book");
    await expect(
      orm.em.findOneOrFail(
        Book,
        { title: "b4" },
