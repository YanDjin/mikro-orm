...
describe("EntityManagerMongo", () => {
  ...
  test("should load entities", async () => {
    ...
    expect(
      await authorRepository.findOne({ favouriteBook: bible._id }),
    ).not.toBe(null);
    orm.em.clear();
    ...
    const fullTextBooks2 = (await booksRepository.find({
      author: god.id,
      $fulltext: "life wall",
    }))!;
    ...
    await expect(
      booksRepository.find({ title: { $fulltext: "life wall" } }),
    ).rejects.toThrow(
      "Full text search is only supported on the top level of the query object.",
    );
    await expect(
      booksRepository.find({ author: { name: { $fulltext: "god" } } }),
    ).rejects.toThrow(
      "Full text search is only supported on the top level of the query object.",
    );
    ...
    const lastBook = await booksRepository.find(
      { author: jon.id },
      { populate: ["author"], orderBy: { title: -1 }, limit: 2, offset: 2 },
    );
    ...
    const lastBook2 = await booksRepository.find(
      { author: jon.id },
      {
        populate: ["author"],
    ...
    const lastBook3 = await orm.em.find(
      Book,
      { author: jon.id },
      {
        populate: ["author"],
  ...
  test(`entity.init() and collection.init() works only for managed entities`, async () => {
    ...
    await expect(wrap(author).init()).rejects.toThrow(
      "Entity Author is not managed. An entity is managed if its fetched from the database or registered as new through EntityManager.persist()",
    );
    await expect(author.books.init()).rejects.toThrow(
      "Entity Author is not managed. An entity is managed if its fetched from the database or registered as new through EntityManager.persist()",
    );
  });
  ...
  test("removing persisted entity will remove it from persist stack first", async () => {
    ...
    await orm.em.persistAndFlush(author);
    expect(
      orm.em.getUnitOfWork().getById<Author>(Author.name, author.id),
    ).toBeDefined();
  ...
  test("removing entity will remove its FK from relations", async () => {
    ...
    const p1 = await orm.em.findOneOrFail(Publisher, publisher, {
      populate: ["books"],
    });
    orm.em.remove(p1);
    await orm.em.flush();
    ...
    expect(books.map((b) => b.publisher)).toEqual([
      undefined,
      undefined,
      undefined,
    ]);
  ...
  test("removing persisted entity via PK", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('author'\)\.deleteMany\({ _id: ObjectId\('\w+'\) }, {}\)/,
    );
  });

  test("should throw when trying to merge entity without id", async () => {
    const author = new Author("test", "test");
    expect(() => orm.em.merge(author)).toThrow(
      `You cannot merge entity 'Author' without identifier!`,
    );
  });
  ...
  test("fork", async () => {
    ...
    expect(fork.getUnitOfWork().getIdentityMap()).not.toBe(
      orm.em.getUnitOfWork().getIdentityMap(),
    );
  ...
  test("findOne with empty where will throw", async () => {
    await expect(orm.em.findOne(Author, {})).rejects.toThrow(
      `You cannot call 'EntityManager.findOne()' with empty 'where' parameter`,
    );
    await expect(orm.em.findOne(Author, undefined!)).rejects.toThrow(
      `You cannot call 'EntityManager.findOne()' with empty 'where' parameter`,
    );
    await expect(orm.em.findOne(Author, null!)).rejects.toThrow(
      `You cannot call 'EntityManager.findOne()' with empty 'where' parameter`,
    );
  });
  ...
  test("findOne supports regexps", async () => {
    ...
    const authors2 = await orm.em.find(Author, {
  ...
  test("stable results of serialization (collection)", async () => {
    ...
    const publisher = (await orm.em.findOne(Publisher, pub.id, {
      populate: ["books"],
    }))!;
    await wrap(newGod).init();
  ...
  test("should return mongo driver", async () => {
    ...
    expect(
      await driver.findOne(BookTag.name, { foo: "bar", books: 123 }),
    ).toBeNull();
    expect(
    ...
    await expect(
      driver.loadFromPivotTable({} as EntityProperty, []),
    ).rejects.toThrow("MongoDriver does not use pivot tables");
    await expect(driver.getConnection().execute("")).rejects.toThrow(
      "MongoConnection does not support generic execute method",
    );
    await expect(driver.getConnection().execute("")).rejects.toThrow(
      "MongoConnection does not support generic execute method",
    );
    expect(driver.getConnection().getCollection(BookTag).collectionName).toBe(
      "book-tag",
    );
    expect(orm.em.getCollection(BookTag).collectionName).toBe("book-tag");
    expect(orm.em.getRepository(BookTag).getCollection().collectionName).toBe(
      "book-tag",
    );
    ...
    const first = await driver.nativeInsert<Publisher>(Publisher.name, {
      name: "test 123",
      type: PublisherType.GLOBAL,
    });
    await driver.nativeUpdate<Publisher>(Publisher.name, first.insertId, {
      name: "test 456",
    });
    await driver.nativeUpdateMany<Publisher>(
      Publisher.name,
      [first.insertId],
      [{ name: "test 789" }],
    );
  ...
  test("ensure indexes", async () => {
    ...
    const authorInfo = await conn
      .getCollection("author")
      .indexInformation({ full: true, session: undefined as any });
    const bookInfo = await conn
      .getCollection("books-table")
      .indexInformation({ full: true, session: undefined as any });
    expect(
      authorInfo.reduce((o: any, i: any) => {
        o[i.name] = i;
        return o;
      }, {} as any),
    ).toMatchObject({
    ...
    expect(
      ...
      "point_2dsphere_title_-1": {
        key: { point: "2dsphere", title: -1 },
        name: "point_2dsphere_title_-1",
      },
    });
  });
  ...
  test("should use user and password as connection options", async () => {
    const config = new Configuration(
      { driver: MongoDriver, user: "usr", password: "pw" } as any,
      false,
    );
  ...
  test("connection returns correct URL", async () => {
    const conn1 = new MongoConnection(
      new Configuration(
        {
          driver: MongoDriver,
    ...
    await expect(conn1.getClientUrl()).toBe(
      "mongodb://usr:*****@example.host.com:34500",
    );
    const conn2 = new MongoConnection(
      new Configuration({ driver: MongoDriver } as any, false),
    );
    await expect(conn2.getClientUrl()).toBe("mongodb://127.0.0.1:27017");
    const clientUrl =
      "mongodb://user:Q#ais@2d-Aa_43:ui!0d.ai6d@mongodb-replicaset-0.cluster.local:27017,mongodb-replicaset-1.cluster.local:27018,...";
    const conn3 = new MongoConnection(
      new Configuration({ driver: MongoDriver, clientUrl } as any, false),
    );
    await expect(conn3.getClientUrl()).toBe(
      "mongodb://user:*****@mongodb-replicaset-0.cluster.local:27017,mongodb-replicaset-1.cluster.local:27018,...",
    );
    const conn4 = new MongoConnection(
      new Configuration(
        {
          driver: MongoDriver,
          clientUrl: "invalid-url-that-was-not-properly-parsed",
        } as any,
        false,
      ),
    );
    await expect(conn4.getClientUrl()).toBe(
      "invalid-url-that-was-not-properly-parsed",
    );
  });
  ...
  test("json properties", async () => {
    ...
    const g = await orm.em.findOneOrFail(Author, god.id, {
      populate: ["books"],
    });
    ...
    expect(g.books[0].metaObject).toEqual({
      category: "god like",
      items: 3,
      valid: true,
      nested: { foo: "123", bar: 321, deep: { baz: 59, qux: false } },
    ...
    const b1 = await orm.em.findOneOrFail(Book, {
      metaObject: { category: "god like" },
    });
    const b2 = await orm.em.findOneOrFail(Book, {
      metaObject: { category: "god like", items: 3 },
    });
    const b3 = await orm.em.findOneOrFail(Book, {
      metaObject: { nested: { bar: 321 } },
    });
    const b4 = await orm.em.findOneOrFail(Book, {
      metaObject: { nested: { foo: "123", bar: 321 } },
    });
    const b5 = await orm.em.findOneOrFail(Book, {
      metaObject: { valid: true, nested: { foo: "123", bar: 321 } },
    });
    const b6 = await orm.em.findOneOrFail(Book, {
      metaObject: {
        valid: true,
        nested: { foo: "123", bar: 321, deep: { baz: 59 } },
      },
    });
    const b7 = await orm.em.findOneOrFail(Book, {
      metaObject: {
        valid: true,
        nested: { foo: "123", bar: 321, deep: { baz: 59, qux: false } },
      },
  ...
  test("many to many relation", async () => {
    ...
    expect(book1.tags.toArray()).toEqual([
      wrap(tag1).toJSON(),
      wrap(tag3).toJSON(),
    ]);
    expect(book1.tags.toJSON()).toEqual([
      wrap(tag1).toJSON(),
      wrap(tag3).toJSON(),
    ]);
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `db.getCollection('book-tag').insertMany(`,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      `db.getCollection('author').insertMany(`,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      `db.getCollection('books-table').insertMany(`,
    );
    orm.em.clear();
    ...
    expect(() => tags[0].books.getItems()).toThrow(
      /Collection<Book> of entity BookTag\[\w{24}] not initialized/,
    );
    ...
    book.tags.remove(
      ...
      tagRepository.getReference(tag5.id),
    ); // tag5 will be ignored as it is not part of collection
    ...
    const items = book.tags
      .getIdentifiers()
      .map((t) => tagRepository.getReference(t));
    book.tags.set(items);
    await orm.em.persistAndFlush(book);
    orm.em.clear();
    book = (await orm.em.findOne(Book, book._id))!;
  ...
  test("populating many to many relation", async () => {
    ...
    orm.em.clear();
    const publishers4 = await repo.findAll({
      orderBy: { id: 1 },
      populate: false,
    });
  ...
  test("many to many relation (ref: true)", async () => {
    ...
    const bt1 = await orm.em.findOneOrFail(BookTag, tag1.id, {
      populate: ["books:ref"],
    });
  ...
  test("cascade persist on owning side", async () => {
    ...
    let book = (await repo.findOne(book1.id, {
  ...
  test("cascade remove on m:1 reference", async () => {
    ...
    book1.publisher =
      book2.publisher =
      book3.publisher =
        Reference.create(new Publisher("to be removed"));
    author.books.add(book1, book2, book3);
    await orm.em.persistAndFlush(author);
    orm.em.clear();
  ...
  test("nested populating", async () => {
    ...
    expect(wrap(tags[0].books[0].publisher!.unwrap()).isInitialized()).toBe(
      true,
    );
  ...
  test("trying to populate non-existing or non-reference property will throw", async () => {
    ...
    ).rejects.toThrow(`Entity 'Author' does not have property 'tests'`);
    await expect(
  ...
  test("many to many collection does have fixed order", async () => {
    ...
    await expect(ent1.tests.getIdentifiers("id")).toEqual([
      t3.id,
      t2.id,
      t1.id,
    ]);
  ...
  test("collection allows custom populate, where and orderBy", async () => {
    ...
    expect(ent1.tags.getIdentifiers("id")).toEqual([
      tag1.id,
      tag2.id,
      tag3.id,
      tag4.id,
    ...
    expect(ent1.tags.getItems().map((t) => t.name)).toEqual([
      tag4.name,
      tag1.name,
      tag3.name,
      tag5.name,
    ...
    expect(ent2.tags.getIdentifiers("id")).toEqual([
      tag1.id,
      tag2.id,
      tag3.id,
      tag4.id,
    ...
    await ent2.tags.init({
      where: { name: { $ne: "funny" } },
      orderBy: { name: QueryOrder.DESC },
    });
    expect(ent2.tags.getItems().map((t) => t.name)).toEqual([
      tag4.name,
      tag1.name,
      tag3.name,
      tag5.name,
  ...
  test("EM supports native insert/update/delete/aggregate", async () => {
    ...
    const res1 = await orm.em.insert(Author, {
      name: "native name 1",
      email: "e1",
    });
    ...
    const res3 = await orm.em.aggregate(Author, [
      { $match: { name: "new native name" } },
    ]);
    ...
    const res5 = await orm.em.insert(Author, {
    ...
    const res6 = await orm.em.nativeUpdate(
      Author,
      { name: "native name 2" },
    ...
    const res7 = await orm.em.insert<any>("test", {
      name: "native name 1",
      test: "abc",
    });
    expect(res7).toBeInstanceOf(ObjectId);

    const res8 = await orm.em.nativeUpdate<any>(
      "test",
      { name: "native name 1" },
      { $unset: { test: 1 } },
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('author'\)\.insertOne\({ name: 'native name 1', email: 'e1' }, {}\);/,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('author'\)\.updateMany\({ name: 'native name 1' }, { '\$set': { name: 'new native name' } }, {}\);/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      /db\.getCollection\('author'\)\.aggregate\(\[ { '\$match': { name: 'new native name' } } ], {}\)\.toArray\(\);/,
    );
    expect(mock.mock.calls[3][0]).toMatch(
      /db\.getCollection\('author'\)\.deleteMany\({ name: 'new native name' }, {}\)/,
    );
    expect(mock.mock.calls[4][0]).toMatch(
      /db\.getCollection\('author'\)\.insertOne\({ createdAt: ISODate\('.*'\), updatedAt: ISODate\('.*'\), name: 'native name 2', email: 'e2' }, {}\);/,
    );
    expect(mock.mock.calls[5][0]).toMatch(
      /db\.getCollection\('author'\)\.updateMany\({ name: 'native name 2' }, { '\$set': { name: 'new native name', updatedAt: ISODate\('.*'\) } }, {}\);/,
    );
    expect(mock.mock.calls[6][0]).toMatch(
      /db\.getCollection\('test'\)\.insertOne\({ name: 'native name 1', test: 'abc' }, {}\);/,
    );
    expect(mock.mock.calls[7][0]).toMatch(
      /db\.getCollection\('test'\)\.updateMany\({ name: 'native name 1' }, { '\$unset': { test: 1 } }, {}\);/,
    );
    expect(mock.mock.calls[8][0]).toMatch(
      /db\.getCollection\('test'\)\.deleteMany\({ name: 'native name 1' }, {}\)/,
    );
  });
  ...
  test("filter by serialized PK inside group condition", async () => {
    ...
    const a1 = (await orm.em.findOne(Author, {
      $or: [{ id: author.id }],
    } as any))!;
    expect(a1).not.toBeNull();
    expect(a1.id).toBe(author.id);

    const a2 = (await orm.em.findOne(Author, {
      $or: [{ id: { $in: [author.id] } }],
    } as any))!;
    expect(a2).not.toBeNull();
    expect(a2.id).toBe(author.id);

    const a3 = (await orm.em.findOne(Author, {
      $and: [{ id: { $in: [author.id] } }],
    } as any))!;
    expect(a3).not.toBeNull();
    expect(a3.id).toBe(author.id);

    const a4 = await orm.em.findOneOrFail(Author, {
      id: { $not: { $gt: author.id } },
    });
  ...
  test("self referencing (1 step)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('author'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), updatedAt: ISODate\('.*'\), foo: '.*', name: '.*', email: '.*', termsAccepted: .* } ], {}\);/,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('books-table'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), title: 'b1', author: ObjectId\('.*'\) }, { createdAt: ISODate\('.*'\), title: 'b2', author: ObjectId\('.*'\) }, { createdAt: ISODate\('.*'\), title: 'b3', author: ObjectId\('.*'\) } ], {}\);/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      /db\.getCollection\('author'\)\.updateMany\({ _id: ObjectId\('.*'\) }, { '\$set': { favouriteAuthor: ObjectId\('.*'\), updatedAt: ISODate\('.*'\) } }, {}\);/,
    );
    expect(mock.mock.calls[3][0]).toMatch(
      /db\.getCollection\('author'\)\.find\(.*\)\.toArray\(\);/,
    );
  });
  ...
  test("self referencing M:N (1 step)", async () => {
    ...
    expect(jon!.friends.toArray()).toMatchObject(
      authors.map((a) => a.toJSON(true, ["id", "email", "friends"])),
    );
  });
  ...
  test("orphan removal 1:1", async () => {
    ...
    bar.baz = baz1;
    await orm.em.persistAndFlush(bar);
    expect(orm.em.getUnitOfWork().getOriginalEntityData(bar)!.baz).toEqual(
      baz1._id,
    );
  ...
  test("loading connected entity will not update identity map for associations", async () => {
    ...
    const a = (await orm.em.findOne(Author, author, {
      populate: ["favouriteBook"],
    }))!;
    expect(a).not.toBe(author);
    a.name = "test 1";
    a.favouriteBook!.title = "test 2";
    expect(wrap(a, true).__originalEntityData).toMatchObject({
      name: "Jon Snow",
    });
    expect(wrap(a.favouriteBook!, true).__originalEntityData).toMatchObject({
      title: "b1",
    });
    const a1 = await orm.em.findOneOrFail(Author, {
  ...
  test("automatically fix array of PKs instead of collection when flushing (m:n)", async () => {
    ...
    await expect(orm.em.flush()).rejects.toThrow(
      `Entity of type BookTag expected for property Book.tags, '0000007b5c9c61c332380f78' of type string given. If you are using Object.assign(entity, data), use em.assign(entity, data) instead.`,
    );

    wrap(book).assign(
      { tags: ["0000007b5c9c61c332380f78", tag] },
      { em: orm.em },
    );
    expect(book.tags).toBeInstanceOf(Collection);
    ...
    await orm.em.flush();
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('author'\)\.insertMany\(\[ { createdAt: ISODate\(.*\), updatedAt: ISODate\(.*\), foo: 'bar', name: 'Jon Snow', email: 'snow@wall\.st', termsAccepted: false } ], {}\);/,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('books-table'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), title: 'B123', author: ObjectId\('.*'\) } ], {}\);/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      /db\.getCollection\('books-table'\)\.updateMany\({ _id: ObjectId\('.*'\) }, { '\$set': { tags: \[ ObjectId\('0000007b5c9c61c332380f78'\), ObjectId\('0000007b5c9c61c332380f79'\) ] } }, {}\);/,
    );
  });
  ...
  test("automatically fix PK in collection instead of entity when flushing (m:n)", async () => {
    const author = new Author("Jon Snow", "snow@wall.st");
    const book = new Book("B123", author);
    expect(() => book.tags.set(["0000007b5c9c61c332380f78" as any])).toThrow(
      `Entity of type BookTag expected for property Book.tags, '0000007b5c9c61c332380f78' of type string given.`,
    );
  });
  ...
  test("automatically map raw results to entities when setting collection items", async () => {
    ...
    expect(() => god.books.add(bookData as any)).toThrow(
      `Entity of type Book expected for property Author.books, { title: 'Bible', author: undefined } of type object given.`,
    );
  });
  ...
  test("reference wrapper", async () => {
    ...
    const ref = orm.em.getReference<Author>(Author, author.id, {
      wrapped: true,
    });
    const ref1 = orm.em
      .getRepository(Author)
      .getReference(author.id, { wrapped: true });
    ...
    expect(() => ref.getEntity()).toThrow(
      `Reference<Author> ${ref.id} not initialized`,
    );
    expect(() => ref.getProperty("email")).toThrow(
      `Reference<Author> ${ref.id} not initialized`,
    );
  ...
  test("find and count", async () => {
    ...
    const [authors2, count2] = await orm.em.findAndCount(
      Author,
      {},
      { limit: 10, offset: 25, fields: ["name"] },
    );
  ...
  test("query highlighting", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /\[90m\[query] \[39mdb\[0m.\[0mgetCollection\(\[33m'author'\[39m\)\[0m.\[0minsertMany\(\[ \{ createdAt: ISODate\('.*'\), updatedAt: ISODate\(.*\), foo: 'bar', name: 'Jon Snow', email: 'snow@wall.st', age: 30, termsAccepted: false } ], \{}\);\[90m \[took \d+ ms]\[39m/,
    );
  ...
  test("findOneOrFail", async () => {
    ...
    expect(a1).not.toBeNull();
    await expect(
      orm.em.findOneOrFail(Author, "0000007b5c9c61c332380f79"),
    ).rejects.toThrow(`Author not found ('0000007b5c9c61c332380f79')`);
    await expect(
      orm.em.findOneOrFail(Author, { name: "test" }),
    ).rejects.toThrow("Author not found ({ name: 'test' })");
    await expect(
      orm.em.findOneOrFail(Author, "0000007b5c9c61c332380f79", {
        failHandler: () => new Error("Test"),
      }),
  ...
  test("many to many working with inverse side", async () => {
    ...
    let tag = await orm.em.findOneOrFail(BookTag, tag1.id, {
      populate: ["books"],
    });
    const err =
      "You cannot modify inverse side of M:N collection BookTag.books when the owning side is not initialized. Consider working with the owning side instead (Book.tags).";
    expect(() => tag.books.add(orm.em.getReference(Book, book4.id))).toThrow(
      err,
    );
    orm.em.clear();
  ...
  test("many to many working with inverse side persistence", async () => {
    ...
    const book2 = new Book("My Life on The Wall, part 2", author);
    const book3 = new Book("My Life on The Wall, part 3", author);
    const book4 = new Book("Another Book", author);
    const tag1 = new BookTag("silly");
    const tag2 = new BookTag("funny");
    const tag3 = new BookTag("sick");
    const tag4 = new BookTag("strange");
    ...
    const tag = await orm.em.findOneOrFail(BookTag, tag1.id);
    tag.books.removeAll();
    await expect(orm.em.flush()).rejects.toThrow(
      "You cannot modify inverse side of M:N collection BookTag.books when the owning side is not initialized. Consider working with the owning side instead (Book.tags)",
    );
  ...
  test("adding items to not initialized collection", async () => {
    ...
    const a2 = await orm.em.findOneOrFail(Author, god.id, {
      populate: ["books"],
    });
    ...
    const a3 = await orm.em.findOneOrFail(Author, god.id, {
      populate: ["books"],
    });
    ...
    const a4 = await orm.em.findOneOrFail(Author, god.id, {
      populate: ["books.tags"],
    });
  ...
  test("exceptions", async () => {
    ...
    await expect(
      driver.nativeInsert(Author.name, { name: "author", email: "email" }),
  ...
  test("validation for `host` option", async () => {
    await expect(
      ...
      "Mongo driver does not support `host` options, use `clientUrl` instead!",
    );
  });

  test("validation for `_id` PK field name", async () => {
    const schema = new EntitySchema({
      name: "WrongPrimaryKeyEntity",
    ...
    await expect(
      ...
      `WrongPrimaryKeyEntity.id has wrong field name, '_id' is required in current driver`,
    );
  });
  ...
  test("extracting child condition when populating (GH #1891)", async () => {
    ...
    const res1 = await orm.em.findOne(
      Author,
      {
        $and: [{ id: author.id }],
      },
    ...
    const res2 = await orm.em.findOne(
      Author,
      {
        $and: [
          {
            $or: [
              { favouriteBook: book3.id },
              { books: { tags: author.books[1].tags.getIdentifiers("id") } },
            ],
          },
        ],
      },
