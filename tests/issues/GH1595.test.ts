...
describe("GH issue 1595", () => {
  ...
  test("mapping PKs from batch insert with custom field name", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      'insert into "a" ("NAME") values ($1), ($2), ($3), ($4), ($5) returning "ID"',
    );
    ...
    expect(mock.mock.calls[4][0]).toMatch(
      'update "a" set "NAME" = case when ("ID" = $1) then $2 when ("ID" = $3) then $4 when ("ID" = $5) then $6 when ("ID" = $7) then $8 when ("ID" = $9) then $10 else "NAME" end where "ID" in ($11, $12, $13, $14, $15)',
    );
    ...
    expect(mock.mock.calls[7][0]).toMatch(
      'delete from "a" where "ID" in ($1, $2, $3, $4, $5)',
    );
