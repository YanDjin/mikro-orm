...
describe("GH issue 1882", () => {
  ...
  test(`GH issue 1882-1`, async () => {
    ...
    await orm.em
      .fork()
      .find(Foo, cond, { populate: ["barItems"], strategy: "select-in" });
    expect(mock.mock.calls[0][0]).toMatch(
      "select `f0`.* from `foo` as `f0` left join `bar` as `b1` on `f0`.`id` = `b1`.`foo_id` where (`b1`.`id` = ? or `f0`.`name` = ?)",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      "select `b0`.* from `bar` as `b0` where `b0`.`foo_id` in (?)",
    );
  });
  ...
  test(`GH issue 1882-2`, async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `f0`.* from `foo` as `f0` left join `bar` as `b1` on `f0`.`id` = `b1`.`foo_id` where `b1`.`id` = ?",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      "select `b0`.* from `bar` as `b0` where `b0`.`foo_id` in (?) and `b0`.`id` = ?",
    );
  ...
  test(`GH issue 1882-3`, async () => {
    ...
    await orm.em
      .fork()
      .find(Foo, cond, { populate: ["barItems"], strategy: "joined" });
    expect(mock.mock.calls[0][0]).toMatch(
      "select `f0`.*, " +
        "`b1`.`id` as `b1__id`, `b1`.`foo_id` as `b1__foo_id`, `b1`.`name` as `b1__name` " +
        "from `foo` as `f0` " +
