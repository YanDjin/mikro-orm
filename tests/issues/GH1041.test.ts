...
describe("GH issue 1041, 1043", () => {
  ...
  test("select-in strategy: populate with apps and remove one app", async () => {
    ...
    expect(log.mock.calls[1][0]).toMatch(
      "delete from `user_apps` where (`app_id` = 2 and `user_id` = 123)",
    );
  ...
  test("joined strategy: populate with apps and remove one app", async () => {
    ...
    expect(log.mock.calls[0][0]).toMatch("begin");
    expect(log.mock.calls[1][0]).toMatch(
      "delete from `user_apps` where (`app_id` = 3 and `user_id` = 123)",
    );
    expect(log.mock.calls[2][0]).toMatch("commit");
  });
  ...
  test("select-in strategy: find by many-to-many relation ID", async () => {
    ...
    expect(log.mock.calls[1][0]).toMatch(
      "select `a1`.*, `u0`.`app_id` as `fk__app_id`, `u0`.`user_id` as `fk__user_id` from `user_apps` as `u0` inner join `app` as `a1` on `u0`.`app_id` = `a1`.`id` where `u0`.`user_id` in (123)",
    );
  });
  ...
  test("select-in strategy: find by many-to-many relation IDs (PopulateHint.INFER)", async () => {
    ...
    expect(log.mock.calls[1][0]).toMatch(
      "select `a1`.*, `u0`.`app_id` as `fk__app_id`, `u0`.`user_id` as `fk__user_id` from `user_apps` as `u0` inner join `app` as `a1` on `u0`.`app_id` = `a1`.`id` where `a1`.`id` in (1, 2, 3) and `u0`.`user_id` in (123)",
    );
  });
