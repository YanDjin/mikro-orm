...
test("validation", async () => {
  await expect(orm.em.upsert(MyEntity1, {})).rejects.toThrow(
    "Unique property value required for upsert, provide one of: id",
  );
  await expect(orm.em.upsertMany(MyEntity1, [{}])).rejects.toThrow(
    "Unique property value required for upsert, provide one of: id",
  );
});
