import { DateType } from "@yandjin-mikro-orm/core";
...
describe("DateType", () => {
  ...
  test("convertToDatabaseValue", () => {
    expect(type.convertToDatabaseValue("2000-01-01", platform)).toBe(
      "2000-01-01",
    );
  ...
  test("getColumnType", () => {
    expect(type.getColumnType({ columnType: "asd" } as any, platform)).toBe(
      "date",
    );
  });
});
