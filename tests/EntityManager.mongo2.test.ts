...
describe("EntityManagerMongo2", () => {
  ...
  test("loaded references and collections", async () => {
    ...
    // type safe `populate: ['*']`
    expect(book1.tags.$[0].books.$[0].tags.$[0].books.$[0].tags.$[0].name).toBe(
      "t1",
    );
    orm.em.clear();
    ...
    await expect(() =>
      orm.em.findOneOrFail(Book, bible, { populate: "populate" }),
    ).rejects.toThrow("Entity 'Book' does not have property 'populate'");
    ...
    expect(books[0].publisher!.get().books.get()[0].publisher!.$.name).toBe(
      "Publisher 123",
    );
    expect(
      // @ts-expect-error
      books[0].publisher!.$.type,
    ).toBeUndefined();
  ...
  test("required fields validation", async () => {
    ...
    await expect(orm.em.persistAndFlush(jon)).rejects.toThrow(
      `Value for Author.email is required, 'undefined' found`,
    );
  });
  ...
  test("partial loading of collections", async () => {
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      for (let j = 1; j <= 15; j++) {
        const tag1 = orm.em.create(BookTag, {
    ...
    expect(() =>
      books[0].tags.add(orm.em.create(BookTag, { name: "new" })),
    ).toThrow(
  ...
  test("loadCount to get the number of entries without initializing the collection (GH issue #949)", async () => {
    let author = orm.em.create(Author, {
      name: "Jon Doe",
    ...
    author.books.add(
      laterRemoved,
      orm.em.create(Book, { title: "bo3", author }),
    );
    ...
    await expect(taggedBook.tags.loadCount(true)).resolves.toEqual(
      tags.length - 1,
    ); // with mongo m:n owners this works based on the collection state
    await orm.em.flush();
    orm.em.clear();
  ...
  test("loadCount with unidirectional m:n (GH issue #1608)", async () => {
    ...
    ent = await orm.em.findOneOrFail(Publisher, publisher.id, {
