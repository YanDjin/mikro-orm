...
describe("GH issue 1721", () => {
  ...
  test(`GH issue 1721`, async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `couch` (`id`, `user_id`, `name`) values ('aaaaaaaa-c65f-42b8-408a-034a6948448f', 'bbbbbbbb-c65f-42b8-408a-034a6948448f', 'n1')",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
    expect(mock.mock.calls[3][0]).toMatch(
      "select `c0`.* from `couch` as `c0` where `c0`.`id` = 'aaaaaaaa-c65f-42b8-408a-034a6948448f' limit 1",
    );
    expect(mock.mock.calls[4][0]).toMatch("begin");
    expect(mock.mock.calls[5][0]).toMatch(
      "update `couch` set `name` = 'n2' where `id` = 'aaaaaaaa-c65f-42b8-408a-034a6948448f'",
    );
