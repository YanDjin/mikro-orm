...
async function createEntities() {
  ...
  const assignee1 = orm.em.create(Assignee, {
    email: "slot1@user.com",
    slot: slot1,
  });
  const assignee2 = orm.em.create(Assignee, {
    email: "slot2@user.com",
    slot: slot2,
  });
...
test("reschedule registration from slot1 to slot2 (lazy loading assignees)", async () => {
  ...
  const slot2 = await orm.em.findOneOrFail(
    Slot,
    { name: "slot2" },
...
test("reschedule registration from slot1 to slot2 (eager loading assignees)", async () => {
  ...
  const slot2 = await orm.em.findOneOrFail(
    Slot,
    { name: "slot2" },
    { populate: ["registrations", "assignees"] },
  );
  ...
  slot1.assignees
    .getItems()
    .forEach((assignee) => expect(assignee.slot.id).toEqual(slot1.id));
  slot2.assignees
    .getItems()
    .forEach((assignee) => expect(assignee.slot.id).toEqual(slot2.id));
});
