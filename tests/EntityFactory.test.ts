...
import type { MikroORM, EntityFactory } from "@yandjin-mikro-orm/core";
import { Collection, ReferenceKind, wrap } from "@yandjin-mikro-orm/core";
...
describe("EntityFactory", () => {
  ...
  test("should load entities", async () => {
    ...
    expect(metadata[Author.name].properties.books.kind).toBe(
    ...
    expect(metadata[Book.name].properties.author.kind).toBe(
  ...
  test("create should flag collections as dirty for new entities", async () => {
    ...
    const b1 = factory.create(
      Book,
      { title: "b1", author: a, tags: [t1, t2, t3] },
      { newEntity: false },
    );
    expect(b1.tags.isDirty()).toBe(false);
    const b2 = factory.create(
      Book,
      { title: "b2", author: a, tags: [t1, t2, t3] },
      { newEntity: true },
    );
  ...
  test("create entity from nested object", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('book-tag'\)\.insertMany\(\[ { name: 't1' } ], {}\);/,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('author'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), updatedAt: ISODate\('.*'\), foo: 'bar', name: 'Jon', email: 'jon@snow\.com', termsAccepted: false } ], {}\);/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      /db\.getCollection\('books-table'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), title: 'B1', author: ObjectId\('.*'\), publisher: ObjectId\('5b0d19b28b21c648c2c8a600'\), tags: \[ ObjectId\('.*'\), ObjectId\('5b0d19b28b21c648c2c8a601'\) ] } ], {}\);/,
    );
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('author'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), updatedAt: ISODate\('.*'\), foo: 'bar', name: 'Jon', email: 'jon2@snow\.com', termsAccepted: false } ], {}\);/,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      /db\.getCollection\('books-table'\)\.insertMany\(\[ { createdAt: ISODate\('.*'\), title: 'B1', author: ObjectId\('.*'\), publisher: ObjectId\('5b0d19b28b21c648c2c8a600'\), tags: \[ ObjectId\('.*'\), ObjectId\('5b0d19b28b21c648c2c8a601'\) ] } ], {}\);/,
    );
