...
const getDropSchemaSQL = jest.spyOn(
...
describe("DropSchemaCommand", () => {
  ...
  test("handler", async () => {
    ...
    await expect(
      cmd.handler({ run: true, dropMigrationsTable: true } as any),
    ...
    await expect(
      cmd.handler({ dump: true, dropMigrationsTable: true } as any),
    ).resolves.toBeUndefined();
    ...
    await expect(
      cmd.handler({ run: true, dropDb: true } as any),
    ).resolves.toBeUndefined();
