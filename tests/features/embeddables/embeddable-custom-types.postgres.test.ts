import type { EntityProperty, Platform } from "@yandjin-mikro-orm/core";
import {
  Embeddable,
  Embedded,
  Entity,
...
describe("embedded entities with custom types", () => {
  ...
  test("persist and load", async () => {
    ...
    expect(mock.mock.calls[3][0]).toMatch(
      'select "p0".* from "parent" as "p0" where "p0"."id" = 1 limit 1',
    );
  ...
  test("snapshot generator", async () => {
    const snapshotGenerator = orm.em
      .getComparator()
      .getSnapshotGenerator("Parent");
  ...
  test("persist and load 2", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      'select "u0".* from "user" as "u0" where "u0"."savings_amount" = $1 limit $2',
    );
