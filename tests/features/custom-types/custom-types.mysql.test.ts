...
describe("custom types [mysql]", () => {
  ...
  test("advanced custom types", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `location` (`point`, `extended_point`) values (ST_PointFromText('point(1.23 4.56)'), ST_PointFromText('point(5.23 9.56)'))",
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "insert into `address` (`location_id`) values (1)",
    );
    expect(mock.mock.calls[3][0]).toMatch("commit");
    expect(mock.mock.calls[4][0]).toMatch(
      "select `l0`.*, ST_AsText(`l0`.`point`) as `point`, ST_AsText(`l0`.`extended_point`) as `extended_point` from `location` as `l0` where `l0`.`id` = 1 limit 1",
    );
    ...
    expect(mock.mock.calls[6][0]).toMatch(
      "update `location` set `point` = ST_PointFromText('point(2.34 9.87)') where `id` = 1",
    );
    ...
    expect(mock.mock.calls[8][0]).toMatch(
      "select `l`.*, ST_AsText(`l`.`point`) as `point`, ST_AsText(`l`.`extended_point`) as `extended_point` from `location` as `l` where `l`.`id` = 1",
    );
    ...
    expect(mock.mock.calls[9][0]).toMatch(
      "select `l0`.*, ST_AsText(`l0`.`point`) as `point`, ST_AsText(`l0`.`extended_point`) as `extended_point` from `location` as `l0` where `l0`.`id` = 1",
    );
    ...
    const a2 = await orm.em.findOneOrFail(Address, addr, {
      populate: ["location"],
      strategy: LoadStrategy.JOINED,
    });
    expect(mock.mock.calls[0][0]).toMatch(
      "select `a0`.*, `l1`.`id` as `l1__id`, `l1`.`rank` as `l1__rank`, ST_AsText(`l1`.`point`) as `l1__point`, ST_AsText(`l1`.`extended_point`) as `l1__extended_point` from `address` as `a0` left join `location` as `l1` on `a0`.`location_id` = `l1`.`id` where `a0`.`id` = 1",
    );
    expect(a2.location.point).toBeInstanceOf(Point);
    expect(a2.location.point).toMatchObject({
      latitude: 2.34,
      longitude: 9.87,
    });
    expect(a2.location.extendedPoint).toBeInstanceOf(Point);
    expect(a2.location.extendedPoint).toMatchObject({
      latitude: 5.23,
      longitude: 9.56,
    });
  ...
  test("extending custom types (gh issue 1442)", async () => {
    ...
    expect(meta.properties.extendedPoint.customType).toBeInstanceOf(
      ExtendedPointType,
    );
  });
  ...
  test("partial loading with custom types (gh issue 4622)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `location` (`point`, `extended_point`) values (ST_PointFromText(?), ST_PointFromText(?))",
    );
    expect(mock.mock.calls[2][0]).toMatch(
      "insert into `address` (`location_id`) values (?)",
    );
    expect(mock.mock.calls[3][0]).toMatch("commit");
    expect(mock.mock.calls[4][0]).toMatch(
      "select `l0`.`id`, ST_AsText(`l0`.`point`) as `point`, ST_AsText(`l0`.`extended_point`) as `extended_point` from `location` as `l0` where `l0`.`id` = ? limit ?",
    );
    expect(mock.mock.calls).toHaveLength(5);
  });
  ...
  test("create and update many records with custom types (gh issue 1625)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `location` (`point`) values (ST_PointFromText('point(-1.23 -4.56)')), (ST_PointFromText('point(-7.89 -0.12)'))",
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
    expect(mock.mock.calls[3][0]).toMatch("begin");
    expect(mock.mock.calls[4][0]).toMatch(
      `update \`location\` set \`point\` = case when (\`id\` = ${locations[0].id}) then ST_PointFromText('point(1.23 4.56)') when (\`id\` = ${locations[1].id}) then ST_PointFromText('point(7.89 0.12)') else \`point\` end where \`id\` in (${locations[0].id}, ${locations[1].id})`,
    );
  ...
  test("find entity by custom types (gh issue 1630)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `l0`.*, ST_AsText(`l0`.`point`) as `point`, ST_AsText(`l0`.`extended_point`) as `extended_point` from `location` as `l0` where ST_AsText(`l0`.`point`) = 'point(1 1)' and ST_AsText(`l0`.`extended_point`) is null limit 1",
    );
    expect(mock.mock.calls).toHaveLength(1);
  ...
  test("find entity by custom types with object subconditions", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `l0`.*, ST_AsText(`l0`.`point`) as `point`, ST_AsText(`l0`.`extended_point`) as `extended_point` from `location` as `l0` where ST_AsText(`l0`.`point`) = 'point(1 1)' and ST_AsText(`l0`.`extended_point`) is not null limit 1",
    );
    expect(mock.mock.calls).toHaveLength(1);
