...
describe("TsMorphMetadataProvider", () => {
  ...
  test("should load entities", async () => {
    ...
    expect(metadata[Book.name].properties.metaArrayOfStrings.type).toBe(
      "string[]",
    );
    ...
    expect(metadata[Publisher.name].properties.types.customType).toBeInstanceOf(
      EnumArrayType,
    );
    expect(
      metadata[Publisher.name].properties.types2.customType,
    ).toBeInstanceOf(EnumArrayType);
  ...
  test("should ignore entity without path", async () => {
    ...
    const initProperties = jest.spyOn(
      TsMorphMetadataProvider.prototype,
      "initProperties" as any,
    );
  ...
  test("should throw when source file not found", async () => {
    ...
    expect(() => provider.getExistingSourceFile("./path/to/entity.js")).toThrow(
      error,
    );
  });
});
