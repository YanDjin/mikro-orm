...
describe.each(Utils.keys(options))("em.upsert [%s]", (type) => {
  ...
  async function assertFooBars(fooBars: FooBar[], mock: jest.Mock) {
    ...
    orm.em.clear();
    const fooBarsReloaded = await orm.em.find(
      FooBar,
      {},
      { orderBy: { name: "asc" } },
    );
  ...
  test("em.upsert(Type, data) with PK", async () => {
    ...
    expect(
      Subscriber.log.map((l) => [l[0], l[1].entity.constructor.name]),
    ).toEqual([
      ["beforeUpsert", "Object"],
      ["onInit", "Author"],
      ["afterUpsert", "Author"],
  ...
  test("em.upsert(entity) with embeddable", async () => {
    const testEntity = orm.em.create(FooBarWithEmbeddable, {
      fooBarEmbeddable: {},
    });
  ...
  test("em.upsert(Type, entity, options) with advanced options", async () => {
    ...
    const mock = mockLogger(orm);
    const a1 = orm.em.create(Author, {
      id: 1,
      email: "a1",
      age: 41,
      foo: true,
    });
    const a2 = orm.em.create(Author, {
      id: 2,
      email: "a2",
      age: 42,
      foo: true,
    });
    const a3 = orm.em.create(Author, {
      id: 5,
      email: "a3",
      age: 43,
      foo: true,
  ...
  test("em.upsertMany(Type, [entity], options) with advanced options (ignore)", async () => {
    ...
    const [author1, author2, author3] = await orm.em.upsertMany(
      Author,
      [a1, a2, a3],
      {
        onConflictFields: ["email"], // specify a manual set of fields pass to the on conflict clause
