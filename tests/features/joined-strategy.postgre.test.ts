...
describe("Joined loading strategy", () => {
  ...
  test("populate OneToMany with joined strategy [findOne()]", async () => {
    ...
    await expect(
      orm.em.findOneOrFail(Author2, author, {
        populate: ["books123", "following"],
      }),
    ).rejects.toThrow(`Entity 'Author2' does not have property 'books123'`);
    // @ts-expect-error test runtime validation
    await expect(
      orm.em.findOneOrFail(Author2, author, {
        populate: ["books2", "followinga"],
      }),
    ).rejects.toThrow(`Entity 'Author2' does not have property 'followinga'`);

    const a2 = await orm.em.findOneOrFail(Author2, author, {
      populate: ["books2", "following"],
    });
    expect(a2.books2).toHaveLength(2);
    expect(a2.books2[0].title).toBe("The Fall");
    expect(a2.books2[0].price).toBe(200.0);
    expect(a2.books2[0].priceTaxed).toBe("238.0000");
    expect(a2.books2[1].title).toBe("The Stranger");
    expect(a2.books2[1].price).toBe(100.0);
  ...
  test("populate OneToMany with joined strategy [find()]", async () => {
    ...
    a1.books2.add(
      new Book2("The Stranger 1", a1, 100),
      new Book2("The Fall 1", a1, 200),
    );
    const a2 = new Author2("Albert Camus 2", "albert.camus2@email.com");
    a2.books2.add(
      new Book2("The Stranger 2", a2, 300),
      new Book2("The Fall 2", a2, 400),
    );
    const a3 = new Author2("Albert Camus 3", "albert.camus3@email.com");
    a3.books2.add(
      new Book2("The Stranger 3", a3, 500),
      new Book2("The Fall 3", a3, 600),
    );
    await orm.em.persistAndFlush([a1, a2, a3]);
    orm.em.clear();

    const ret = await orm.em.find(
      Author2,
      {},
      { populate: ["books2", "following"], orderBy: { email: "asc" } },
    );
  ...
  test("populate ManyToOne with joined strategy [findOne()]", async () => {
    ...
    const b1 = await orm.em.findOneOrFail(Book2, stranger, {
      populate: ["author"],
    });
  ...
  test("should only fire one query [find()]", async () => {
    ...
    // @ts-expect-error test runtime validation
    await expect(
      orm.em.find(Author2, {}, { populate: ["books123", "following"] }),
    ).rejects.toThrow(`Entity 'Author2' does not have property 'books123'`);
    // @ts-expect-error test runtime validation
    await expect(
      orm.em.find(Author2, {}, { populate: ["books2", "followinga"] }),
    ).rejects.toThrow(`Entity 'Author2' does not have property 'followinga'`);
    ...
    await orm.em.find(
      Author2,
      { id: author2.id },
      { populate: ["books2.perex"], filters: false },
    );
    ...
    await orm.em.find(
      Author2,
      { id: author2.id },
      { populate: ["books"], strategy: LoadStrategy.JOINED },
    );
    expect(mock.mock.calls.length).toBe(1);
    expect(mock.mock.calls[0][0]).toMatch(
      'select "a0".*, ' +
  ...
  test("populate ManyToMany with joined strategy", async () => {
    ...
    expect(books[0].tags.getItems().map((t) => t.name)).toEqual([
      "strange",
      "sexy",
      "funny",
    ]);
    expect(books[1].tags.getItems().map((t) => t.name)).toEqual([
      "silly",
      "sick",
    ]);
    expect(books[2].tags.getItems().map((t) => t.name)).toEqual([
      "silly",
      "sexy",
      "funny",
    ]);
  ...
  test("can populate all related entities", async () => {
    ...
    const a2 = await orm.em.findOneOrFail(
      Author2,
      { id: author2.id },
      { populate: ["*"], strategy: LoadStrategy.SELECT_IN },
    );
    expect(a2.books2).toHaveLength(2);
  ...
  test("populate OneToOne relation on inverse side", async () => {
    ...
    const b1 = (await orm.em.findOne(
      FooBaz2,
      { id: baz.id },
      { populate: ["bar"], strategy: LoadStrategy.JOINED },
    ))!;
    ...
    const b3 = await orm.em.findOneOrFail(
      FooBaz2,
      { bar: bar.id },
      { populate: ["bar.lazyRandom"], populateWhere: "infer" },
    );
    ...
    expect(wrap(b3).toJSON()).toMatchObject({
      bar: { id: bar.id, baz: baz.id, name: "bar" },
    });
