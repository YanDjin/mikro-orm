...
export class MigrationCommandFactory {
  ...
  static create<U extends Opts = Opts>(
    ...
    return {
      ...
      builder: (args: Argv) =>
        MigrationCommandFactory.configureMigrationCommand(
          args,
          command,
        ) as Argv<U>,
      handler: (args: ArgumentsCamelCase<U>) =>
        MigrationCommandFactory.handleMigrationCommand(args, command),
    };
  }
  ...
  static async handleMigrationCommand(
    args: ArgumentsCamelCase<Opts>,
    method: MigratorMethod,
  ): Promise<void> {
  ...
  private static async handleListCommand(migrator: IMigrator) {
    ...
    CLIHelper.dumpTable({
      ...
      rows: executed.map((row) => {
        ...
        const executedAt =
          (row.executed_at ?? (row as Dictionary).created_at)?.toISOString() ??
          "";
  ...
  private static async handleCreateCommand(
    ...
    if (ret.diff.up.length === 0) {
      return CLIHelper.dump(
        colors.green(`No changes required, schema is up-to-date`),
      );
    }

    if (args.dump) {
      CLIHelper.dump(
        colors.green("Creating migration with following queries:"),
      );
      ...
      if (config.getDriver().getPlatform().supportsDownMigrations()) {
        ...
        CLIHelper.dump(
          ret.diff.down.map((sql) => "  " + sql).join("\n"),
          config,
        );
      } else {
        CLIHelper.dump(
          colors.yellow(
            `(${config.getDriver().constructor.name} does not support automatic down migrations)`,
          ),
        );
      }
    }
  ...
  private static async handleCheckCommand(
    migrator: IMigrator,
    orm: MikroORM,
  ): Promise<void> {
    ...
    CLIHelper.dump(
      colors.yellow(
        `Changes detected. Please create migration to update schema.`,
      ),
    );
    process.exit(1);
  }
  ...
  private static async handleFreshCommand(
    ...
    if (args.seed !== undefined) {
      ...
      CLIHelper.dump(
        colors.green(
          `Database seeded successfully with seeder class ${seederClass}`,
        ),
      );
    }
  }
