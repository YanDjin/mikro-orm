...
import { wrap, LoadStrategy, serialize } from "@yandjin-mikro-orm/core";
...
describe("result cache (postgres)", () => {
  ...
  test("result caching (find)", async () => {
    ...
    expect(res1.map((e) => wrap(e).toObject())).toEqual(
      res2.map((e) => wrap(e).toObject()),
    );
    orm.em.clear();
