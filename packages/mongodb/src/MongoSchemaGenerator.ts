...
export class MongoSchemaGenerator extends AbstractSchemaGenerator<MongoDriver> {
  static register(orm: MikroORM): void {
    orm.config.registerExtension(
      ...
      () => new MongoSchemaGenerator(orm.em),
    );
  }
        ...
        this.connection.createCollection(meta.collection).catch((err) => {
          ...
          if (
            !(
              ...
              err.message.includes(existsErrorMessage)
            )
          ) {
            throw err;
          }
  ...
  async dropIndexes(options?: {
    ...
    for (const collection of collections) {
      if (
        options?.collectionsWithFailedIndexes &&
        !options.collectionsWithFailedIndexes.includes(collection.name)
      ) {
        continue;
      }
      ...
      for (const index of indexes) {
        const isIdIndex =
          index.key._id === 1 && Utils.getObjectKeysSize(index.key) === 1;

        /* istanbul ignore next */
        if (
          !isIdIndex &&
          !options?.skipIndexes?.find(
            (idx) =>
  ...
  override async ensureIndexes(
    ...
    for (const meta of this.getOrderedMetadata()) {
      if (
        Array.isArray(options?.retry) &&
        !options.retry.includes(meta.collection)
      ) {
        continue;
      }
    ...
    if (res.some((r) => r.status === "rejected") && options.retry !== false) {
      ...
      if (options.retryLimit === 0) {
        throw new Error(
          `Failed to create indexes: ${collectionsWithFailedIndexes.join(", ")}`,
        );
      }
  ...
  private createIndexes(meta: EntityMetadata): [string, Promise<string>][] {
    ...
    meta.indexes.forEach((index) => {
      ...
      if (
        Array.isArray(index.options) &&
        index.options.length === 2 &&
        properties.length === 0
      ) {
        res.push([
          collection.collectionName,
          collection.createIndex(index.options[0], index.options[1]),
        ]);
        return;
      }
