import type { EntityName } from "@yandjin-mikro-orm/core";
import {
  ArrayCollection,
  Collection,
  EntityManager,
...
describe.each(["sqlite", "better-sqlite"] as const)(
  "EntityManager (%s)",
  (driver) => {
    let orm: MikroORM;
    ...
    test("raw query with array param", async () => {
      const q1 = await orm.em
        .getPlatform()
        .formatQuery(`select * from author4 where id in (?) limit ?`, [
          [1, 2, 3],
          3,
        ]);
      ...
      expect(q2).toBe(
        `select * from author4 where id in ('1', '2', '3') limit 3`,
      );
    });

    test("transactions", async () => {
      const god1 = orm.em.create(Author4, {
        name: "God1",
        email: "hello@heaven1.god",
      });
      ...
      const ret = await orm.em.transactional(async (em) => {
        const god2 = orm.em.create(Author4, {
          name: "God2",
          email: "hello@heaven2.god",
        });
      ...
      try {
        await orm.em.transactional(async (em) => {
          const god3 = orm.em.create(Author4, {
            name: "God4",
            email: "hello@heaven4.god",
          });
    ...
    test("transactions respect the tx context", async () => {
      const god1 = await orm.em.insert(Author4, {
        name: "God1",
        email: "hello@heaven1.god",
      });
    ...
    test("nested transaction rollback with save-points will commit the outer one", async () => {
      ...
      const transaction = orm.em.transactional(async (em) => {
        ...
        await em.persistAndFlush(
          orm.em.create(Author4, {
            name: "God Persisted!",
            email: "hello-persisted@heaven.god",
          }),
        );
      });
      ...
      await expect(
        orm.em.findOne(Author4, { name: "God Persisted!" }),
    ...
    test("should load entities", async () => {
      ...
      const god = orm.em.create(Author4, {
        name: "God",
        email: "hello@heaven.god",
      });
      ...
      const publisher7k = await orm.em
        .getRepository(Publisher4)
        .findOneOrFail({ name: "7K publisher" });
      ...
      expect(
        await authorRepository.findOne({ favouriteBook: bible.id }),
      ).not.toBe(null);
      orm.em.clear();
      ...
      const jon = await authorRepository.findOneOrFail(
        { name: "Jon Snow" },
    ...
    test("findOne supports optimistic locking [testStandardFailureThrowsException]", async () => {
      ...
      await orm.em.nativeUpdate<ITest4>(
        "Test4",
        { id: test.id },
        { name: "Changed!" },
    ...
    test("pessimistic locking requires active transaction", async () => {
      ...
      await orm.em.flush();
      await expect(
        orm.em.findOne(Test4, test.id, { lockMode: LockMode.PESSIMISTIC_READ }),
      ).rejects.toThrow("An open transaction is required for this operation");
      await expect(
        orm.em.findOne(Test4, test.id, {
          lockMode: LockMode.PESSIMISTIC_WRITE,
        }),
      ).rejects.toThrow("An open transaction is required for this operation");
      await expect(
        orm.em.lock(test, LockMode.PESSIMISTIC_READ),
      ).rejects.toThrow("An open transaction is required for this operation");
      await expect(
        orm.em.lock(test, LockMode.PESSIMISTIC_WRITE),
      ).rejects.toThrow("An open transaction is required for this operation");
    });
    ...
    test("findOne does not support pessimistic locking [pessimistic write]", async () => {
      ...
      expect(mock.mock.calls[1][0]).toMatch(
        "select 1 from `author4` as `a0` where `a0`.`id` = ?",
      );
    ...
    test("stable results of serialization", async () => {
      ...
      const bible = orm.em.create(Book4, { title: "Bible", author: god });
      const bible2 = orm.em.create(Book4, {
        title: "Bible pt. 2",
        author: god,
      });
    ...
    test("stable results of serialization (collection)", async () => {
      ...
      const publisher = await orm.em.findOneOrFail(
        "Publisher4" as EntityName<IPublisher4>,
        pub.id,
        { populate: ["books"] },
      );
      await wrap(newGod).init();
    ...
    test("json properties", async () => {
      ...
      const g = await orm.em.findOneOrFail(Author4, god.id, {
        populate: ["books"],
      });
      ...
      const b1 = await orm.em.findOneOrFail(Book4, {
        meta: { category: "god like" },
      });
      const b2 = await orm.em.findOneOrFail(Book4, {
        meta: { category: "god like", items: 3 },
      });
      const b3 = await orm.em.findOneOrFail(Book4, {
        meta: { nested: { bar: 321 } },
      });
      const b4 = await orm.em.findOneOrFail(Book4, {
        meta: { nested: { foo: "123", bar: 321 } },
      });
      const b5 = await orm.em.findOneOrFail(Book4, {
    ...
    test("findOne by id", async () => {
      const authorRepository = orm.em.getRepository(Author4);
      const jon = orm.em.create(Author4, {
        name: "Jon Snow",
        email: "snow@wall.st",
      });
    ...
    test("many to many relation", async () => {
      ...
      orm.em.clear();
      book = (await orm.em.findOne(Book4, book.id, {
      ...
      // contains
      expect(book.tags.contains(tagRepository.getReference(tag1.id))).toBe(
        true,
      );
      expect(book.tags.contains(tagRepository.getReference(tag2.id))).toBe(
        false,
      );
      expect(book.tags.contains(tagRepository.getReference(tag3.id))).toBe(
        true,
      );
      expect(book.tags.contains(tagRepository.getReference(tag4.id))).toBe(
        false,
      );
      expect(book.tags.contains(tagRepository.getReference(tag5.id))).toBe(
        false,
      );
    ...
    test("partial loading of collections", async () => {
      ...
      for (let i = 1; i <= 15; i++) {
        const book = orm.em.create(Book4, {
          title: `book ${("" + i).padStart(2, "0")}`,
        });
        author.books.add(book);

        for (let j = 1; j <= 15; j++) {
          const tag1 = orm.em.create(BookTag4, {
            name: `tag ${("" + i).padStart(2, "0")}-${("" + j).padStart(2, "0")}`,
          });
          book.tags.add(tag1);
        }
      }
      ...
      const books = await a.books.matching({
        limit: 5,
        offset: 10,
        orderBy: { title: "asc" },
      });
      ...
      expect(() =>
        books[0].tags.add(orm.em.create(BookTag4, { name: "new" })),
      ).toThrow(
      ...
      const [book11, ...rest] = await tags[0].books.matching({
        where: { title: "book 11" },
      });
      ...
      const filteredTags = await books[0].tags.matching({
        filters: { testFilter: true },
      });
    ...
    test("populating many to many relation", async () => {
      ...
      expect(wrap(publishers[1].tests.getItems()[0]).isInitialized()).toBe(
        true,
      );
    });
    ...
    test("trying to populate non-existing or non-reference property will throw", async () => {
      ...
      ).rejects.toThrow(`Entity 'Author4' does not have property 'tests'`);
      await expect(
    ...
    test("EM supports native insert/update/delete", async () => {
      ...
      expect(res0).toEqual([]);
      const res1 = await orm.em.insert<IAuthor4>("Author4", {
        name: "native name 1",
      ...
      const res3 = await orm.em.nativeDelete(Author4, {
        name: "new native name",
      });
      expect(res3).toBe(1);
    ...
    test("datetime is stored in correct timezone", async () => {
      ...
      const res = await orm.em
        ...
        .execute<
          { created_at: number }[]
        >(`select created_at as created_at from author4 where id = ${author.id}`);
    ...
    test("merging results from QB to existing entity", async () => {
      ...
      await orm.em
        .createQueryBuilder(FooBar4)
        .select(raw(`id, '123' as virtual`))
        .getResultList();
    ...
    test("merging detached entity", async () => {
      ...
      const jon = await orm.em.findOneOrFail(Author4, author.id, {
    ...
    test("batch update with changing OneToOne relation (GH issue #1025)", async () => {
      ...
      expect(mock.mock.calls[2][0]).toMatch(
        "update `foo_bar4` set `foo_bar_id` = case when (`id` = ?) then ? when (`id` = ?) then ? else `foo_bar_id` end, `updated_at` = case when (`id` = ?) then ? when (`id` = ?) then ? else `updated_at` end, `version` = `version` + 1 where `id` in (?, ?) returning `version`",
      );
    ...
    test("em.refresh", async () => {
      ...
      await expect(orm.em.refreshOrFail(e2)).rejects.toThrow(
        `Author4 not found (${e2.id})`,
      );
    });
    ...
    test("qb.getCount()`", async () => {
      ...
      const mock = mockLogger(orm);
      const count1 = await orm.em
        .createQueryBuilder(Author4)
        .limit(10, 20)
        .getCount();
      expect(count1).toBe(50);
      const count2 = await orm.em
        .createQueryBuilder(Author4)
        .getCount("termsAccepted");
      expect(count2).toBe(50);
      const count3 = await orm.em
        .createQueryBuilder(Author4)
        .getCount("termsAccepted", true);
      expect(count3).toBe(2);
      const count4 = await orm.em
        .createQueryBuilder(Author4)
        .where({ email: "123" })
        .getCount();
      expect(count4).toBe(0);
      expect(mock.mock.calls[0][0]).toMatch(
        "select count(*) as `count` from `author4` as `a0`",
      );
      expect(mock.mock.calls[1][0]).toMatch(
        "select count(`a0`.`terms_accepted`) as `count` from `author4` as `a0`",
      );
      expect(mock.mock.calls[2][0]).toMatch(
    ...
    test("perf: one to many", async () => {
      ...
      for (let i = 1; i <= 3_000; i++) {
        const book = orm.em.create(Book4, {
          title: "My Life on The Wall, part " + i,
          author,
        });
        author.books.add(book);
      }
    ...
    test("loadCount to get the number of entries without initializing the collection (GH issue #949)", async () => {
      let author = orm.em.create(Author4, {
        name: "Jon Doe",
        email: "doe-jon@wall.st",
      });
      ...
      await expect(taggedBook.tags.loadCount()).resolves.toEqual(
        tags.length - 1,
      );
      await expect(taggedBook.tags.loadCount(true)).resolves.toEqual(
        tags.length,
      );
      await orm.em.flush();
      orm.em.clear();
    ...
    test("loadCount with unidirectional m:n (GH issue #1608)", async () => {
      ...
      ent = await orm.em.findOneOrFail(Publisher4, publisher.id, {
