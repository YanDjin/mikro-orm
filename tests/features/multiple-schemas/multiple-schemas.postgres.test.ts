...
describe("multiple connected schemas in postgres", () => {
  ...
  test("should work", async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      `update "n1"."author" set "name" = 'new name' where "id" = 1`,
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch(`begin`);
    expect(mock.mock.calls[1][0]).toMatch(
      `delete from "n2"."book" where "id" in (1, 2, 3)`,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      `delete from "n1"."author" where "id" in (1)`,
    );
    expect(mock.mock.calls[3][0]).toMatch(
      `delete from "n2"."book_tag" where "id" in (1, 2, 3, 4, 5, 6, 7, 8, 9)`,
    );
    expect(mock.mock.calls[4][0]).toMatch(`commit`);
  ...
  test("use different schema via options", async () => {
    ...
    expect(mock.mock.calls[4][0]).toMatch(
      `insert into "n1"."author" ("name") values ('a1') returning "id"`,
    );
    expect(mock.mock.calls[5][0]).toMatch(
      `insert into "n3"."book" ("author_id") values (1) returning "id"`,
    );
    expect(mock.mock.calls[6][0]).toMatch(
      `insert into "n5"."book" ("author_id") values (1), (1) returning "id"`,
    );
    expect(mock.mock.calls[7][0]).toMatch(
      `insert into "n4"."book" ("author_id") values (1) returning "id"`,
    );
    expect(mock.mock.calls[8][0]).toMatch(
      `update "n5"."book" set "based_on_id" = 1 where "id" = 1`,
    );
    expect(mock.mock.calls[9][0]).toMatch(
      `update "n4"."book" set "based_on_id" = 1 where "id" = 1`,
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select "a0".* from "n1"."author" as "a0" where "a0"."id" = 1 limit 1`,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      `select "b0".* from "n5"."book" as "b0" where "b0"."author_id" in (1)`,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      `select "b1".*, "b0"."book_tag_id" as "fk__book_tag_id", "b0"."book_id" as "fk__book_id" from "n5"."book_tags" as "b0" inner join "n5"."book_tag" as "b1" on "b0"."book_tag_id" = "b1"."id" where "b0"."book_id" in (2, 1)`,
    );
    mock.mockReset();
    ...
    expect(mock.mock.calls[0][0]).toMatch(`begin`);
    expect(mock.mock.calls[1][0]).toMatch(
      `delete from "n3"."book" where "id" in (1)`,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      `delete from "n4"."book" where "id" in (1)`,
    );
    expect(mock.mock.calls[3][0]).toMatch(
      `delete from "n5"."book" where "id" in (1, 2)`,
    );
    expect(mock.mock.calls[4][0]).toMatch(
      `delete from "n1"."author" where "id" in (1)`,
    );
    expect(mock.mock.calls[5][0]).toMatch(
      `delete from "n3"."book_tag" where "id" in (1, 2, 3)`,
    );
    expect(mock.mock.calls[6][0]).toMatch(
      `delete from "n4"."book_tag" where "id" in (1, 2, 3)`,
    );
    expect(mock.mock.calls[7][0]).toMatch(
      `delete from "n5"."book_tag" where "id" in (1, 2, 3, 4, 5, 6)`,
    );
    expect(mock.mock.calls[8][0]).toMatch(`commit`);
