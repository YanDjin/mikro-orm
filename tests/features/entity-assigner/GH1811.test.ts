...
describe("GH issue 1811", () => {
  ...
  test("assigning to 1:m with nested entities", async () => {
    ...
    const r1 = await orm.em
      .fork()
      .findOneOrFail(Recipe, recipe, { populate: ["ingredients"] });
  ...
  test("assigning to m:1 with nested entities", async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      "update `ingredient` set `recipe_id` = ?, `name` = ? where `id` = ?",
    );
    expect(mock.mock.calls[3][0]).toMatch("commit");

    const r2 = await orm.em
      .fork()
      .find(Recipe, {}, { populate: ["ingredients"] });
  ...
  test("assigning to m:m with nested entities", async () => {
    ...
    expect(mock.mock.calls[3][0]).toMatch(
      "delete from `recipe_authors` where (`user_id` = ? and `recipe_id` = ?)",
    );
    expect(mock.mock.calls[4][0]).toMatch(
      "insert into `recipe_authors` (`user_id`, `recipe_id`) values (?, ?)",
    );
    expect(mock.mock.calls[5][0]).toMatch("commit");

    const r1 = await orm.em
      .fork()
      .findOneOrFail(Recipe, recipe, {
        populate: ["authors"],
