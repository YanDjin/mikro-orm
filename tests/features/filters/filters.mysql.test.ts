...
import { QueryOrder } from "@yandjin-mikro-orm/core";
...
describe("filters [mysql]", () => {
  ...
  test("filters (select-in)", async () => {
    ...
    expect(mock.mock.calls[8][0]).toMatch(
      "update `book2` set `title` = 'b123' where length(perex) > 10000 and `uuid_pk` = '123'",
    );
    ...
    expect(mock.mock.calls[9][0]).toMatch(
      "delete from `book2` where length(perex) > 10000 and `uuid_pk` = '321'",
    );
