...
import { Entity, Property, wrap } from "@yandjin-mikro-orm/core";
import { EntityManager, MikroORM } from "@yandjin-mikro-orm/mongodb";
...
class BookWithAuthor {
  ...
  @Property({
    ...
    serializer: (val) => `The value is: ${val}`,
  })
  authorName!: string;
}
...
describe("virtual entities (mongo)", () => {
  ...
  test("with callback", async () => {
    ...
    const book = await orm.em.findOneOrFail(BookWithAuthor, {
    ...
    const someBooks1 = await orm.em.find(
      BookWithAuthor,
      {},
      { limit: 2, offset: 1, orderBy: { title: 1 } },
    );
    ...
    const someBooks4 = await orm.em.find(BookWithAuthor, {
      title: {
        $in: ["My Life on the Wall, part 1/2", "My Life on the Wall, part 1/3"],
      },
    });
