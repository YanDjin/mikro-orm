...
import { MikroORM, PostgreSqlDriver } from "@yandjin-mikro-orm/postgresql";
...
describe("embedded entities in postgres", () => {
  ...
  test("schema", async () => {
    await expect(
      orm.schema.getCreateSchemaSQL({ wrap: false }),
    ).resolves.toMatchSnapshot("nested embeddables 1");
    await expect(
      orm.schema.getUpdateSchemaSQL({ wrap: false }),
    ).resolves.toMatchSnapshot("nested embeddables 2");
    await expect(
      orm.schema.getDropSchemaSQL({ wrap: false }),
    ).resolves.toMatchSnapshot("nested embeddables 3");
  });

  test("diffing", async () => {
    expect(
      orm.em.getComparator().getSnapshotGenerator("User").toString(),
    ).toMatchSnapshot();
  });
  ...
  test("persist and load", async () => {
    ...
    user1.name = "Uwe";
    user1.profile1 = new Profile(
      "u1",
      new Identity("e1", new IdentityMeta("f1", "b1")),
    );
    user1.profile2 = new Profile(
      "u2",
      new Identity("e2", new IdentityMeta("f2", "b2")),
    );

    const user2 = new User();
    user2.name = "Uschi";
    user2.profile1 = new Profile("u3", new Identity("e3"));
    user2.profile1.identity.links.push(
      new IdentityLink("l1"),
      new IdentityLink("l2"),
    );
    user2.profile2 = new Profile(
      "u4",
      new Identity("e4", new IdentityMeta("f4")),
    );
    user2.profile2.identity.links.push(
      new IdentityLink("l3"),
      new IdentityLink("l4"),
    );
    ...
    expect(Object.keys(u1).sort()).toEqual([
      "id",
      "name",
      "profile1",
      "profile2",
    ...
    const u3 = await orm.em.findOneOrFail(User, {
      ...
      profile2: {
        identity: { email: "e2", meta: { foo: "f2", bar: "bababar" } },
      },
    });
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0".* from "user" as "u0" where "u0"."profile1_identity_email" = 'e123' and "u0"."profile1_identity_meta_foo" = 'foooooooo' and "u0"."profile2"->'identity'->>'email' = 'e2' and "u0"."profile2"->'identity'->'meta'->>'foo' = 'f2' and "u0"."profile2"->'identity'->'meta'->>'bar' = 'bababar' limit 1`,
    );
    ...
    const u4 = await orm.em.findOneOrFail(User, {
      ...
      profile2: {
        identity: { email: "e2", meta: { foo: "f2", bar: { $re: "(ba)+r" } } },
      },
    });
    expect(u4.id).toEqual(u1.id);
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0".* from "user" as "u0" where "u0"."profile1_identity_email" = 'e123' and "u0"."profile1_identity_meta_foo" ~ 'fo+' and "u0"."profile2"->'identity'->>'email' = 'e2' and "u0"."profile2"->'identity'->'meta'->>'foo' = 'f2' and "u0"."profile2"->'identity'->'meta'->>'bar' ~ '(ba)+r' limit 1`,
    );
    orm.em.clear();
    mock.mock.calls.length = 0;
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0".* from "user" as "u0" where ("u0"."profile1_identity_meta_foo" = 'foooooooo' or "u0"."profile2"->'identity'->'meta'->>'bar' = 'bababar') limit 1`,
    );
    ...
    await expect(
      orm.em.findOneOrFail(User, {
        profile1: { identity: { city: "London 1" } as any },
      }),
    ...
    await expect(
      orm.em.findOneOrFail(User, {
        profile2: { identity: { city: "London 1" } as any },
      }),
  ...
  test("partial loading", async () => {
    ...
    await orm.em
      .fork()
      .find(User, {}, { fields: ["profile1.identity.meta.foo"] });

    // partial loading works also for object mode embeddables, including nesting (GH #4199)
    const u = await orm.em
      .fork()
      .find(User, {}, { fields: ["profile2.identity.meta.foo"] });
    expect(wrap(u[0]).toObject()).toEqual({
      id: 1,
      profile2: { identity: { meta: { foo: "f2" } } },
    });
    expect(wrap(u[1]).toObject()).toEqual({
      id: 2,
      profile2: { identity: { meta: { foo: "f4" } } },
    });

    // @ts-expect-error old syntax is still technically supported, but not on type level
    await orm.em
      .fork()
      .find(User, {}, { fields: [{ profile1: ["identity"] }] });
    // @ts-expect-error old syntax is still technically supported, but not on type level
    await orm.em
      .fork()
      .find(User, {}, { fields: [{ profile2: ["identity"] }] });

    expect(mock.mock.calls[0][0]).toMatch(
      'select "u0"."id", "u0"."profile1_username", "u0"."profile1_identity_email", "u0"."profile1_identity_meta_foo", "u0"."profile1_identity_meta_bar", "u0"."profile1_identity_links" from "user" as "u0"',
    );
    expect(mock.mock.calls[1][0]).toMatch(
      'select "u0"."id", "u0"."profile2" from "user" as "u0"',
    );
    expect(mock.mock.calls[2][0]).toMatch(
      'select "u0"."id", "u0"."profile1_username" from "user" as "u0"',
    );
    expect(mock.mock.calls[3][0]).toMatch(
      'select "u0"."id", "u0"."profile2" from "user" as "u0"',
    );
    expect(mock.mock.calls[4][0]).toMatch(
      'select "u0"."id", "u0"."profile1_identity_email", "u0"."profile1_identity_meta_foo", "u0"."profile1_identity_meta_bar", "u0"."profile1_identity_links" from "user" as "u0"',
    );
    expect(mock.mock.calls[5][0]).toMatch(
      'select "u0"."id", "u0"."profile2" from "user" as "u0"',
    );
    expect(mock.mock.calls[6][0]).toMatch(
      'select "u0"."id", "u0"."profile1_identity_email" from "user" as "u0"',
    );
    expect(mock.mock.calls[7][0]).toMatch(
      'select "u0"."id", "u0"."profile2" from "user" as "u0"',
    );
    expect(mock.mock.calls[8][0]).toMatch(
      'select "u0"."id", "u0"."profile1_identity_meta_foo" from "user" as "u0"',
    );
  ...
  test("#assign() works with nested embeddables", async () => {
    ...
    orm.em.assign(jon, {
      profile1: {
        username: "u1",
        identity: { email: "e1", meta: { bar: "b1", foo: "f1" } },
      },
      profile2: {
        username: "u2",
        identity: { email: "e2", meta: { bar: "b2", foo: "f2" } },
      },
    });
    expect(jon.profile1).toMatchObject({
      username: "u1",
      identity: { email: "e1", meta: { bar: "b1", foo: "f1" } },
    });
    ...
    expect(jon.profile2).toMatchObject({
      username: "u2",
      identity: { email: "e2", meta: { bar: "b2", foo: "f2" } },
    });
  ...
  test("partial loading 2", async () => {
    ...
    await orm.em
      .fork()
      .qb(User)
      .select("profile1.identity.email")
      .where({ profile1: { identity: { email: "foo@bar.baz" } } })
      .execute();
    expect(mock.mock.calls[0][0]).toMatch(
      'select "u0"."profile1_identity_email" from "user" as "u0" where "u0"."profile1_identity_email" = $1',
    );
    ...
    await orm.em
      ...
      .findOne(
        User,
        { profile1: { identity: { email: "foo@bar.baz" } } },
        { fields: ["profile1.identity.email"] },
      );
    expect(mock.mock.calls[1][0]).toMatch(
      'select "u0"."id", "u0"."profile1_identity_email" from "user" as "u0" where "u0"."profile1_identity_email" = $1 limit $2',
    );
    ...
    await orm.em
      ...
      .execute();
    expect(mock.mock.calls[2][0]).toMatch(
      'select "u0"."profile1_username", "u0"."profile1_identity_email", "u0"."profile1_identity_meta_foo", "u0"."profile1_identity_meta_bar", "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_email" = $1',
    );
    ...
    expect(mock.mock.calls[3][0]).toMatch(
      'select "u0"."id", "u0"."profile1_username", "u0"."profile1_identity_email", "u0"."profile1_identity_meta_foo", "u0"."profile1_identity_meta_bar", "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_email" = $1 limit $2',
    );
    ...
    await orm.em
      .fork()
      .qb(User)
      .select("profile2.identity.email")
      .where({ profile2: { identity: { email: "foo@bar.baz" } } })
      .execute(); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->>'email' = $1`,
    );
    ...
    await orm.em
      ...
      .findOne(
        User,
        { profile2: { identity: { email: "foo@bar.baz" } } },
        { fields: ["profile2.identity.email"] },
      ); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[1][0]).toMatch(
      `select "u0"."id", "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->>'email' = $1 limit $2`,
    );
    ...
    await orm.em
      .fork()
      .qb(User)
      .select("profile1.identity.links.url")
      .where({ profile1: { identity: { links: { url: "foo@bar.baz" } } } })
      .execute(); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_links"->>'url' = $1`,
    );
    ...
    await orm.em
      ...
      .findOne(
        User,
        { profile1: { identity: { links: { url: "foo@bar.baz" } } } },
        { fields: ["profile1.identity.links.url"] },
      ); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[1][0]).toMatch(
      `select "u0"."id", "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_links"->>'url' = $1 limit $2`,
    );

    await orm.em
      .fork()
      .qb(User)
      .select("profile1_identity_links")
      .where({ profile1: { identity: { links: { url: "foo@bar.baz" } } } })
      .execute();
    expect(mock.mock.calls[2][0]).toMatch(
      `select "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_links"->>'url' = $1`,
    ...
    await orm.em
      .fork()
      .qb(User)
      .select("profile2.identity.links.url")
      .where({ profile2: { identity: { links: { url: "foo@bar.baz" } } } })
      .execute(); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->'links'->>'url' = $1`,
    );
    ...
    await orm.em
      ...
      .findOne(
        User,
        { profile2: { identity: { links: { url: "foo@bar.baz" } } } },
        { fields: ["profile2.identity.links.url"] },
      ); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[1][0]).toMatch(
      `select "u0"."id", "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->'links'->>'url' = $1 limit $2`,
    );
    ...
    await orm.em
      ...
      .findOne(
        User,
        { profile2: { identity: { links: { url: "foo@bar.baz" } } } },
        { fields: ["profile2.identity.links"] },
      );
    expect(mock.mock.calls[3][0]).toMatch(
      `select "u0"."id", "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->'links'->>'url' = $1 limit $2`,
    );
  ...
  test("unique constraints", async () => {
    ...
    await expect(orm.em.persistAndFlush([user1, user2])).rejects.toThrow(
