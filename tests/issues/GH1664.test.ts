...
describe("embedded entities in postgresql", () => {
  ...
  test("persist and load single item", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      'insert into "multiple_unique_nullable_properties" ("first", "second") values ($1, $2) returning "id"',
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      'update "multiple_unique_nullable_properties" set "first" = $1, "second" = $2 where "id" = $3',
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
  });
  ...
  test("persist and load multiple items", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch("begin");
    expect(mock.mock.calls[1][0]).toMatch(
      'insert into "multiple_unique_nullable_properties" ("first", "second") values ($1, $2), ($3, $4) returning "id"',
    );
    expect(mock.mock.calls[2][0]).toMatch("commit");
    mock.mock.calls.length = 0;
