...
import { SqlEntityRepository } from "@yandjin-mikro-orm/knex";
...
export async function initORMMySql<
  D extends MySqlDriver | MariaDbDriver = MySqlDriver,
>(
  ...
  let orm = MikroORM.initSync<AbstractSqlDriver>(
    Utils.merge(
      {
        entities: [
          Author2,
...
export async function initORMSqlite2(
  type: "sqlite" | "better-sqlite" = "sqlite",
) {
