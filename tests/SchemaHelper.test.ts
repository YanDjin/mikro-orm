import type { Column } from "@yandjin-mikro-orm/sqlite";
import { SchemaHelper, SqlitePlatform } from "@yandjin-mikro-orm/sqlite";
import { MySqlPlatform } from "@yandjin-mikro-orm/mysql";
import {
  ColumnDifference,
  PostgreSqlPlatform,
  TableDifference,
...
describe("SchemaHelper", () => {
  test("default schema helpers", async () => {
    ...
    await expect(
      helper.getEnumDefinitions(jest.fn() as any, [], ""),
    ).resolves.toEqual({});
    expect(() => helper.getListTablesSQL()).toThrow(
      "Not supported by given driver",
    );
    expect(() => helper.getAlterNativeEnumSQL("table")).toThrow(
      "Not supported by given driver",
    );
    expect(() => helper.getDropNativeEnumSQL("table")).toThrow(
      "Not supported by given driver",
    );
    expect(() => helper.getForeignKeysSQL("table")).toThrow(
      "Not supported by given driver",
    );
    await expect(helper.getColumns({} as any, "table")).rejects.toThrow(
      "Not supported by given driver",
    );
    await expect(helper.getIndexes({} as any, "table")).rejects.toThrow(
      "Not supported by given driver",
    );
    await expect(helper.getChecks({} as any, "table")).rejects.toThrow(
      "Not supported by given driver",
    );
  });
  ...
  describe("postgresql schema helper", () => {
    test("helper quotes schema name when dropping contraints", () => {
      ...
      const fromEnum = {
        name: "test_enum",
        nullable: false,
        mappedType: Type.getType(EnumType),
      } as Column;
      const toEnum = {
        name: "test_enum",
        nullable: false,
        mappedType: Type.getType(StringType),
      } as Column;
      const fromUuid = {
        name: "test_uuid",
        nullable: false,
