import type { MikroORM, ValidationError } from "@yandjin-mikro-orm/core";
import { LoadStrategy, wrap } from "@yandjin-mikro-orm/core";
import { AbstractSqlConnection, MySqlDriver } from "@yandjin-mikro-orm/mysql";
import {
  Author2,
  Configuration2,
  FooBar2,
...
describe("composite keys in mysql", () => {
  ...
  beforeAll(
    async () =>
      (orm = await initORMMySql("mysql", { loadStrategy: "joined" }, true)),
  );
  ...
  test("working with composite entity", async () => {
    ...
    const p1 = await orm.em.findOneOrFail(
      FooParam2,
      [param.bar.id, param.baz.id],
      { populate: ["bar"] },
    );
  ...
  test("composite key references", async () => {
    ...
    expect(() => orm.em.getReference(Car2, 1 as any)).toThrow(
      "Composite key required for entity Car2.",
    );
  ...
  test("composite key in em.create()", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "insert into `car2` (`name`, `year`, `price`) values (?, ?, ?), (?, ?, ?)",
    ); // c1, c2
    expect(mock.mock.calls[2][0]).toMatch(
      "insert into `user2` (`first_name`, `last_name`) values (?, ?)",
    ); // u1
  ...
  test("changing PK", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "update `car2` set `year` = 2015 where `name` = 'Audi A8' and `year` = 2010",
    );
