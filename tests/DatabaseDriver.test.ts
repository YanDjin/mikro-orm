...
describe("DatabaseDriver", () => {
  const config = new Configuration(
    { driver: MongoDriver, allowGlobalContext: true } as any,
    false,
  );
  ...
  test("default validations", async () => {
    ...
    await expect(driver.aggregate("", [])).rejects.toThrow(
      "Aggregations are not supported by Driver driver",
    );
    await expect(driver.nativeUpdateMany("", [], [])).rejects.toThrow(
      "Batch updates are not supported by Driver driver",
    );
    await expect(
      driver.lockPessimistic({}, { lockMode: LockMode.NONE }),
    ).rejects.toThrow("Pessimistic locks are not supported by Driver driver");
  ...
  test("not supported", async () => {
    expect(() =>
      driver.getPlatform().getFullTextWhereClause({} as any),
    ).toThrow("Full text searching is not supported by this driver.");
    expect(() => driver.getPlatform().supportsCreatingFullTextIndex()).toThrow(
      "Full text searching is not supported by this driver.",
    );
    expect(() =>
      driver
        .getPlatform()
        .getFullTextIndexExpression({} as any, {} as any, {} as any, {} as any),
    ).toThrow("Full text searching is not supported by this driver.");
  });
});
