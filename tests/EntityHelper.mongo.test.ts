...
import type { AnyEntity, MikroORM } from "@yandjin-mikro-orm/core";
import { ref, Reference, serialize, wrap } from "@yandjin-mikro-orm/core";
...
describe("EntityHelperMongo", () => {
  ...
  test("#toJSON() should return DTO", async () => {
    ...
    expect(author.toJSON(false)).toMatchObject({
      fooBar: 123,
      email: author.email,
    });
  });
  ...
  test("#toJSON properly calls child entity toJSON with correct params", async () => {
    ...
    const newGod = await orm.em.findOneOrFail(Author, god.id, {
  ...
  test("BaseEntity methods", async () => {
    ...
    god.populated();
    await expect(god.populate(["favouriteAuthor"])).rejects.toThrow(
      "Entity Author is not managed.",
    );
  ...
  test("custom inspect shows get/set props", async () => {
    ...
    expect(inspect((bar as AnyEntity).__helper)).toBe(
      "[WrappedEntity<FooBar>]",
    );
    ...
    const god = orm.em.create(Author, {
      name: "God",
      email: "hello@heaven.god",
    });
