...
describe("check typings", () => {
  ...
  test("EntityDTO", async () => {
    ...
    const a = { books: [{ tags: [{}] }] } as unknown as EntityDTO<
      Loaded<Author2, "books.tags" | "books.publisher">
    >;
    const a11 = a.books;
    const a12 = a.books[0];
  ...
  test("FilterValue", async () => {
    assert<
      Has<
        FilterValue<string>,
        RegExp | string | null | never[] | OperatorMap<string>
      >
    >(true); // strings allow regexps
    assert<
      Has<FilterValue<number>, number | null | never[] | OperatorMap<number>>
    >(true);
    assert<Has<FilterValue<string>, number>>(false);
    assert<Has<FilterValue<Date>, Date | null | never[] | OperatorMap<Date>>>(
      true,
    );
    assert<
      Has<FilterValue<RegExp>, RegExp | null | never[] | OperatorMap<RegExp>>
    >(true);
    assert<Has<FilterValue<string>, number>>(false);
  ...
  test("Query", async () => {
    ...
    assert<IsAssignable<ExpandQuery<Author2>, { favouriteBook: number }>>(
      false,
    );
    assert<
      IsAssignable<ExpandQuery<Author2>, { favouriteBook: { author: number } }>
    >(true);
    assert<
      IsAssignable<ExpandQuery<Author2>, { favouriteBook: { author: null } }>
    >(true);
    assert<
      IsAssignable<ExpandQuery<Author2>, { favouriteBook: { author: string } }>
    >(false);
    ...
    assert<
      Has<
        ExpandQuery<Author2>,
        { books: { author: { born?: string } }; favouriteBook: null }
      >
    >(false);
    assert<
      Has<
        ExpandQuery<Author2>,
        { books: { author: { born?: number } }; favouriteBook: null }
      >
    >(false);
    ...
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { favouriteBook: { author: { born: Date } } }
      >
    >(false);
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { favouriteBook: { author: { born: string } } }
      >
    >(true);
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { favouriteBook: { author: { books: string[] } } }
      >
    >(true);
    assert<IsAssignable<ExpandQuery<Book2>, { author: { books: string[] } }>>(
      true,
    );
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { books: { author: { born: string } } }
      >
    >(true);
    assert<
      IsAssignable<ExpandQuery<Author2>, { books: { author: { born: Date } } }>
    >(false);
    assert<
      IsAssignable<ExpandQuery<Author2>, { books: { author: { born: null } } }>
    >(true);
    assert<IsAssignable<ExpandQuery<Author2>, { favouriteBook: null }>>(true);
    assert<IsAssignable<ExpandQuery<Author2>, { favouriteBook: string }>>(true);
    ...
    assert<
      Has<
        ExpandQuery<Book2>,
        { author: { born?: Date }; favouriteBook: string }
      >
    >(false); // favouriteBook does not exist on Book2
    assert<
      IsAssignable<
        ExpandQuery<Book2>,
        { author: { books: { publisher: number } } }
      >
    >(true);
    assert<
      IsAssignable<
        ExpandQuery<Book2>,
        { author: { books: { publisher: null } } }
      >
    >(true);
    assert<Has<ExpandQuery<Author2>, { favouriteBook?: ExpandQuery<Book2> }>>(
      true,
    );
    assert<IsAssignable<ExpandQuery<Author2>, { books: FilterValue<Book2> }>>(
      true,
    );
    ...
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { books: { author: Author2 }; favouriteBook: Book2 }
      >
    >(true);
    assert<
      IsAssignable<
    ...
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { books: { author: { born: string } }; favouriteBook: Book2 }
      >
    >(true);
    assert<
      IsAssignable<
    ...
    assert<
      IsAssignable<
        ExpandQuery<Author2>,
        { books: { author: { born: string } }; favouriteBook: { title: null } }
      >
    >(true);
  ...
  test("FilterQueryOrPrimary", async () => {
    ...
    assert<
      IsAssignable<FilterQuery<Author2>, { books: { author: { name: "asd" } } }>
    >(true);
    assert<IsAssignable<FilterQuery<Author2>, { books: { author: Author2 } }>>(
      true,
    );
    assert<IsAssignable<FilterQuery<Author2>, { books: { author: 123 } }>>(
      true,
    );
    ...
    assert<
      IsAssignable<
        FilterQuery<Author2>,
        { books: { title: "123" }; favouriteBook: null }
      >
    >(true);
    ...
    assert<
      IsAssignable<
        FilterQuery<Author2>,
        { $and: [{ favouriteBook: Book2 }, { name: string }] }
      >
    >(true);
    assert<
      IsAssignable<
        FilterQuery<Author2>,
        { $and: [{ favouriteBook: { title: string } }, { name: string }] }
      >
    >(true);
    assert<
      IsAssignable<
        FilterQuery<Author2>,
        { $and: [{ favouriteBook: string }, { name: string }] }
      >
    >(true);
    // assert<Has<FilterQuery<Author2>, Author2>>(true);
    assert<Has<FilterQuery<Author2>, number>>(true);
    assert<Has<FilterQuery<Author2>, { favouriteBook?: ExpandQuery<Book2> }>>(
      true,
    );
    ...
    assert<
      IsAssignable<
        FilterQuery<Book2>,
        { author: { favouriteBook: { tags: FilterValue<BookTag2> } } }
      >
    >(true);
    assert<
      IsAssignable<
        FilterQuery<Book2>,
        { author: { favouriteBook: { tags: BookTag2[] } } }
      >
    >(true);
    assert<
      IsAssignable<
        FilterQuery<Book2>,
        { author: { favouriteBook: { tags: string[] } } }
      >
    >(true);
    assert<IsAssignable<FilterQuery<Book2>, { tags: string[] }>>(true);
    assert<IsAssignable<FilterQuery<Book2>, { tags: string }>>(true);
    assert<IsAssignable<FilterQuery<Author2>, { books: { tags: number[] } }>>(
      true,
    );
    assert<IsAssignable<FilterQuery<Author2>, { books: { tags: bigint[] } }>>(
      true,
    );
    assert<IsAssignable<FilterQuery<Author2>, { books: { tags: string[] } }>>(
      true,
    );
    assert<IsAssignable<FilterQuery<Author2>, { books: { tags: boolean[] } }>>(
      false,
    );
  });
  ...
  test("AutoPath with optional nullable properties", async () => {
    ...
    await em.findOne("MessageRecipient" as EntityName<MessageRecipient>, "1", {
      populate: [
        ...
        "message.phoneService.phoneServiceVendor",
      ],
    });
  });
  ...
  test("Loaded type and assignability with extending the ORM BaseEntity (#3865)", async () => {
    ...
    const test: MemberNotification = {} as Loaded<
      MemberNotification,
      "notification"
    >;
  });
  ...
  test("inference of entity type", async () => {
    ...
    const res: Loaded<MemberNotification> | null = await em.findOne(
      "MemberNotification" as EntityName<MemberNotification>,
  ...
  test("Ref.load() returns Loaded type (#3755)", async () => {
    ...
    const populated3: Loaded<Parent, "children"> = await parent.loadOrFail({
      populate: ["children"],
    });
    const populated4: Loaded<Parent, "children"> = await parent.loadOrFail({
      populate: ["children", "id"],
    });
    const populated5: Loaded<Parent, "children"> = await parent.loadOrFail({
  ...
  test("assignability of Loaded<T> to Ref<T> via ref() helper", async () => {
    ...
    const circularReferenceBook = {} as Loaded<
      Book,
      "publishedBy.foundingAuthor"
    >;
  ...
  test("tuple type after entity serializized", async () => {
    assert<IsExact<EntityDTO<Book>["point"], [number, number] | undefined>>(
      true,
    );
  ...
  test("GH #3277", async () => {
    ...
    const owner1 = {} as Loaded<
      Owner,
      "vehicles.manufacturer" | "vehicles.type"
    >;
  ...
  test("GH #3277 (2)", async () => {
    ...
    const event1 = {} as Loaded<
      CalendarEvent,
      "calendar.owner" | "order.customer.foobar"
    >;
    const event2 = {} as Loaded<
      CalendarEvent,
      "calendar.owner.foobar" | "order.customer"
    >;
    const event3 = {} as Loaded<
      CalendarEvent,
      "calendar.owner" | "order.customer"
    >;
    const event4 = {} as Loaded<CalendarEvent, "calendar.owner" | "order">;
    const event5 = {} as Loaded<
      CalendarEvent,
      "calendar.owner" | "order.foobar"
    >;
