...
import { SchemaGenerator } from "@yandjin-mikro-orm/knex";
...
describe("SchemaGenerator", () => {
  ...
  test("update schema [mysql]", async () => {
    ...
    meta
      .get("author2_following")
      .props.forEach((prop) => (prop.kind = ReferenceKind.SCALAR));
    meta
      .get("author_to_friend")
      .props.forEach((prop) => (prop.kind = ReferenceKind.SCALAR));
  ...
  test("rename column [mysql]", async () => {
    ...
    const index = authorMeta.indexes.find(
      (i) => Utils.asArray(i.properties).join() === "name,age",
    )!;
    ...
    ).resolves.toMatchSnapshot("mysql-update-schema-rename-column");
  ...
  test("update schema enums [mysql]", async () => {
    ...
    newTableMeta.properties.enumTest.columnTypes[0] = Type.getType(
      EnumType,
    ).getColumnType(newTableMeta.properties.enumTest, orm.em.getPlatform());
