...
describe("MikroORM", () => {
  test("should throw when not enough config provided", async () => {
    const err = `No driver specified, please fill in the \`driver\` option or use \`defineConfig\` helper (to define your ORM config) or \`MikroORM\` class (to call the \`init\` method) exported from the driver package (e.g. \`import { defineConfig } from '@yandjin-mikro-orm/mysql'; export defineConfig({ ... })\`).`;
    expect(
      () => new MikroORM({ entities: ["entities"], clientUrl: "" }),
    ).toThrow(err);
    expect(
      () =>
        new MikroORM({
    ...
    expect(
      () => new MikroORM({ driver: MongoDriver, entities: [], dbName: "test" }),
    ).toThrow("No entities found, please use `entities` option");
    expect(
      () =>
        new MikroORM({
          driver: MongoDriver,
  ...
  test("should throw when only abstract entities were discovered", async () => {
    const err =
      "Only abstract entities were discovered, maybe you forgot to use @Entity() decorator?";
    await expect(
      MikroORM.init({
        driver: MongoDriver,
        dbName: "test",
  ...
  test("should use CLI config", async () => {
    ...
    pathExistsMock.mockImplementation(async (path) => {
      ...
      return (
        str.endsWith(".json") ||
        (str.endsWith("/mikro-orm.config.ts") &&
          !str.endsWith("/src/mikro-orm.config.ts"))
      );
    });
  ...
  test("should report connection failure", async () => {
    ...
    expect(logger.mock.calls[0][0]).toEqual(
      `[info] MikroORM version: ${await Utils.getORMVersion()}`,
    );
    expect(logger.mock.calls[1][0]).toEqual(
      "[info] MikroORM failed to connect to database not-found on mysql://root@127.0.0.1:3306",
    );
  });
