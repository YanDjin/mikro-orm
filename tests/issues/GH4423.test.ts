...
describe('GH issue 4423', () => {
  ...
  test('The owning side is in the main entity, This one chooses the wrong column of the pivot table', async () => {
    ...
    // joins on `m0`.`id` = `t1`.`task_id` rather than `m0`.`id` = `t1`.`manager_id`
    expect(mock.mock.calls[1][0]).toMatch(
      "select `m0`.*, `t1`.`task_id` as `fk__task_id`, `t1`.`manager_id` as `fk__manager_id` from `user` as `m0` left join `task_managers` as `t1` on `m0`.`id` = `t1`.`manager_id` where `t1`.`task_id` in (1) and `m0`.`type` = 'manager'",
    );
  });
  ...
  test('The owning side is in the relation, This one works normally', async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      'select `t0`.*, `t1`.`task_id` as `fk__task_id`, `t1`.`manager_id` as `fk__manager_id` from `task` as `t0` left join `task_managers` as `t1` on `t0`.`id` = `t1`.`task_id` where `t1`.`manager_id` in (1)',
    );
  });
});
