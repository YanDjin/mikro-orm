...
import { EntityManager, MikroORM } from "@yandjin-mikro-orm/better-sqlite";
...
const authorProfilesSQL =
  "select name, age, identity, " +
  ...
  expression: (em: EntityManager) => {
    return em
      ...
      .select([
        ...
        raw("group_concat(t.name) as tags"),
      ])
      .join("b.author", "a")
      .join("b.tags", "t")
      .groupBy("b.id");
  ...
  expression: (em: EntityManager) => {
    return em
      ...
      .select([
        "b.title",
        "a.name as author_name",
        sql`group_concat(t.name) as tags`,
      ])
      .join("b.author", "a")
      .join("b.tags", "t")
      .groupBy("b.id")
...
describe("virtual entities (sqlite)", () => {
  ...
  async function createEntities(index: number): Promise<IAuthor4> {
    ...
    const book1 = orm.em.create(Book4, {
      title: "My Life on the Wall, part 1/" + index,
      author,
    });
    const book2 = orm.em.create(Book4, {
      title: "My Life on the Wall, part 2/" + index,
      author,
    });
    const book3 = orm.em.create(Book4, {
      title: "My Life on the Wall, part 3/" + index,
      author,
    });
  ...
  test("with SQL expression", async () => {
    ...
    const someProfiles01 = await orm.em
      .qb(AuthorProfile)
      .limit(2)
      .offset(1)
      .orderBy({ name: "asc" });
    expect(someProfiles01).toHaveLength(2);
    expect(someProfiles01.map((p) => p.name)).toEqual([
      "Jon Snow 2",
      "Jon Snow 3",
    ]);

    const someProfiles02 = await orm.em
      .qb(AuthorProfile2)
      .limit(2)
      .offset(1)
      .orderBy({ name: "asc" });
    expect(someProfiles02).toHaveLength(2);
    expect(someProfiles02.map((p) => p.name)).toEqual([
      "Jon Snow 2",
      "Jon Snow 3",
    ]);

    const someProfiles1 = await orm.em.find(
      AuthorProfile2,
      {},
      { limit: 2, offset: 1, orderBy: { name: "asc" } },
    );
    expect(someProfiles1).toHaveLength(2);
    expect(someProfiles1.map((p) => p.name)).toEqual([
      "Jon Snow 2",
      "Jon Snow 3",
    ]);

    const someProfiles2 = await orm.em.find(
      AuthorProfile2,
      {},
      { limit: 2, orderBy: { name: "asc" } },
    );
    expect(someProfiles2).toHaveLength(2);
    expect(someProfiles2.map((p) => p.name)).toEqual([
      "Jon Snow 1",
      "Jon Snow 2",
    ]);

    const someProfiles3 = await orm.em.find(
      AuthorProfile2,
      { $and: [{ name: { $like: "Jon%" } }, { age: { $gte: 0 } }] },
      { limit: 2, orderBy: { name: "asc" } },
    );
    expect(someProfiles3).toHaveLength(2);
    expect(someProfiles3.map((p) => p.name)).toEqual([
      "Jon Snow 1",
      "Jon Snow 2",
    ]);

    const someProfiles4 = await orm.em.find(AuthorProfile2, {
      name: ["Jon Snow 2", "Jon Snow 3"],
    });
    expect(someProfiles4).toHaveLength(2);
    expect(someProfiles4.map((p) => p.name)).toEqual([
      "Jon Snow 2",
      "Jon Snow 3",
    ]);
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select count(*) as count from (${authorProfilesSQL}) as \`a0\``,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      `select * from (${authorProfilesSQL}) as \`a0\``,
    );
    expect(mock.mock.calls[2][0]).toMatch(
      `select \`a0\`.* from (${authorProfilesSQL}) as \`a0\` order by \`a0\`.\`name\` asc limit 2 offset 1`,
    );
    expect(mock.mock.calls[3][0]).toMatch(
      `select \`a0\`.* from (${authorProfilesSQL}) as \`a0\` order by \`a0\`.\`name\` asc limit 2 offset 1`,
    );
    expect(mock.mock.calls[4][0]).toMatch(
      `select * from (${authorProfilesSQL}) as \`a0\` order by \`a0\`.\`name\` asc limit 2 offset 1`,
    );
    expect(mock.mock.calls[5][0]).toMatch(
      `select * from (${authorProfilesSQL}) as \`a0\` order by \`a0\`.\`name\` asc limit 2`,
    );
    expect(mock.mock.calls[6][0]).toMatch(
      `select * from (${authorProfilesSQL}) as \`a0\` where \`a0\`.\`name\` like 'Jon%' and \`a0\`.\`age\` >= 0 order by \`a0\`.\`name\` asc limit 2`,
    );
    expect(mock.mock.calls[7][0]).toMatch(
      `select * from (${authorProfilesSQL}) as \`a0\` where \`a0\`.\`name\` in ('Jon Snow 2', 'Jon Snow 3')`,
    );
  ...
  test("with callback", async () => {
    ...
    const someBooks01 = await orm.em
      .qb(BookWithAuthor)
      .limit(2)
      .offset(1)
      .orderBy({ title: "asc" });
    expect(someBooks01).toHaveLength(2);
    expect(someBooks01.map((p) => p.title)).toEqual([
      "My Life on the Wall, part 1/2",
      "My Life on the Wall, part 1/3",
    ]);

    const someBooks02 = await orm.em
      .qb(BookWithAuthor2)
      .limit(2)
      .offset(1)
      .orderBy({ title: "asc" });
    expect(someBooks02).toHaveLength(2);
    expect(someBooks02.map((p) => p.title)).toEqual([
      "My Life on the Wall, part 1/2",
      "My Life on the Wall, part 1/3",
    ]);

    const someBooks1 = await orm.em.find(
      BookWithAuthor2,
      {},
      { limit: 2, offset: 1, orderBy: { title: "asc" } },
    );
    expect(someBooks1).toHaveLength(2);
    expect(someBooks1.map((p) => p.title)).toEqual([
      "My Life on the Wall, part 1/2",
      "My Life on the Wall, part 1/3",
    ]);

    const someBooks2 = await orm.em.find(
      BookWithAuthor2,
      {},
      { limit: 2, orderBy: { title: "asc" } },
    );
    expect(someBooks2).toHaveLength(2);
    expect(someBooks2.map((p) => p.title)).toEqual([
      "My Life on the Wall, part 1/1",
      "My Life on the Wall, part 1/2",
    ]);

    const someBooks3 = await orm.em.find(
      BookWithAuthor2,
      {
        $and: [{ title: { $like: "My Life%" } }, { authorName: { $ne: null } }],
      },
      { limit: 2, orderBy: { title: "asc" } },
    );
    expect(someBooks3).toHaveLength(2);
    expect(someBooks3.map((p) => p.title)).toEqual([
      "My Life on the Wall, part 1/1",
      "My Life on the Wall, part 1/2",
    ]);

    const someBooks4 = await orm.em.find(BookWithAuthor2, {
      title: ["My Life on the Wall, part 1/2", "My Life on the Wall, part 1/3"],
    });
    expect(someBooks4).toHaveLength(2);
    expect(someBooks4.map((p) => p.title)).toEqual([
      "My Life on the Wall, part 1/2",
      "My Life on the Wall, part 1/3",
    ]);
    ...
    expect(mock.mock.calls).toHaveLength(8);
    expect(mock.mock.calls[0][0]).toMatch(
      `select count(*) as count from (${sql}) as \`b0\``,
    );
    expect(mock.mock.calls[1][0]).toMatch(`select * from (${sql}) as \`b0\``);
    expect(mock.mock.calls[2][0]).toMatch(
      `select \`b0\`.* from (${sql}) as \`b0\` order by \`b0\`.\`title\` asc limit 2 offset 1`,
    );
    expect(mock.mock.calls[3][0]).toMatch(
      `select \`b0\`.* from (${sql}) as \`b0\` order by \`b0\`.\`title\` asc limit 2 offset 1`,
    );
    expect(mock.mock.calls[4][0]).toMatch(
      `select * from (${sql}) as \`b0\` order by \`b0\`.\`title\` asc limit 2 offset 1`,
    );
    expect(mock.mock.calls[5][0]).toMatch(
      `select * from (${sql}) as \`b0\` order by \`b0\`.\`title\` asc limit 2`,
    );
    expect(mock.mock.calls[6][0]).toMatch(
      `select * from (${sql}) as \`b0\` where \`b0\`.\`title\` like 'My Life%' and \`b0\`.\`author_name\` is not null order by \`b0\`.\`title\` asc limit 2`,
    );
    expect(mock.mock.calls[7][0]).toMatch(
      `select * from (${sql}) as \`b0\` where \`b0\`.\`title\` in ('My Life on the Wall, part 1/2', 'My Life on the Wall, part 1/3')`,
    );
    ...
    // pagination
    {
      const [books, total] = await orm.em.findAndCount(
        BookWithAuthor,
        {},
        { flags: [QueryFlag.PAGINATE], limit: 3, offset: 3 },
      );
      expect(books).toEqual([
        {
          title: "My Life on the Wall, part 1/2",
