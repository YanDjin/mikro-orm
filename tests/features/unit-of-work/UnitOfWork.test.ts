...
describe("UnitOfWork", () => {
  ...
  test("entity validation when persisting [not strict]", async () => {
    ...
    expect(() => computer.computeChangeSet(author)).toThrow(
      `Trying to set Author.name of type 'string' to '111' of type 'number'`,
    );

    // string date with unknown format will throw
    Object.assign(author, { name: "333", email: "444", createdAt: "asd" });
    expect(() => computer.computeChangeSet(author)).toThrow(
      `Trying to set Author.createdAt of type 'Date' to 'asd' of type 'string'`,
    );
    ...
    expect(() => computer.computeChangeSet(author)).toThrow(
      `Trying to set Author.termsAccepted of type 'boolean' to '2' of type 'number'`,
    );
    ...
    expect(() => computer.computeChangeSet(author)).toThrow(
      `Trying to set Author.age of type 'number' to 'asd' of type 'string'`,
    );
    Object.assign(author, { age: new Date() });
    expect(() => computer.computeChangeSet(author)).toThrow(
      /Trying to set Author\.age of type 'number' to '.*' of type 'Date'/,
    );
    Object.assign(author, { age: false });
    expect(() => computer.computeChangeSet(author)).toThrow(
      `Trying to set Author.age of type 'number' to 'false' of type 'boolean'`,
    );
    author.age = 21;
    ...
    delete author.books;
    expect(() => computer.computeChangeSet(author)).toThrow(
      `Author.books is not initialized, define it as 'books = new Collection<Book>(this);'`,
    );
  });
  ...
  test("entity validation when persisting [strict]", async () => {
    ...
    expect(() =>
      validator.validate(author, payload, orm.getMetadata().get(Author.name)),
    ).toThrow(
      `Trying to set Author.createdAt of type 'Date' to '2018-01-01' of type 'string'`,
    );
  });
  ...
  test("changeSet is null for empty payload", async () => {
    ...
    expect(
      uow.getIdentityMap().get("Author-00000001885f0a3cc37dc9f0"),
    ).toBeUndefined();
    ...
    expect(
      uow.getIdentityMap().get("Author-00000001885f0a3cc37dc9f2"),
    ).toBeUndefined();
    uow.clear();
  ...
  test("manually changing the UoW state during flush", async () => {
    ...
    class Subscriber implements EventSubscriber {
      async onFlush(args: FlushEventArgs): Promise<void> {
        ...
        const cs = changeSets.find(
          (cs) =>
            cs.type === ChangeSetType.CREATE && cs.entity instanceof FooBar,
        );
        ...
        const toRemove = changeSets.find(
          (cs) => cs.entity instanceof FooBar && cs.entity.name === "remove me",
        );
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `db.getCollection('foo-baz').insertMany([ { name: 'dynamic' } ], {})`,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('foo-bar'\)\.insertMany\(\[ { name: 'bar', onCreateTest: true, onUpdateTest: true, baz: ObjectId\('\w+'\) } ], {}\)/,
    );
