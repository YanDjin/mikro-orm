...
import { FullTextType, PostgreSqlDriver } from "@yandjin-mikro-orm/postgresql";
...
describe("SchemaGenerator [postgres]", () => {
  ...
  test("update indexes [postgres]", async () => {
    ...
    meta.get("Book2").properties.title.columnTypes[0] = Type.getType(
      FullTextType,
    ).getColumnType(meta.get("Book2").properties.title, orm.em.getPlatform());
    meta.get("Book2").indexes.push({ type: "fulltext", properties: ["title"] });

    diff = await orm.schema.getUpdateSchemaSQL({ wrap: false });
    expect(diff).toMatchSnapshot(
      "postgres-update-schema-add-fulltext-index-tsvector",
    );
    await orm.schema.execute(diff, { wrap: true });
