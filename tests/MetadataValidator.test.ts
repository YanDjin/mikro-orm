...
describe("MetadataValidator", () => {
  ...
  test("validates entity definition", async () => {
    const meta = {
      Author: { name: "Author", className: "Author", properties: {} },
    } as any;
    meta.Author.root = meta.Author;
    expect(() =>
      validator.validateEntityDefinition(
        new MetadataStorage(meta as any),
        "Author",
        options,
      ),
    ).toThrow("Author entity is missing @PrimaryKey()");

    // many to one
    meta.Author.primaryKeys = ["_id"];
    meta.Author.properties.test = {
      name: "test",
    ...
    ).toThrow("Author.test is missing type definition");
    ...
    ).toThrow("Author.test has unknown type: Test");
    ...
    ).toThrow(`Author.tests is missing 'mappedBy' option`);
    ...
    ).toThrow(`Author.tests has unknown 'mappedBy' reference: Test.foo`);

    meta.Test.properties.foo = {
      name: "foo",
    ...
    ).toThrow("Author.books has unknown type: Book");
    ...
    ).toThrow(`Author.books has unknown 'inversedBy' reference: Book.bar`);
    ...
    ).toThrow(`Author.books has unknown 'mappedBy' reference: Book.bar`);
    ...
    ).toThrow("Foo.bar has unknown type: Bar");
    ...
    ).toThrow(`Foo.bar has unknown 'inversedBy' reference: Bar.bar`);
    ...
    ).toThrow(`Foo.bar has unknown 'mappedBy' reference: Bar.bar`);
    ...
    // version field
    meta.Author.properties.version = {
      name: "version",
    ...
    meta.Author.properties.version2 = {
      name: "version2",
      kind: ReferenceKind.SCALAR,
      type: "number",
      version: true,
  ...
  test("validates duplicities in fieldName", async () => {
    ...
    expect(() =>
      validator.validateEntityDefinition(
        new MetadataStorage({ Foo1: schema1.meta }),
        "Foo1",
        options,
      ),
    ).toThrow(
      "Duplicate fieldNames are not allowed: Foo1.name (fieldName: 'name'), Foo1.age (fieldName: 'name')",
    );
    schema1.meta.properties.age.fieldNames[0] = "age";
    expect(() =>
      validator.validateDiscovered([schema1.meta], options),
    ).not.toThrow();
  });

  test("MetadataStorage.get throws when no metadata found", async () => {
    const storage = new MetadataStorage({});
    expect(() => storage.get("Test")).toThrow(
      "Metadata for entity Test not found",
    );
  });
  ...
  describe("Duplicate Entity Strategy", () => {
    ...
    test("throws an error when duplicate entity is not provided", async () => {
      ...
      // Assert
      expect(validateDiscoveryCommand).toThrow(
        "Duplicate entity names are not allowed: Foo1",
      );
    });
  });
});
