...
export class QueryBuilder<T extends object = AnyEntity> {
  ...
  addSelect(fields: Field<T> | Field<T>[]): SelectQueryBuilder<T> {
    ...
    return this.select([
      ...Utils.asArray(this._fields),
      ...Utils.asArray(fields),
    ]);
  }
  ...
  join(
    field: string | Knex.QueryBuilder | QueryBuilder<any>,
    alias: string,
    cond: QBFilterQuery = {},
  ...
  joinAndSelect(
    field: string | [field: string, qb: Knex.QueryBuilder | QueryBuilder<any>],
    alias: string,
    cond: QBFilterQuery = {},
    type = JoinType.innerJoin,
    path?: string,
  ...
  protected getFieldsForJoinedLoad(
    ...
    const joinKey = Object.keys(this._joins).find((join) =>
      join.endsWith(`#${alias}`),
    );
  ...
  where(
    ...
    if (rawField) {
      ...
      cond = QueryHelper.processWhere({
        ...
        aliased:
          !this.type || [QueryType.SELECT, QueryType.COUNT].includes(this.type),
    ...
    const criteriaNode = CriteriaNodeFactory.createNode<T>(
      this.metadata,
    ...
    if (
      ...
      this.setFlag(
        this.type === QueryType.UPDATE
          ? QueryFlag.UPDATE_SUB_QUERY
          : QueryFlag.DELETE_SUB_QUERY,
      );
  ...
  orderBy(
    ...
    Utils.asArray<QBQueryOrderMap<T>>(orderBy).forEach((o) => {
      ...
      this._orderBy.push(
        CriteriaNodeFactory.createNode<T>(
          this.metadata,
          this.mainAlias.entityName,
          processed,
        ).process(this, { matchPopulateJoins: true }),
      );
    });
  ...
  having(
    ...
    this._having = CriteriaNodeFactory.createNode<T>(
      this.metadata,
      this.mainAlias.entityName,
      cond,
    ).process(this);
  ...
  from<T extends AnyEntity<T> = AnyEntity>(
    target: EntityName<T> | QueryBuilder<T>,
    aliasName?: string,
  ): SelectQueryBuilder<T> {
    this.ensureNotFinalized();
    ...
    if (target instanceof QueryBuilder) {
      ...
      if (
        ...
        throw new Error(
          `Cannot override the alias to '${aliasName}' since a query already contains references to '${this._mainAlias.aliasName}'`,
        );
      }
  ...
  getKnexQuery(processVirtualEntity = true): Knex.QueryBuilder {
    ...
    Utils.runIfNotEmpty(
      () => this.helper.appendQueryCondition(type, this._cond, qb),
      this._cond && !this._onConflict,
    );
    Utils.runIfNotEmpty(
      () => qb.groupBy(this.prepareFields(this._groupBy, "groupBy")),
      this._groupBy,
    );
    Utils.runIfNotEmpty(
      () =>
        this.helper.appendQueryCondition(
          type,
          this._having,
          qb,
          undefined,
    ...
    Utils.runIfNotEmpty(() => {
      const queryOrder = this.helper.getQueryOrder(
        type,
        this._orderBy as FlatQueryOrderMap[],
        this._populateMap,
      );
    ...
    Utils.runIfNotEmpty(
      () => this._comments.forEach((comment) => qb.comment(comment)),
      this._comments,
    );
    Utils.runIfNotEmpty(
      () => this._hintComments.forEach((comment) => qb.hintComment(comment)),
      this._hintComments,
    );
    Utils.runIfNotEmpty(
      () => this.helper.appendOnConflictClause(type, this._onConflict!, qb),
      this._onConflict,
    );
    ...
    if (
      ...
      return (this.#query!.qb = this.knex.raw(
        qb.toSQL().toNative().sql + " cascade",
      ) as any);
    }
  ...
  getAliasForJoinPath(
    ...
    const join =
      typeof path === "string" ? this.getJoinForPath(path, options) : path;
  ...
  getAliasMap(): Dictionary<string> {
    return Object.fromEntries(
      Object.entries(this._aliases).map(
        ([key, value]: [string, Alias<any>]) => [key, value.entityName],
      ),
    );
  }
  ...
  async execute<U = any>(
    ...
    options?: ExecuteOptions | boolean,
  ): Promise<U> {
    options =
    ...
    if (
      ...
      [
        ...
        QueryType.DELETE,
        QueryType.TRUNCATE,
      ].includes(this.type ?? QueryType.SELECT)
    ) {
      this.connectionType = "write";
    }

    const query = this.toQuery()._sql;
    const cached = await this.em?.tryCache<T, U>(
      this.mainAlias.entityName,
      this._cache,
      ["qb.execute", query.sql, query.bindings, method],
    );
    ...
    const write =
      method === "run" || !this.platform.getConfig().get("preferReadReplicas");
    ...
    if (Array.isArray(res)) {
      ...
      mapped = res.map(
        (r) =>
          this.driver.mapResult<T>(r as T, meta, this._populate, this, map)!,
      );

      if (options.mergeResults && joinedProps.length > 0) {
        mapped = this.driver.mergeJoinedResult(
          mapped,
          this.mainAlias.metadata!,
          joinedProps,
        );
  ...
  async getResultList(limit?: number): Promise<T[]> {
    ...
    function propagatePopulateHint<U extends object>(
      entity: U,
      hint: PopulateOptions<U>[],
    ) {
      ...
      hint.forEach((hint) => {
        ...
        if (Utils.isEntity<U>(value, true)) {
          ...
          value
            .getItems(false)
            .forEach((item) =>
              propagatePopulateHint<any>(item, hint.children ?? []),
            );
        }
      });
    }
    ...
    for (const r of res) {
      const entity = this.em!.map<T>(this.mainAlias.entityName, r, {
        schema: this._schema,
      });
  ...
  then<TResult1 = any, TResult2 = never>(
    ...
    onrejected?:
      ...
      | undefined
      | null,
  ): Promise<T[] | number | QueryResult<T>> {
    let type = this.type ?? QueryType.SELECT;

    if (
      this.flags.has(QueryFlag.UPDATE_SUB_QUERY) ||
      this.flags.has(QueryFlag.DELETE_SUB_QUERY)
    ) {
  ...
  clone(reset?: boolean | string[]): QueryBuilder<T> {
    ...
    for (const prop of Object.keys(this)) {
      ...
      (qb as any)[prop] = properties.includes(prop)
        ? Utils.copy(this[prop as keyof this])
  ...
  private joinReference(
    field: string | Knex.QueryBuilder | QueryBuilder,
    alias: string,
    cond: Dictionary,
    type: JoinType,
    path?: string,
    ...
    const [fromAlias, fromField] = this.helper.splitField(
      field as EntityKey<T>,
    );
    ...
    if (prop.kind === ReferenceKind.ONE_TO_MANY) {
      ...
      const joins = this.helper.joinManyToManyReference(
        prop,
        fromAlias,
        alias,
        pivotAlias,
      ...
      // MANY_TO_ONE
      this._joins[aliasedName] = this.helper.joinManyToOneReference(
        prop,
        fromAlias,
        alias,
        type,
  ...
  private prepareFields<T, U extends string | Knex.Raw>(
    fields: Field<T>[],
    ...
    fields.forEach((field) => {
      ...
      const join = Object.keys(this._joins).find(
        (k) => field === k.substring(0, k.indexOf("#")),
      )!;

      if (join && type === "where") {
        ret.push(
          ...(this.helper.mapJoinColumns(
          ...
          const nest = (prop: EntityProperty): void => {
            for (const childProp of Object.values(prop.embeddedProps)) {
              if (
                ...
                (childProp.kind !== ReferenceKind.EMBEDDED ||
                  childProp.object) &&
    ...
    if (
      ...
      requiresSQLConversion.forEach((p) =>
    ...
    Object.keys(this._populateMap).forEach((f) => {
      if (!fields.includes(f.replace(/#\w+$/, "")) && type === "where") {
        const cols = this.helper.mapJoinColumns(
          this.type ?? QueryType.SELECT,
          this._joins[f],
        );
  ...
  private init(type: QueryType, data?: any, cond?: any): this {
    ...
    if (
      [QueryType.UPDATE, QueryType.DELETE].includes(type) &&
      Utils.hasObjectKeys(this._cond)
    ) {
      throw new Error(
    ...
    if (data) {
      if (Utils.isEntity(data)) {
        data =
          this.em?.getComparator().prepareEntity(data as T) ??
          serialize(data as T);
      }

      this._data = this.helper.processData(
        data,
        this.flags.has(QueryFlag.CONVERT_CUSTOM_TYPES),
        false,
      );
  ...
  private getQueryBase(processVirtualEntity: boolean): Knex.QueryBuilder {
    ...
    if (this._indexHint) {
      ...
      const tableName =
        schemaQuoted +
        this.platform.quoteIdentifier(
          this.helper.getTableName(this.mainAlias.entityName),
        ) +
        alias;
    ...
    switch (this.type) {
      case QueryType.SELECT:
        ...
        break;
      case QueryType.COUNT: {
        const m = this.flags.has(QueryFlag.DISTINCT)
          ? "countDistinct"
          : "count";
        qb[m]({
          count: this._fields!.map((f) =>
            this.helper.mapper(f as string, this.type),
          ),
        });
        this.helper.processJoins(qb, this._joins, joinSchema);
        break;
  ...
  private applyDiscriminatorCondition(): void {
    ...
    const types = Object.values(meta.root.discriminatorMap!).map(
      (cls) => this.metadata.find(cls)!,
    );
  ...
  private finalize(): void {
    ...
    this._populate.forEach(({ field }) => {
      ...
      const join = Object.keys(this._joins).find(
        (k) => `${aliasedField}#${this._joins[k].alias}` === k,
      );
    ...
    if (
      ...
      meta.props
        .filter(
          (prop) =>
            prop.formula &&
            (!prop.lazy || this.flags.has(QueryFlag.INCLUDE_LAZY_FORMULAS)),
        )
        .map((prop) => {
  ...
  private processPopulateWhere() {
    if (
      this._populateWhere == null ||
      this._populateWhere === PopulateHint.ALL
    ) {
      return;
    }
      ...
      Object.keys(cond).forEach((k) => {
        if (Utils.isOperator(k)) {
          if (Array.isArray(cond[k])) {
            return cond[k].forEach((c: Dictionary) =>
              replaceOnConditions(c, k),
            );
          }
    ...
    if (typeof this._populateWhere === "object") {
      const cond = CriteriaNodeFactory.createNode<T>(
        this.metadata,
        this.mainAlias.entityName,
        this._populateWhere,
      ).process(this, {
  ...
  private hasToManyJoins(): boolean {
    return Object.values(this._joins).some((join) => {
      return [ReferenceKind.ONE_TO_MANY, ReferenceKind.MANY_TO_MANY].includes(
        join.prop.kind,
      );
    });
  }
  ...
  private wrapPaginateSubQuery(meta: EntityMetadata): void {
    ...
    Object.values(subQuery._joins).forEach(
      (join) => (join.cond = join.cond_ ?? {}),
    );
    ...
    if (this._orderBy.length > 0) {
      ...
      for (const orderMap of this._orderBy) {
        for (const [field, direction] of Object.entries(orderMap)) {
          ...
          const fieldName = this.helper.mapper(
            field,
            this.type,
            undefined,
            null,
  ...
  private wrapModifySubQuery(meta: EntityMetadata): void {
    ...
    const method = this.flags.has(QueryFlag.UPDATE_SUB_QUERY)
      ? "update"
      : "delete";
  ...
  private getSchema(alias: Alias<any>): string | undefined {
    ...
    const metaSchema =
      metadata?.schema && metadata.schema !== "*" ? metadata.schema : undefined;
    return (
      this._schema ??
      metaSchema ??
      this.em?.schema ??
  ...
  private fromSubQuery<T extends AnyEntity<T> = AnyEntity>(
    target: QueryBuilder<T>,
    aliasName?: string,
  ): void {
  ...
  private ensureFromClause(): void {
    ...
    if (!this._mainAlias) {
      throw new Error(
        `Cannot proceed to build a query because the main alias is not set.`,
      );
    }
  }
...
export interface RunQueryBuilder<T extends object> extends Omit<
  QueryBuilder<T>,
...
export interface CountQueryBuilder<T extends object> extends QueryBuilder<T> {
  ...
  execute<U = QueryResult<{ count: number }>>(
    method: "run",
    mapResults?: boolean,
  ): Promise<U>;
...
export interface TruncateQueryBuilder<
  T extends object,
> extends RunQueryBuilder<T> {}
