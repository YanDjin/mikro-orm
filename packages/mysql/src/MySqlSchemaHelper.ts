...
export class MySqlSchemaHelper extends SchemaHelper {
  ...
  async getAllIndexes(
    ...
    for (const index of allIndexes) {
      ...
      if (
        ...
        indexDef.expression = this.getCreateIndexSQL(
          index.table_name,
          indexDef,
          !!index.expression,
        );
  ...
  async getAllColumns(
    ...
    const extra = (val: string) =>
      val
        .replace(
          /auto_increment|default_generated|(stored|virtual) generated/i,
          "",
        )
        .trim();
    const ret = {} as Dictionary;
  ...
  override getPreAlterTable(tableDiff: TableDifference, safe: boolean): string {
    ...
    const pk = Object.values(tableDiff.removedIndexes).find(
      (idx) => idx.primary,
    );
  ...
  override configureColumnDefault(
    ...
    if (changedProperties || column.default !== undefined) {
      if (column.default == null) {
        ...
        col.defaultTo(
          knex.raw(column.default + (column.extra ? " " + column.extra : "")),
        );
      }
    }
  ...
  override createTableColumn(
    ...
    if (column.mappedType instanceof MediumIntType) {
      return table.specificType(
        column.name,
        this.getColumnDeclarationSQL(column, true),
      );
    }
  ...
  override getForeignKeysSQL(tableName: string, schemaName?: string): string {
    return (
      `select distinct k.constraint_name as constraint_name, k.column_name as column_name, k.referenced_table_name as referenced_table_name, k.referenced_column_name as referenced_column_name, c.update_rule as update_rule, c.delete_rule as delete_rule ` +
      `from information_schema.key_column_usage k ` +
  ...
  private async supportsCheckConstraints(
    connection: AbstractSqlConnection,
  ): Promise<boolean> {
