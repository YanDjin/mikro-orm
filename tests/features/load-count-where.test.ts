...
describe("Collection.loadCount where option", () => {
  ...
  test("Collection.loadCount with where option should work", async () => {
    ...
    const event2 = orm.em.create(Event, {
      id: 2,
      name: "MikroORM second party!",
    });
    ...
    const booking1 = orm.em.create(Booking, {
      id: 1,
      endUser,
      event: event1.id,
    });
    ...
    const booking2 = orm.em.create(Booking, {
      id: 2,
      endUser,
      event: event2.id,
    });
    ...
    const userFirstPartyBookingCount = await endUser.bookings.loadCount({
      where: { event: event1 },
    });
