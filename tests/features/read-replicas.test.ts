...
import { MySqlDriver, wrap } from "@yandjin-mikro-orm/mysql";
...
describe("read-replicas", () => {
  ...
  describe("when preferReadReplicas is true (default behaviour)", () => {
    test("will prefer replicas for read operations outside a transaction", async () => {
      ...
      expect(mock.mock.calls[1][0]).toMatch(
        /insert into `author2`.*via write connection '127\.0\.0\.1'/,
      );
      expect(mock.mock.calls[2][0]).toMatch(
        /insert into `book2`.*via write connection '127\.0\.0\.1'/,
      );
      ...
      expect(mock.mock.calls[4][0]).toMatch(
        /select `a0`\.\*, `a1`\.`author_id` as `address_author_id` from `author2` as `a0` left join `address2` as `a1` on `a0`\.`id` = `a1`\.`author_id` where `a0`.`id` = \? limit \?.*via read connection 'read-\d'/,
      );
      ...
      expect(mock.mock.calls[8][0]).toMatch(
        /update `author2` set `name` = \?, `updated_at` = \? where `id` = \?.*via write connection '127\.0\.0\.1'/,
      );
      ...
      expect(mock.mock.calls[10][0]).toMatch(
        /select `a`.* from `author2` as `a` where `a`.`name` like \?.*via write connection '127\.0\.0\.1'/,
      );
      ...
      expect(mock.mock.calls[12][0]).toMatch(
        /select.*via write connection '127\.0\.0\.1'/,
      );
      expect(mock.mock.calls[13][0]).toMatch(
        /update.*via write connection '127\.0\.0\.1'/,
      );
      expect(mock.mock.calls[14][0]).toMatch(
        /commit.*via write connection '127\.0\.0\.1'/,
      );
    });
    ...
    test("can explicitly set connection type for find operations", async () => {
      ...
      expect(mock.mock.calls[6][0]).toMatch(
        /via write connection '127\.0\.0\.1'/,
      );
      ...
      await orm.em.upsertMany(Author2, [
        {
          ...
          email: "dany@dragonstone.ts",
          born: "1990-03-23",
        },
        {
          name: "Jaime Lannister",
          email: "jaime@casterlyrock.ts",
          born: "1980-03-23",
        },
      ]);
      expect(mock.mock.calls[2][0]).toMatch(
  ...
  describe("when preferReadReplicas is false", () => {
    test("will always use write connections", async () => {
      ...
      expect(mock.mock.calls[4][0]).toMatch(
        /select `a0`\.\*, `a1`\.`author_id` as `address_author_id` from `author2` as `a0` left join `address2` as `a1` on `a0`\.`id` = `a1`\.`author_id` where `a0`.`id` = \? limit \?.*via write connection '127\.0\.0\.1'/,
      );
      expect(mock.mock.calls[5][0]).toMatch(
        /select `a0`\.\*, `a1`\.`author_id` as `address_author_id` from `author2` as `a0` left join `address2` as `a1` on `a0`\.`id` = `a1`\.`author_id` where `a0`.`id` = \? limit \?.*via write connection '127\.0\.0\.1'/,
      );
      expect(mock.mock.calls[6][0]).toMatch(
        /select `a0`\.\* from `author2` as `a0` where `a0`.`id` = \? limit \?.*via write connection '127\.0\.0\.1'/,
      );
  ...
  test("find with different schema", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select \`b0\`.*, \`b0\`.price * 1.19 as \`price_taxed\`, \`t1\`.\`id\` as \`test_id\` from \`${schema}\`.\`book2\` as \`b0\` left join \`${schema}\`.\`test2\` as \`t1\` on \`b0\`.\`uuid_pk\` = \`t1\`.\`book_uuid_pk\` where \`b0\`.\`author_id\` is not null and \`b0\`.\`publisher_id\` is not null`,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      "select `b0`.*, `b0`.price * 1.19 as `price_taxed`, `t1`.`id` as `test_id` from `book2` as `b0` left join `test2` as `t1` on `b0`.`uuid_pk` = `t1`.`book_uuid_pk` where `b0`.`author_id` is not null and `b0`.`publisher_id` is not null",
    );
