...
export class PostgreSqlSchemaHelper extends SchemaHelper {
  ...
  override async loadInformationSchema(
    ...
    const nativeEnums = await this.getNativeEnumDefinitions(
      connection,
      schemas,
    );
    ...
    for (const t of tables) {
      ...
      const enums = await this.getEnumDefinitions(
        connection,
        checks[key] ?? [],
      );
  ...
  async getAllIndexes(
    ...
    for (const index of allIndexes) {
      ...
      if (
        index.index_def.some((col: string) => col.match(/[(): ,"'`]/)) ||
  ...
  async getAllColumns(
    ...
    const str = (val: string | number | undefined) =>
      val != null ? "" + val : val;
    const ret = {} as Dictionary;
    ...
    for (const col of allColumns) {
      ...
      ret[key] ??= [];
      let type =
        col.data_type.toLowerCase() === "array"
      ...
      const column: Column = {
        ...
        default: str(
          this.normalizeDefaultValue(col.column_default, col.length),
        ),
        unsigned: increments,
  ...
  override createTableColumn(
    ...
    if (
      column.mappedType instanceof EnumType &&
      column.enumItems?.every((item) => Utils.isString(item))
    ) {
      return table.enum(column.name, column.enumItems);
    }
  ...
  override configureColumn(
    ...
    const guard = (key: string) =>
      !changedProperties || changedProperties.has(key);

    Utils.runIfNotEmpty(
      () => col.nullable(),
      column.nullable && guard("nullable"),
    );
    Utils.runIfNotEmpty(
      () => col.notNullable(),
      !column.nullable && guard("nullable"),
    ...
    Utils.runIfNotEmpty(
      () => col.unsigned(),
      column.unsigned && guard("unsigned"),
    );
    Utils.runIfNotEmpty(
      () => col.comment(column.comment!),
      column.comment && !changedProperties,
    );
  ...
  override getPreAlterTable(tableDiff: TableDifference, safe: boolean): string {
    ...
    /* istanbul ignore next */
    const name =
      (schemaName && schemaName !== this.platform.getDefaultSchemaName()
        ? schemaName + "."
        : "") + tableName;
    ...
    for (const col of changedEnums) {
      if (
        !col.fromColumn.nativeEnumName &&
        col.column.nativeEnumName &&
        col.fromColumn.default
      ) {
        ret.push(
          `alter table ${quotedName} alter column "${col.column.name}" drop default`,
        );
      }
      ...
      if (!col.fromColumn.nativeEnumName) {
        ...
        ret.push(
          `alter table ${quotedName} drop constraint if exists "${constraintName}"`,
        );
      }
    }
    ...
    const uuids = Object.values(tableDiff.changedColumns).filter(
      (col) =>
        col.changedProperties.has("type") && col.fromColumn.type === "uuid",
    );
  ...
  override getPostAlterTable(
    ...
    for (const col of changedEnums) {
      if (
        ...
        ret.push(
          `alter table ${quotedName} alter column "${col.column.name}" set default ${col.column.default}`,
        );
      }
    }
  ...
  override getAlterColumnAutoincrement(
    tableName: string,
    column: Column,
    schemaName?: string,
  ): string {
    ...
    if (column.autoincrement) {
      const seqName = this.platform.getIndexName(
        tableName,
        [column.name],
        "sequence",
      );
      ...
      ret.push(
        `select setval('${seqName}', (select max(${quoted(column.name)}) from ${quoted(name)}))`,
      );
      ret.push(
        `alter table ${quoted(name)} alter column ${quoted(column.name)} set default nextval('${seqName}')`,
      );
    } else if (column.default == null) {
      ret.push(
        `alter table ${quoted(name)} alter column ${quoted(column.name)} drop default`,
      );
    }
  ...
  override getChangeColumnCommentSQL(
    tableName: string,
    to: Column,
    schemaName?: string,
  ): string {
    const name = this.platform.quoteIdentifier(
      (schemaName && schemaName !== this.platform.getDefaultSchemaName()
        ? schemaName + "."
        : "") + tableName,
    );
  ...
  override normalizeDefaultValue(defaultValue: string, length: number) {
    if (!defaultValue || (typeof defaultValue as unknown) !== "string") {
      return super.normalizeDefaultValue(
        defaultValue,
        length,
  ...
  override getManagementDbName(): string {
    return (
      this.platform.getConfig().get("schemaGenerator", {} as Dictionary)
