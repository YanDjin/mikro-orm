...
const initialMetadataProcessor: MetadataProcessor = (metadata, platform) => {
  ...
  const virtualEntityBase = metadata.find(
    (entity) => entity.className === "Author2",
  )!;
      ...
      entity.addProperty({
        ...
        fieldNames: [
          platform
            .getConfig()
            .getNamingStrategy()
            .propertyToColumnName("secondsSinceLastModified"),
        ],
        columnTypes: ["int"],
        type: "number",
        lazy: true,
        formula: (alias) =>
          `TIMESTAMPDIFF(SECONDS, NOW(), ${alias}.updated_at)`,
      });
  ...
  const virtualEntityMeta2 = new EntityMetadata<any>({
    ...
    collection: platform
      ...
      .classToTableName("AuthorPartialView"),
    virtual: true,
    expression: (em: typeof orm.em) =>
      em.createQueryBuilder<Author2>("Author2").select(["name", "email"]),
    comment: "test",
  });
  ...
  const embeddableEntityMeta = new EntityMetadata({
    ...
    collection: platform
      .getConfig()
      .getNamingStrategy()
      .classToTableName("IdentitiesContainer"),
    embeddable: true,
  });
  ...
  const companyOwner2def = new EntityMetadata({
    ...
    collection: platform
      .getConfig()
      .getNamingStrategy()
      .classToTableName("CompanyOwner2"),
    virtual: true,
    relations: [],
  });
...
const processedMetadataProcessor: GenerateOptions["onProcessedMetadata"] = (
  metadata,
  platform,
) => {
  metadata.forEach((entity) => {
