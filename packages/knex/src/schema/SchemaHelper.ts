...
export abstract class SchemaHelper {
  ...
  async getForeignKeys(
    ...
    const fks = await connection.execute<any[]>(
      this.getForeignKeysSQL(tableName, schemaName),
    );
  ...
  async loadInformationSchema(
    ...
    for (const t of tables) {
      ...
      const fks = await this.getForeignKeys(
        connection,
        table.name,
        table.schema,
      );
  ...
  hasNonDefaultPrimaryKeyName(table: DatabaseTable): boolean {
    ...
    const defaultName = this.platform.getDefaultPrimaryName(
      table.name,
      pkIndex.columnNames,
    );
  ...
  createTableColumn(
    ...
    if (
      ...
      const primaryKey =
        !changedProperties && !this.hasNonDefaultPrimaryKeyName(fromTable);
  ...
  configureColumn(
    ...
    Utils.runIfNotEmpty(
      () => col.notNullable(),
      !column.nullable && !column.generated,
    );
  ...
  configureColumnDefault(
    ...
    if (changedProperties) {
      Utils.runIfNotEmpty(
        () =>
          col.defaultTo(
            column.default == null ? null : knex.raw(column.default),
          ),
        guard("default"),
      );
    } else {
  ...
  mapForeignKeys(
    ...
    return fks.reduce((ret, fk: any) => {
      if (ret[fk.constraint_name]) {
        ...
        ret[fk.constraint_name].referencedColumnNames.push(
