import type { Dictionary, Platform } from "@yandjin-mikro-orm/core";
import {
  Embeddable,
  Embedded,
  Entity,
...
describe("embedded entities in mongo", () => {
  ...
  test("create collections", async () => {
    ...
    expect(createCollection.mock.calls.map((c) => c[0])).toEqual([
      "custom-user",
      "parent",
      "user",
  ...
  test("persist and load", async () => {
    ...
    expect(mock.mock.calls[4][0]).toMatch(
      /db\.getCollection\('user'\)\.find\({ address1_city: 'London 1', address1_postalCode: '123' }, {}\)\.limit\(1\).toArray\(\);/,
    );
  ...
  test("validation of object embeddables (GH issue #466)", async () => {
    ...
    await expect(orm.em.persistAndFlush(user)).rejects.toThrow(
      `Trying to set User.address4_postalCode of type 'string' to '123' of type 'number'`,
    );
  });
  ...
  test("#assign() works with embeddables", async () => {
    const jon = new User();
    orm.em.assign(jon, {
      address1: { city: "1", country: "2", postalCode: "3", street: "4" },
    });
    expect(jon.address1).toMatchObject({
      city: "1",
      country: "2",
      postalCode: "3",
      street: "4",
    ...
    expect(jon.address4).toMatchObject({
      city: "41",
      country: "42",
      postalCode: "43",
      street: "44",
  ...
  test("embeddable with inherited properties (GH issue #1049)", async () => {
    ...
    const j2 = await orm.em.findOneOrFail(User, {
      address1: { street: "Rainbow st. 1" },
    });
  ...
  test("assign entity changes on embeddables (GH issue 1083)", async () => {
    ...
    const mock = mockLogger(orm);
    john = await orm.em.findOneOrFail(User, {
      address1: { street: "Rainbow st. 1" },
    });
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('user'\)\.updateMany\({ _id: .* }, { '\$set': { address1_street: 'Rainbow st. 33', address1_postalCode: '003', address1_country: 'UKK' } }, {}\);/,
    );

    const j1 = await orm.em.findOneOrFail(User, {
      address1: { street: "Rainbow st. 33" },
    });
  ...
  test("embeddables should work with custom type properties", async () => {
    ...
    const retrievedUser = await orm.em.findOneOrFail(CustomUser, {
  ...
  test("assigning to array embeddables (GH #1699)", async () => {
    ...
    const u1 = await orm.em.findOneOrFail(User, {
      addresses: { $eq: [address1, address2] },
    });
