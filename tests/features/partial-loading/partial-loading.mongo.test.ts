...
describe("partial loading (mongo)", () => {
  ...
  test("partial nested loading (1:m)", async () => {
    ...
    const r1 = await orm.em.find(Author, god, {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('author'\)\.find\({ _id: ObjectId\('.*'\) }, { projection: { _id: 1 } }\)\.toArray\(\);/,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ author: { '\$in': \[ ObjectId\('.*'\) ] } }, { projection: { _id: 1, author: 1, title: 1 } }\)\.sort\(\[ \[ 'title', 1 ] ]\)\.toArray\(\);/,
    );
    orm.em.clear();
  ...
  test("partial nested loading (m:1)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ _id: ObjectId\('.*'\) }, { projection: { _id: 1, title: 1, author: 1 } }\)\.toArray\(\);/,
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('author'\)\.find\({ _id: { '\$in': \[ ObjectId\('.*'\) ] } }, { projection: { _id: 1, email: 1 } }\)\.toArray\(\);/,
    );
    orm.em.clear();
    mock.mock.calls.length = 0;
  ...
  test("partial nested loading (m:n)", async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "db.getCollection('book-tag').find({}, { projection: { _id: 1, name: 1 } }).toArray();",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ tags: { '\$in': \[ ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\) ] } }, { projection: { _id: 1, title: 1, tags: 1 } }\)\.toArray\(\);/,
    );
    orm.em.clear();
    mock.mock.calls.length = 0;
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "db.getCollection('book-tag').find({ name: 't1' }, { projection: { _id: 1, name: 1 } }).toArray();",
    );
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ tags: { '\$in': \[ ObjectId\('.*'\) ] } }, { projection: { _id: 1, title: 1, tags: 1 } }\)\.toArray\(\);/,
    );
  });
  ...
  test("partial nested loading (mixed)", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      /db\.getCollection\('books-table'\)\.find\({ tags: { '\$in': \[ ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\), ObjectId\('.*'\) ] } }, { projection: { _id: 1, title: 1, tags: 1, author: 1 } }\)\.toArray\(\);/,
    );
