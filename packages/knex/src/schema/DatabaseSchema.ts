...
export class DatabaseSchema {
  ...
  getTable(name: string): DatabaseTable | undefined {
    return this.tables.find(
      (t) => t.name === name || `${t.schema}.${t.name}` === name,
    );
  }
  ...
  static fromMetadata(
    ...
    for (const meta of metadata) {
      meta.props
        ...
        .forEach(
          (prop) =>
            (nativeEnums[prop.nativeEnumName!] =
    ...
    const rootMetadata = Utils.uniqueBy(
      metadata.map((e) => e.getStiRoot()),
      "name",
    );
  ...
  private static shouldHaveColumn(
    ...
    const getRootProperty: (prop: EntityProperty) => EntityProperty = (
      prop: EntityProperty,
    ) =>
  ...
  prune(schema: string | undefined, wildcardSchemaTables: string[]): void {
    ...
    this.namespaces.forEach(
      (ns) =>
        !this.tables.find((t) => t.schema === ns) && this.namespaces.delete(ns),
    );
  }
}
