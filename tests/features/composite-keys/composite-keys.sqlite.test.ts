...
import { AbstractSqlConnection, SqliteDriver } from "@yandjin-mikro-orm/sqlite";
...
describe("composite keys in sqlite", () => {
  ...
  test("working with composite entity", async () => {
    ...
    const p1 = await orm.em.findOneOrFail(
      FooParam2,
      [param.bar.id, param.baz.id] as const,
      { populate: ["bar"] },
    );
  ...
  test("removing composite entity in m:n relationship, one of entity is composite (GH #1961)", async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "delete from `user2_sandwiches` where (`sandwich_id` = 2 and `user2_first_name` = 'Henry' and `user2_last_name` = 'Doe 2') or (`sandwich_id` = 3 and `user2_first_name` = 'Henry' and `user2_last_name` = 'Doe 2')",
    );
