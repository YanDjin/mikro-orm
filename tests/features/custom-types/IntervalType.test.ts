...
test("interval columns (postgres)", async () => {
  ...
  await expect(orm.schema.getCreateSchemaSQL()).resolves.toMatch(
    '"duration_buggy" interval null',
  );
  ...
  expect(mock.mock.calls[1][0]).toMatch(
    'insert into "something" ("duration_buggy") values (\'1s\') returning "id"',
  );
  expect(mock.mock.calls[2][0]).toMatch("commit");
  expect(mock.mock.calls[3][0]).toMatch(
    'select "s0".* from "something" as "s0" where "s0"."id" = 1 limit 1',
  );
  expect(mock.mock.calls[4][0]).toMatch("begin");
  expect(mock.mock.calls[5][0]).toMatch(
    'update "something" set "duration_buggy" = \'5 seconds\' where "id" = 1',
  );
...
test("interval columns (sqlite)", async () => {
  ...
  expect(mock.mock.calls[4][0]).toMatch("begin");
  expect(mock.mock.calls[5][0]).toMatch(
    "update `something` set `duration_buggy` = '5s' where `id` = 1",
  );
  expect(mock.mock.calls[6][0]).toMatch("commit");
